...
/* Template for debugging
		int numberChars = sprintf ((char*)&gTextString3,
												" SDisThem::xxx (entered routine. %s", 
												gEndOfLine);
...
//		ioBufferPtr = (HUCharPtr)StripAddress ( (void*)ioBufferPtr );
...
//	symbolToPalettePtr = (unsigned char*)StripAddress ( (void*)symbolToPalettePtr );
//	addressMode = true32b;  
...

{
	...
	localClassGroupsPtr = (SInt16*)GetHandlePointer(
												localClassGroupsHandle, kLock, kNoMoveHi);
...
//				if (gImageWindowInfoPtr->grafPortType == kGrafType) 
//					DisplayPImage (displaySpecsPtr);
...

{
	...
	DrawPopUpMenuItem (dialogPtr, itemNumber, 
									gPopUpClassGroupMenu, gClassGroupSelection, TRUE);
...

{
	...
	displayClassGroupsPtr = (short int*)GetHandlePointer(
							displaySpecsPtr->displayClassGroupsHandle, kNoLock, kNoMoveHi);
...

{
	...
	classGroupPtr = (SInt16*)GetHandlePointer(classGroupHandle, kNoLock, kNoMoveHi);
...

{
	...
	if (listHandle != NULL && classNameHandle != NULL && numberClasses > 0)
		...
		classNamePtr = (HCharPtr)GetHandlePointer(classNameHandle, kLock, kNoMoveHi);
...
//			displaySpecsPtr->pixelSize = 32;
		...
		if (displaySpecsPtr->filePaletteFlag)
			...
			FileStringPtr fileNamePtr = (FileStringPtr)GetFileNamePPointer (gImageFileInfoPtr);
...

{
	...
	if (listHandle != NULL && groupNameHandle != NULL && numberGroups > 0)
		...
		groupNamePtr = (char*)GetHandlePointer(
														groupNameHandle, kLock, kNoMoveHi);
