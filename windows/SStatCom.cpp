...
//									All rights reserved.
//
//	File:						SStatCom.cpp (statCompute.c)
//
...

{
	...
	if (classNamesPtr[classStorage].numberOfTrainFields > 0 && 
		...
		if (gProjectInfoPtr->keepClassStatsOnlyFlag)
			...
			ComputeMeanStdDevVector (classChanPtr, 
									classSumSquaresPtr, 
									gProjectInfoPtr->numberStatisticsChannels, 
...

{
	...
	if (numberOutputChannels <= 0 || !classChannelStatsPtr || 
											!classCovariancePtr)  
																							return;
	...
	if (covarianceStatsToUse == kEnhancedStats &&
														classNamesPtr->modifiedStatsFlag)
		{
				// Get pointers to the memory for the first order and 				
				// second order field statistics												
			...
			if (outputStatisticsCode == kMeanCovariance)
				...
				if (covarianceStatsToUse == kLeaveOneOutStats)
					...
					if (inputCommonCovariancePtr != NULL &&
						...
						GetLOOCovariance (	
										classNamesPtr->mixingParameterCode,
	...
	if (gTransformationMatrix.eigenFeatureHandle && 
		...
		if (gTransformationMatrix.createdByCode < 16)
			...
			for (index=0; index<numberEigenFeatures; index++)
				{
				while ( eigenFeaturePtr[index] != (UInt16)gProjectInfoPtr->channelsPtr[index2] &&
			...
			{
					// This section is for the offset-gain type of transformation. The feature
					// vector will include all project channels.
			
//			UInt32 processorNumberFeatures = numberFeatures;
					
			for (index=0; index<(UInt32)numberFeatures; index++)
				{
				while ( outputFeaturePtr[index] != (UInt16)gProjectInfoPtr->channelsPtr[index2] &&
							index2 < numberStatisticsChannels)
					index2++;
	...
	while (fieldNumber != -1)
		...
		if (fieldIdentPtr[fieldNumber].fieldType == kTrainingType)
			...
			if (fieldIdentPtr[fieldNumber].loadedIntoClassStats)
				numberOfPixelsLoadedInClass += fieldIdentPtr[fieldNumber].numberPixelsUsedForStats;
...

{
	...
	if (variancesEqualFlag)
		...
		for (feature1=0; feature1<numberFeatures; feature1++)
			...
			if (squareMatrixFlag)
				{
						// Skip the upper right portion of matrix. It will be
						// handled below with "CopyLowerToUpperSquareMatrix".
	...
	for (classIndex=0; classIndex<numberClasses; classIndex++)
		...
		if (classNamesPtr[classStorage].covarianceStatsToUse == kEnhancedStats)
			...
			if (!classNamesPtr[classStorage].modifiedStatsFlag)
				...
				classNamesPtr[classStorage].covarianceStatsToUse = savedCovarianceStatsToUse;
	...
	if (statsWindowMode >= kUpdateProject && statsWindowMode <= kUpdateField)
		...
		if (requestFlag)
			...
			if (LoadSpecifiedStringNumberStringP (
											kAlertStrID, 
											IDS_Alert42,	// Update statistics before continuing?
											(char*)gTextString,
											(char*)gTextString2,
											TRUE,
											(char*)&gTextString3[1] ) )
...
*/					
					...
					if (returnCode == 1)	
						{
						returnCode = UpdateProjectLOOStats (
														statsWindowMode,
														fileIOInstructionsPtr);	
