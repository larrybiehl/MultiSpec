...
//
//	Brief description:	This file contains routines that release memory
//								assigned to the structure related to the various
//								non-project processors.
//
...

{
	if (*clusterSpecsHandlePtr != NULL)
		...
		gClusterSpecsPtr = (ClusterSpecsPtr)GetHandlePointer(
															*clusterSpecsHandlePtr,
															kLock,
															kNoMoveHi);
				
				// Release memory for cluster feature pointer vector if it has		
				// been allocated.																
				
		UnlockAndDispose (gClusterSpecsPtr->featureHandle);
				
				// Release memory for cluster channels vector if it has				
				// been allocated.																
				
		UnlockAndDispose (gClusterSpecsPtr->channelsHandle);
				
				// Release memory for cluster symbols vector if it has been 		
				// allocated.																		
				
		UnlockAndDispose (gClusterSpecsPtr->symbolsHandle);
				
				// Release memory for cluster classes vector if it has been 		
				// allocated.																		
...

{
	...
	if (*evaluateCovarianceSpecsHPtr != NULL)
		...
		evaluateCovarianceSpecsPtr = (EvaluateCovarianceSpecsPtr)GetHandlePointer(
				...
				// Release memory for classes handle if it has				
				// been allocated.																
...
{
	*evaluateTransformSpecsHPtr = 
								UnlockAndDispose (*evaluateTransformSpecsHPtr);
			...
			
{
	...
	if (*listResultsSpecsHandlePtr != NULL)
		...
		*listResultsSpecsHandlePtr = 
										UnlockAndDispose (*listResultsSpecsHandlePtr);
			...
			
{
	...
	if (*principalCompSpecsHPtr != NULL)
		...
		principalCompSpecsPtr = (PrincipalCompSpecsPtr)GetHandlePointer(
				...
				// Release memory for principal component classes vector if it		
				// has been allocated.															
		
		UnlockAndDispose (principalCompSpecsPtr->classHandle);
				
				// Release memory for principal component feature pointer 			
				// vector if it has been allocated.											
				
		UnlockAndDispose (principalCompSpecsPtr->featureHandle);
				
				// Release memory for principal component channels vector if 		
				// it has been allocated.														
				
		UnlockAndDispose (principalCompSpecsPtr->channelsHandle);
			
				// Make certain that memory for the covariance matrix and			
				// and mean vector have been released.										
	...
	if (*reformatOptionsHPtr != NULL)
		...
		if (reformatOptionsPtr->rectifyImageOptionsPtr)
			reformatOptionsPtr->rectifyImageOptionsPtr->rectifyChannelPtr = 
				...
				// Release memory for rectification parameters if it 					
				// has been allocated.															
		...
		reformatOptionsPtr->rectifyImageOptionsPtr = (RectifyImageOptionsPtr)
			CheckAndDisposePtr ((Ptr)reformatOptionsPtr->rectifyImageOptionsPtr);
				...
				// Release memory for reformat feature pointer 							
				// vector if it has been allocated.											
...
//													reformatOptionsPtr->channelPtr);
		UnlockAndDispose (reformatOptionsPtr->channelHandle);
				...
				// Release memory for transform coeffiecents vector if 				
				// it has been allocated.														
...
//													reformatOptionsPtr->coefficientsPtr);
		UnlockAndDispose (reformatOptionsPtr->coefficientsHandle);
				...
				// Release memory for reformat principal component pointer 			
				// vector if it has been allocated.											
...
//													reformatOptionsPtr->pcChannelPtr);
		UnlockAndDispose (reformatOptionsPtr->pcChannelHandle);
				...
				// Release memory for transform channels vector if 					
				// it has been allocated.														
...
//													reformatOptionsPtr->transformOperatorPtr);
		UnlockAndDispose (reformatOptionsPtr->transformOperatorHandle);
				...
				// Release memory for reformat classes vector if it 					
				// has been allocated.															
			...
			
{
	...
	if (*statisticsImageSpecsHPtr != NULL)
		...
		statisticsImageSpecsPtr = (StatisticsImageSpecsPtr)GetHandlePointer(
				...
				// Release memory for statistics image classes vector if it			
				// has been allocated.															
		
		UnlockAndDispose (statisticsImageSpecsPtr->classHandle);
			
				// Release memory for statistics image field vector if it			
				// has been allocated.															
		
		UnlockAndDispose (statisticsImageSpecsPtr->fieldHandle);
				
				// Release memory for statistics image feature pointer 				
				// vector if it has been allocated.											
				
		UnlockAndDispose (statisticsImageSpecsPtr->featureHandle);
				
				// Release memory for statistics image channels vector if 			
				// it has been allocated.														
