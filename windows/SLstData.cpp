...
//	Language:				C
//
//	System:					Macintosh Operating and Windows Operating System
//
...

{
	...
	DrawPopUpMenuItem (dialogPtr, itemNumber, 
							gPopUpListDataFormatMenu, gListDataFormatCode, TRUE );
	...
	
{
	...
	SetDLogControl (
		dialogPtr, 24, (gListDataSpecsPtr->outputStorageType & 0x0001) );
	...
	SetDLogControl (dialogPtr, 29, 
									gListDataSpecsPtr->fieldTypeCode & kTrainingType);
	
			// Set control for including test fields.										
			
	SetDLogControl (dialogPtr, 30, 
									gListDataSpecsPtr->fieldTypeCode & kTestingType);
...
{	
	...
	SInt16* featurePtr = (SInt16*)GetHandlePointer (
						listDataSpecsPtr->featureHandle, kNoLock, kNoMoveHi);
	...
	SInt16* channelsPtr = (SInt16*)GetHandlePointer(
									listDataSpecsPtr->channelsHandle,
									kNoLock,
									kNoMoveHi);																
	...
	if (GetCoordinateViewUnits (listDataSpecsPtr->windowInfoHandle) == kDMSLatLongUnitsMenuItem)
...
#	if defined multispec_lin
			//Disable all active windows except list data window in the wxwidgets linux version.
		if (gListDataSpecsPtr->graphDataFlag)
			m_windowDisabler = new wxWindowDisabler((wxWindow*)gGraphRecordPtr->graphViewCPtr->m_frame);  
#	endif
					...
					if (includePixelFlag)
						...
						if (latLongValueFlag)
							...
							if (gListDataSpecsPtr->latLongUnitsCode == kDMSLatLongUnitsMenuItem)
								...
								}		// end "else ...->latLongUnitsCode == kDecimalLatLongUnitsMenuItem"
				...
				for ( index=0; index<localNumberChannels; index++)
					...
					if (index == 0)
						...
						charIndex += 4;
								...
								// Put a space in the string in place of the line number if needed.
			...
			
{
	...
	if (returnFlag && gListDataSpecsPtr->projectFlag)
		{
		bytesNeeded =
				(SInt32)gProjectInfoPtr->numberStatisticsFields * sizeof(SInt16);
		fieldPtr = (SInt16*)CheckHandleSize ( 
										&gListDataSpecsPtr->fieldHandle,
										&returnFlag, 
										&changedFlag, 
										bytesNeeded);
		if (changedFlag)
			...
			// If memory is not full, check memory for									
	...
	if (returnFlag)
		...
		if (gListDataSpecsPtr->channelSet == kAllMenuItem)		
			LoadFeatureVector (kNoTransformation,
									(short int*)&gListDataSpecsPtr->numberChannels,
									featurePtr,
									gListDataSpecsPtr->numberChannels);
