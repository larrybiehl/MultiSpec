...
#				endif
		...
		if (continueFlag)													
			...
			char* fileNamePtr = (char*)GetFileNameCPointer (inputFileStreamPtr);
			sprintf ((char*)&gTextString, 
							"    ROI text file = '%s'%s",
							fileNamePtr,
							gEndOfLine);
			...
			
{
	...
	if (continueFlag)
		...
		if (continueFlag)
			{
			if (CompareStringsNoCase (*inputStringPtrPtr, (UCharPtr)"; ENVI Output of ROIs:", 19) == 0)
//			if (strncmp (*inputStringPtrPtr, (UCharPtr)"; ENVI Output of ROIs", 19) == 0)
	...
	if (pointsPerClassVector != NULL)
		...
		for (index=0; index<numberClasses; index++)
			...
			while (continueFlag && lineNotFoundFlag)
				...
				if (errCode == noErr)
					{
					if (CompareStringsNoCase (inputStringPtr, (UCharPtr)"; ROI name:", 11) == 0)
						{
						ENVI_ROIToThematicGetClassName (inputStringPtr, classNamePtr);
						classNamePtr += 32;
			...
			if (continueFlag)
				...
				if (continueFlag && 
						CompareStringsNoCase (inputStringPtr, (UCharPtr)"; ROI rgb value:", 16) == 0)
					{
					returnCode = sscanf ((char*)inputStringPtr, 
			...
			if (continueFlag)
				...
				if (continueFlag &&
							CompareStringsNoCase (inputStringPtr, (UCharPtr)"; ROI npts:", 11) == 0)
					{
					returnCode = sscanf ((char*)inputStringPtr, 
													"; ROI npts: %d",
