...
{	
	...
	DrawPopUpMenuItem (dialogPtr, itemNumber, 
										gPopUpAllSubsetMenu, gClassAreaSelection, TRUE);
...
{	
	...
	descriptionPtr = (char*)GetHandlePointer(
													descriptionHandle, kNoLock, kNoMoveHi);
...

{
	...
	FileStringPtr fileNamePtr = (FileStringPtr)GetFileNameCPointer (gImageFileInfoPtr);
	CtoPstring (fileNamePtr, gTextString3);
	
			// Remove the .LAN or .GIS extension if it exits at the end of		
			// the name.																		
	...
	if (gImageFileInfoPtr->format == kTIFFType || gImageFileInfoPtr->format == kGeoTIFFType)
	...
	do
		...
		if (windowPtr != gActiveImageWindow)
			...
			compareWindowInfoPtr = (WindowInfoPtr)GetHandleStatusAndPointer(
									compareWindowInfoHandle, &handleStatus1, kNoMoveHi);
			
			compareFileInfoHandle = GetFileInfoHandle(compareWindowInfoHandle);
			compareFileInfoPtr = (FileInfoPtr)GetHandleStatusAndPointer (
									compareFileInfoHandle, &handleStatus2, kNoMoveHi);
			
			fileNamePtr = (FileStringPtr)GetFileNameCPointer (compareFileInfoPtr);
...
{                                        
	...
	if (groupOrderedFlag)
		...
		groupNamePtr = (char*)GetHandlePointer(
									gImageFileInfoPtr->groupNameHandle, kNoLock, kNoMoveHi);
	...
	if (classPtr != NULL)
		...
		if (clsfyVariablePtr->summaryCode != kGroupSummary)
			maxNameLength = (SInt16)GetClassNameMaxLength (
											numberClassAreas, 
											classAreaPtr,
											NULL );
...
//			break;
		...
		classifyVectorIndex = GetCountVectorIndex (
												clsfyVariablePtr,
												classAreaPtr,
												(UInt16)classIndex,
												groupOrderedFlag);
		...
		if (clsfyVariablePtr->summaryCode == kGroupSummary)
			...
			listGroupSummaryFlag = DetermineIfLastClassForGroup (
																			clsfyVariablePtr,
																			groupIndex,
																			(SInt16)classIndex,
																			classAreaPtr, 
...

{
	...
	if (clsfyVariablePtr->totalNumberSamples > 0)
		...
		continueFlag = ListReliabilityAccuracyLine (
												clsfyVariablePtr,
												classPtr, 
												numberTableListColumns, 
												outputFileStreamPtr, 
...
//				"   OVERALL PERFORMANCE ( %*lld / %*lld ) = %5.1f%%",
		...
		double kappaStatistic = ComputeKappaStatistic (
														clsfyVariablePtr,
														classPtr, 
														numberClasses,
														&kappaVariance);
...

{
	...
	if (savedSummaryCode & kFieldSummary)
		...
		continueFlag = ListFCGPerformanceTable (
									clsfyVariablePtr, 
									fieldType, 
									outputFileStreamPtr, 
									outputCodePtr, 
	...
	if ( (savedSummaryCode & kFieldSummary+kClassSummary) && continueFlag )
		...
		if ( !(savedSummaryCode & kGroupSummary) || 
														clsfyVariablePtr->tableType == kClassTable)
			{
			continueFlag = ListOverallPerformanceValues (
											clsfyVariablePtr, 
											outputFileStreamPtr, 
											outputCodePtr, 
											classPtr, 
	...
	if (continueFlag)	
		continueFlag = ListTableFootNotes (
											clsfyVariablePtr, 
											outputFileStreamPtr,
											outputCodePtr);
		...
		if (columnIndex == 0 && clsfyVariablePtr->backgroundIndexOffset == 1)
			...
			*stringPtr = '\t';
			...
			numberClassifiedSamples = classifyTotalVectorPtr[classifyVectorIndex];
					...
					// Compute the reliability or user accuracy percent correct for the class or 
					// group.					
...
//	SInt32								numberInputBytesNeeded; 
	...
	SInt16* classPtr = (SInt16*)GetHandlePointer(
							gListResultsSpecsPtr->classHandle, kNoLock, kNoMoveHi);	
	...
	if (continueFlag)
		...
		if (gProjectInfoPtr != NULL)
			clsfyVariablePtr->tableWidth = MAX( clsfyVariablePtr->tableWidth,
...
//									kMoveHi);
				...
				if (gImageFileInfoPtr->classDescriptionH != NULL)
					classSymbolPtr = (unsigned short int*)GetHandlePointer(
										gImageFileInfoPtr->classDescriptionH, kNoLock, kNoMoveHi);
...
{				
	if (thresholdResultsFlag)
		...
		if ( thresholdTypeCode == kMaxLikeMode )
			...
			LoadDItemString (dialogPtr, IDC_ProbabilityPrompt, (CharPtr)"\0Probabilities <");
			...
			HideDialogItem (dialogPtr, IDC_LRdegrees);			// 39
			LoadDItemString (dialogPtr, IDC_ProbabilityPrompt, (CharPtr)"\0CEM Values <");
			...
			LoadDItemString (dialogPtr, IDC_ProbabilityPrompt, (CharPtr)"\0SAM Values >");
...

{
	...
	continueFlag = TRUE;		
	classNamePtr = (char*)GetHandlePointer(
										fileInfoPtr->classDescriptionH, kNoLock, kNoMoveHi);
	...
	for (classNumber=0; classNumber<numberClasses; classNumber++)
		...
		if (groupOrderedFlag)
			...
			do
				...
				for (classIndex=startClassIndex; 
													classIndex<numberClasses; classIndex++)
					{
					if (classToGroupPtr[classIndex] == groupIndex)
						{
						foundFlag = TRUE;
		...
		pstr ((char*)&gTextString2, 
					(char*)&classNamePtr[classIndex*32], &strLength);
		gTextString2[strLength] = ' ';
		gTextString2[kMaxClassFieldNameLength] = kNullTerminator;
		stringPtr = (char*)&gTextString;
		sprintf (stringPtr, "    %4d: %s", classIndex+identifierOffset, (char*)gTextString2);
		stringPtr += 41;
...

{
	...
	classAreaPtr = (SInt16*)GetHandlePointer(
							gListResultsSpecsPtr->classAreaHandle, kNoLock, kNoMoveHi);
...

{
	...
	sprintf (*stringPtrPtr, "%*.*f ", columnWidth, columnPrec, 
...
{  
	...
	gListResultsSpecsPtr->areaConversionFactor = 
								GetDefaultAreaConversionFactor (
															gImageFileInfoPtr->mapProjectionHandle,
	...
	if (areaCode != kAreaType)
		...
		if (summaryUnitsCode == kAreaUnits)
			...
			double areaConversionFactor = GetDefaultAreaConversionFactor (
														...
														gImageWindowInfoPtr->windowInfoHandle);
			/*
			double difference = fabs(clsfyVariablePtr->variable1-areaConversionFactor);
			
			int numberChars = sprintf ((char*)&gTextString3,
												" SLstRslt::LoadListResultsTitle (difference, variable1, areaConversionFactor). %18.16f, %18.16f, %18.16f%s",
												difference,
												clsfyVariablePtr->variable1,
												areaConversionFactor,
												gEndOfLine);
