...
//		CMFileStream
// ---------------------------------------------------------------------------
//	Contruct a file stream object based on the name and location of the file
// represents by the input file stream object.  
...
//		CMFileStream( Ptr )
// ---------------------------------------------------------------------------
//	Contruct a FileStream from a Toolbox File System Specification after 
...
{   
	...
	if (length == 0)
		...
		length = (UInt16)StringLength ((void*)&inFilePathPtr[1]); 
...
{			
	...
	if (returnCode == kReturnASCII)
		{	
		//memcpy (&mFileASCIIName[1], 
		//			T2A(fileNamePtr), 
		//			(size_t)fileNamePtr[0]); 
		//
		//mFileASCIIName[0] = (UInt8)nameLength;
		...
		if (wideFileNamePtr[0] == 0)
			{
					// Force Pascal File Name to be generated from the
					// Full wide (unicode) path name.
...
#	if defined multispec_lin
		if (IsOpened())
			...
			wxFileOffset	cmarker = Tell();
					...
					// First see if the number of bytes left in file is less than bytes to read
...
#			endif	// end defined multispec_win
			...
			GetFileNameCPtr (kReturnUnicode);
					...
					// Now force the multibyte file name to be created if they are requested.
...
#			endif	// defined multispec_win
			...
			mFilePathName[0] = (TBYTE)StringLength(&mFilePathName[1]);
