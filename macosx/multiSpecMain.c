 ...
 
{
	...
	if (getrlimit (RLIMIT_NOFILE, &resourceLimits) == 0)
		{
		gNumberOfOpenFilesLimit = resourceLimits.rlim_cur;		
		gNumberOfOpenFilesHardLimit = MIN (resourceLimits.rlim_max, SInt32_MAX);
	...
	gNumberOfOpenFilesLimit -= 20;
			...
			// Now force a block of memory to be allocated which will be use by
			// the New operator.
	...
	InitializeMultiSpec ();
			...
			// Mark the initialization segment as being removeable for memory.	
			
	UnloadSeg (InitializeMultiSpec);
	/*			
	rlimit						resourceLimits;
