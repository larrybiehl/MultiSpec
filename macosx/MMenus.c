				...
				SInt16								menuItem)
{
	...
	if (windowType == kImageWindowType || windowType == kThematicWindowType)
		{
				// Handle Undo/Cut/Copy/Paste/Clear properly between WSScrap and the 
				// Clipboard.									
		...
		switch (menuItem)
			...
			case kCopy:
				...
				destinationRect.right = imageRect.right;
						...
						// Save the current clip region and set the clip region so that 
						// the selected area will be copied to the clipboard.															
				...
				if (gTempRegion1 != NULL)
					...
					ClipRect (&destinationRect);
							...
							// Save the destination rectangle; It may be needed to draw 
							// project fields.
...

{
	...
	numberChars = InsertCommasInNumberString ((char*)&freeMemoryText[1], 
															numberChars, 
															-1,
															numberChars);
	...
	numberChars = InsertCommasInNumberString ((char*)&contiguousMemoryText[1], 
															numberChars, 
															-1,
															numberChars);
	...
	numberChars = InsertCommasInNumberString ((char*)&freeBytesPossibleText[1], 
															numberChars, 
															-1,
															numberChars);
	...
	numberChars = InsertCommasInNumberString ((char*)&stackSpaceText[1], 
															numberChars, 
															-1,
															numberChars);
	...
	if (dialogPtr != NULL)
		...
		openFilesText[0] = InsertCommasInNumberString ((char*)&openFilesText[1], 
																		openFilesText[0], 
																		-1,
																		openFilesText[0]);
		LoadDItemString (dialogPtr, 12, (Str255*)openFilesText);
		...
		openFilesText[0] = InsertCommasInNumberString ((char*)&openFilesText[1], 
																		...
																		-1,
																		openFilesText[0]);
		LoadDItemString (dialogPtr, 14, (Str255*)openFilesText);
		...
		openFilesText[0] = InsertCommasInNumberString ((char*)&openFilesText[1], 
																		...
																		-1,
																		openFilesText[0]);
		LoadDItemString (dialogPtr, 16, (Str255*)openFilesText);
	      		...
	      		gProcessorCode = kAboutMultiSpecProcessor;
					...
					if (dialogPtr != NULL)
						...
						//HideDialogItem (dialogPtr, 1);
								// If option key is down, show the option for setting simpler 
								// version of MultiSpec
						if (gProjectInfoPtr == NULL && 
						...
						ShowDialogWindow (dialogPtr, 
												kMoveableAboutMultiSpecID, 
						...
						do 
						   ...
						   ModalDialog (gProcessorDialogFilterPtr, &itemHit);
							...
							if (itemHit == 15)
								...
								GetDialogItem (
											dialogPtr, itemHit, &theType, &theHandle, &theBox);
...

{
	...
	windowInfoPtr = (WindowInfoPtr)GetHandlePointer (gActiveWindowInfoH,
																		kLock);
														
	activeImageWindowInfoPtr = (WindowInfoPtr)GetHandlePointer (gActiveImageWindowInfoH,
																					kLock);
	...
	if (gUpdateEditMenuItemsFlag)
		...
		UpdateEditTextPaste (NULL);
			...
			if (TXNCanUndo (gOutputTextH, NULL))
					// Following only available in OSX 10.4 or later
			//if (TXNCanUndoAction (gOutputTextH, &actionName)) 
...

{
	...
	if (imageBaseAddressH == NULL && offscreenGWorld == NULL)
  	...
  	if (gPaletteWindow != NULL)
 		SetMenuItemText (gMultiSpecMenus[kPaletteM], 
								kShowClosePaletteW,
...

{
	...
	if (gActiveImageWindow == NULL && gProjectInfoPtr == NULL)
		...
		DisableMenuItem (
						gMultiSpecMenus[kReformatM], kReformatConvertMultispectralMenuItem);
		...
		EnableMenuItem (gMultiSpecMenus[kUtilitiesM], kPrincipalComponentsMenuItem);
	 	...
	 	if (gActiveImageWindow != NULL)
		   ...
		   if (projectWindowFlag || DetermineIfProjectPossible (
														activeImageWindowInfoPtr->windowInfoHandle))
				EnableMenuItem (gMultiSpecMenus[kProcessorM], kClusterMItem);
		   	
		   else	// ... windowType != kImageWindowType && !projectWindowFlag ... 
				DisableMenuItem (gMultiSpecMenus[kProcessorM], kClusterMItem);
	 		
		   if (gFullVersionFlag && (projectWindowFlag || DetermineIfProjectPossible (
													activeImageWindowInfoPtr->windowInfoHandle)))
		   ...
		   if (gFullVersionFlag && windowType == kImageWindowType)
		 		EnableMenuItem (gMultiSpecMenus[kUtilitiesM], 
										kPrincipalComponentsMenuItem);
		 		
		 	else	// windowType != kImageWindowType 
		 		DisableMenuItem (gMultiSpecMenus[kUtilitiesM], 
										kPrincipalComponentsMenuItem);
	...
	if (gFullVersionFlag)
		...
		DisableMenuItem (gMultiSpecMenus[kReformatM], 
								kReformatConvertENVIASCIIMenuItem);
...

{
	...
	if (gProjectInfoPtr == NULL)
		...
		/*		
		if (gActiveImageWindowInfoH != NULL && 
				((WindowInfoPtr)*gActiveImageWindowInfoH)->windowType == 
																					kImageWindowType &&
	...
	if (activeImageFileInfoPtr != NULL &&
			!activeImageFileInfoPtr->thematicType && 
						gQuickDrawVersion >= 0x00000230 &&
								!gOSXCoreGraphicsFlag)
		EnableMenuItem (gMultiSpecMenus[kReformatM], 
								kReformatConvertMultispectralMenuItem);
