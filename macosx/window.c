				...
				
{
	...
	if (horizontalChange == (SInt16)0x8000)
		...
		if (legendWidth < 
						((WindowInfoPtr)*windowInfoHandle)->legendTitleWidth + 30)
	...
	if (gActiveLegendWidth != legendWidth)
		...
		horizontalChange = legendWidth - gActiveLegendWidth;
				...
				// Erase the legend title to be ready for a new title to be			
...

{
	...
	if ((windowType == kImageWindowType || windowType == kThematicWindowType) && 
		...
		windowInfoPtr->mask.fileStreamHandle = 
				...
				// Save the palette object if available before closing other support memory.
		
		paletteObject = NULL;
		if (windowInfoPtr->displaySpecsHandle != NULL)
			paletteObject = ((DisplaySpecsPtr)*windowInfoPtr->displaySpecsHandle)->paletteObject;
		...
		HideWindow (theWindow);
				...
				// Update global variables before doing activate and update			
				//	events.																			
		...
		if (gSelectionGraphViewCPtr != NULL)
			...
			if (GetSelectionGraphImageWindow (gSelectionGraphViewCPtr) == theWindow)
						// If the selection graph window is empty, set the title to the default.
...
//			}		// end "if (thematicWindowFlag)" 
		...
		GetImageWindowName (NULL, 
									...
									gTextString, 
									FALSE);
		//SetWTitle (theWindow, (ConstStr255Param)&gTextString);		                      
...
//			{
			...
			SetRect (&gViewRect, 2, 16,
								gDefaultLegendWidth-4, gGrowAreaMinimum-2-kSBarWidth);
...
//			}		// end "if (thematicWindowFlag)" 
	...
	if (continueFlag)
		...
		HLock (fileInfoHandle);
		FileStringPtr fileNamePtr = (FileStringPtr)GetFileNamePPointer ((FileInfoPtr)*fileInfoHandle);
...
//
//	Software purpose:	The purpose of this routine is to create a palette
//							window.
//
//	Parameters in:		None
//
...
//	gTempRect = qd.screenBits.bounds;
	...
	#if TARGET_API_MAC_CARBON	
		theWindow = NewCWindow (NULL, &gTempRect, "\p", 1, 
													noGrowDocProc, front, FALSE, (SInt32)NULL);
		grafPortType = kCGrafType;
	#else		
		if ( gColorQDflag )
			{
			...
			theWindow = NewCWindow (*gPaletteWindowHandle, &gTempRect, "\p", 1, 
														noGrowDocProc, front, FALSE, (SInt32)NULL);
			grafPortType = kCGrafType;
			...
			theWindow = NewWindow (*gPaletteWindowHandle, &gTempRect, "\p", 1, 
														noGrowDocProc, front, FALSE, (SInt32)NULL);
	...
	if (windowInfoHandle != NULL)
		...
		InitializeWindowInfoStructure (windowInfoHandle, 
				...
				// Initialize items specific for palette window information			
				// structure.																		
...
//		if (controlHandle != NULL)
//			SetBevelButtonContentInfo (controlHandle,
//                                 	&controlButtonInfo);
...

{
	...
	if (windowInfoPtr != NULL)
		...
		imageSecondOrderStatsPtr->sumSquaresHandle =
				...
				// Dispose of or initialize information in the display specs structure that 
				// relates to the image being disposed of in memory and prepare the structure
				// for a new image.
				...
				HIViewRef*								textViewPtr)
{
	...
	if	(window != NULL)
		{
		status = HIViewFindByID (HIViewGetRoot(window), kTextViewControlID, textViewPtr);
