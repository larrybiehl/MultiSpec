		...
		if (gOSXCoreGraphicsFlag)
			{
				...
				lDestinationRect.right = 
							(SInt32)(legendWidth + displaySpecsPtr->imageDimensions[kHorizontal]);
...
//				GetWindowClipRectangle (windowPtr, kImageArea, &imageRect);
				...
				rectangle.size.width = destinationRectPtr->right - destinationRectPtr->left;
				rectangle.size.height = destinationRectPtr->bottom - destinationRectPtr->top;
...
//				if (vMagnificationSize + titleHeight >= printPortRect.bottom)
				...
				hOffset = (float)(destinationRectPtr->left - sourceRectPtr->left * magnification);
	...
	if ( (imageWindowInfoPtr->numberOverlays || 
							imageWindowInfoPtr->projectWindowFlag) && 
									sourceImageWidth > 0 && 
											!AbortPrint () )
		{
		SetOrigin (-destinationRectPtr->left+legendWidth,
...

{
	...
	MoveTo (pageRight - (StringWidth ( (ConstStr255Param)pageStr ) + 
					StringWidth ( pageNumStr ) + 5), pageTopLeft.v + ascent);
...

{
	...
	GetPort (&savedPort);
			...
			// If this is the output text window, then save the current clip		
			// region so that the clip region can be set to the entire port		
			// rectangle so that the controls can be adjusted when printing.			
			...
			
{
	...
	if (status == noErr)
		status = PMSetOrientation (pageFormat, GetPrintPageOrientation(windowInfoHandle), kPMUnlocked);
	...
	if ( (status = PMSessionPageSetupDialog (printSession, pageFormat, &accepted) ) != noErr)
		goto cleanup;
...

{
	...
	if (status == noErr)
		status = PMSetOrientation (pageFormat, windowInfoPtr->printPageOrientation, kPMUnlocked);
	...
	if (status == noErr)
		status = PMSessionValidatePageFormat (printSession, pageFormat, &pageValidationResultFlag);
	...
	if (status == noErr && 
				(DoJobDlog || windowType == kImageWindowType || windowType == kThematicWindowType))
		{
...
//			PMRectToRect (&pmRect, &printableArea);
	...
	if (status == noErr && !aborted)
		...
		if (windowType == kImageWindowType || windowType == kThematicWindowType)
			aborted = SetUpPrintedImageInfo (windowInfoPtr, &sMagnification, &sPrintableArea);
...

{
	...
	LoadDItemValue (dialogPtr, 4, (SInt32)totalNumberPages);
			...
			// Get the height and width of the area to draw the scaled pages		
			// into.																					
	...
	scaledImageHeight = 
					((SInt32)destinationHeight * scaledPageHeight)/pageHeight;
	scaledImageWidth = 
					((SInt32)destinationWidth * scaledPageWidth)/pageWidth;
	...
	scaledMagnification = (sMagnification * scaledPageWidth)/pageWidth;
			...
			// Get the left and top position to start drawing the scaled image	
			// outline.																				
			
	left = (scaledWidth - scaledImageWidth)/2 + theRect.left;
	top = (scaledHeight - scaledImageHeight)/2 + theRect.top;
	
			// Set the scaled rectangle that will be used to represent the			
			// printed image.																		
...

{
	...
	*noPageHeightPtr = (*destinationHeightPtr-1)/(*pageHeightPtr) + 1;
	...
	ratio = ldiv (	(SInt32)(sourceRectPtr->right - sourceRectPtr->left), *noPageWidthPtr);
	*middlePageWidthPtr = ratio.quot * magnification + .5;
	...
	ratio = ldiv (	(SInt32)(sourceRectPtr->bottom - sourceRectPtr->top), *noPageHeightPtr);
...

{
	...
	bottomPageHeight = sourceHeight - topPageHeight - noMiddlePages*middlePageHeight;
			...
			// If this is a Thematic type of image window and a legend is to		
			// be shown, create the list for the legend for this print 				
			// window.																				
...

{
	...
	*magnificationPtr = userDefinedMagnification;
	...
	dialogPtr = LoadRequestedDialog (kPageLayoutSpecificationID, kDoNotCopyScrap, 1, 2);
	if (dialogPtr == NULL)													
																					return(TRUE);	
	...
	ShowDialogWindow (
				dialogPtr, kPageLayoutSpecificationID, kDoNotSetUpDFilterTable);
