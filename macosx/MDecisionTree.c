				...
				
{
 	switch (alg) 
		...
		case 2:
	   	return TransformedDivergence (st1->MeanVec,
													st2->MeanVec,
													st1->CovMat,
													st2->CovMat,
													st1->CovInv,
		...
		case 4:
	   	return NonCovarianceBhattacharyya (st1->MeanVec,
															st2->MeanVec,
															st1->CovMat,
															st2->CovMat,
															st1->CovDet,
							...
							
{
   ...
   fs->classesPtr = (SInt16*)malloc (
						nnodes*(MAXCLASS>>BITSHIFT)*sizeof (ULONG) + ngrp*sizeof (UBYTE));
   if (!fs->classesPtr)
																								return (0);
   ...
   fs->maxsep = maxsep;
    		...
    		// hmmm... for accuracy, just use min. of inter-class separabilities*ngrp 																
...
//
//	Software purpose:	The purpose of this routine is to the find the distances between 
//							the class pairs for the given channels.
//
//	Parameters in:		None
//
//	Parameters out:	None
...

{
   ...
   EmptyNode (curnode);
   curnode->nchild = n;
   curnode->child = (struct node*)calloc (
										1, n*sizeof (struct node)+sizeof (struct nodeinfo));
				...
				
{
	...
	for (r=1,pri=input,pro=output; r<n; r++,pri+=n+1,pro+=n) 
		...
		for (i=r,ri=pri+n,ro=pro+n; i<n; i++) 
			...
			}	// end "for (i=r,ri=pri+n,ro=pro+n;	i<n; i++)"
			
		}	// end "for (r=1,pri=input,pro=output; r<n; r++,pri+=n+1,pro+=n)"
	...
	for (r=n-1,pro+=n; r; r--,pri-=n+1,pro-=n)
		for (i=r,ri=pri-n,ro=pro-n; i>=0; i--,ri-=n) 
			...
			}	// end "for (i=r,ri=pri-n,ro=pro-n; i>=0; i--,ri-=n)"
				...
				
{
	...
	if (n->nchild) 
		...
		for (cl=0,st1=imagestats,m=1,i=0; cl<MAXCLASS; cl++,st1++,m<<=1) 
			...
			}	// end "for (cl=0,st1=imagestats,m=1,i=0; cl<MAXCLASS; cl++,st1++,m<<=1)"
	...
	for (r=0,l=image; r<imf->numberLines; r++,l++)
		...
		for (c=0; c<imf->numberColumns; c++) 
			...
			for (i=j=0; j<MAXCLASS>>BITSHIFT; j++) 
				...
				}	// end "for (i=j=0;j<MAXCLASS>>BITSHIFT;j++)"
				
			*rp++ = i==MAXCLASS?0:i;
			
			}	// end "for (c=0; c<imf->numberColumns; c++)"
			
		}	// end "for (r=0,l=image; r<imf->numberLines; r++,l++)"
				...
				
{
   ...
   if (!nn)
		if (fwrite (n->channelsPtr,
						(MAXCHANNELS>>BITSHIFT)*sizeof (*n->channelsPtr),
						1,
						f) != 1)
																									return 0;
