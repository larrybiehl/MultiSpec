...

{
	...
	if (gAppleEventsFlag)
		...
		returnError = SendAEOpenOrPrintDocument (NULL, kAEOpenDocuments);
	 	...
	 	{    	
		projectVersionCode = OpenImageFile (NULL);
...

{
	...
	if (continueFlag)
		...
		CopyPToP (aliasedFileStreamPtr->fileName, 
												localAppFilePtr->fNamePtr);
	...
	fileStreamPtr = (CMFileStream*)CheckAndDisposePtr ( 
													(Ptr)fileStreamPtr );
...
//	Revised By:			Larry L. Biehl			Date: 03/17/1995	
...

{
	...
	if (localAppFilePtr->type != 'ISTA' && 
		...
		returnCode = OpenImageFile (localAppFilePtr);
		gProcessorCode = 0;
		if (returnCode == -1)
			returnCode = 0;
