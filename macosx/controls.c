...

{
	...
	displaySpecsPtr->origin[kVertical] = (SInt32)(MAX(0, 
						imageLineCenter - windowLineCenter/magnification + .5));
	
	displaySpecsPtr->origin[kHorizontal] = (SInt32)(MAX(0, 
...
//
//	Coded By:			Larry L. Biehl			Date:12/11/1996
//	Revised By:			Larry L. Biehl			Date:12/13/1996			
...

{
	...
	if (gAppearanceManagerFlag)
		visibleFlag = IsControlVisible (theControl);
			...
			visibleFlag = (*theControl)->contrlVis;
	...
	*verticalMaximumPtr = 0;
	...
	if (gOSXFlag)
	   ...
	   if (vControlHandle !=NULL)
		...
		{
	   if (hControlHandle != NULL) 			
	   	*horizontalMaximumPtr = GetControlMaximum (hControlHandle);
	   	
	   if (vControlHandle !=NULL)
	  		*verticalMaximumPtr = GetControlMaximum (vControlHandle);	
...

{
    ...
    if (controlHHandl != NULL && controlVHandl != NULL)
		...
		if (gOSXFlag)
			...
			{
    		controlVValue = GetControlMaximum (controlVHandl);
    		controlHValue = GetControlMaximum (controlHHandl);
    	...
    	if (controlVValue != 0 | controlHValue != 0)
			...
			vWindowDimensions = 
						(SInt32)( (destRect.bottom-destRect.top)/magnification );
			hWindowDimensions = 
						(SInt32)( (destRect.right-destRect.left)/magnification );
			...
			do
				...
				if (scrollHStep != 0 || scrollVStep != 0)
					...
					if (gOSXFlag)
						...
						{
						SetControlValue (controlHHandl, (SInt16)hOrigin);
						SetControlValue (controlVHandl, (SInt16)vOrigin);
		...
		switch (refCon)
			...
			case kHorizontalControl:		// Horizontal scroll bar 
				...
				if (partCode == kControlIndicatorPart)
					...
					#else	// TARGET_CPU_68K
								// We get an error on 68K machines when returning from the
								// ImageWThumbScrollAction routine. Will not allow this option
								// for now.
//						if (gEventRecord.modifiers & optionKey)
						if (FALSE)
					#endif	// !TARGET_CPU_68K, else...
						{
						...
						if (gOSXFlag)
							...
							SetControlValue (gCurrentControlHandle, 
															(SInt16)displaySpecsPtr->origin[refCon]);
						...
						if (partCode != 0)
							...
							if (gOSXFlag)
								...
								origin = GetControlValue (theControl);
							...
							origin = (SInt16)(oldOrigin - scrollPixels/displaySpecsPtr->magnification);	
							displaySpecsPtr->origin[refCon] = origin;
					
							if (gOSXFlag)
								SetControl32BitValue (theControl, origin);
							else		// !gOSXFlag
								SetControlValue (theControl, origin);
				...
				else // partCode== kControlUpButtonPart, kControlDownButtonPart, kControlPageUpPart, kControlPageDownPart 
			...
			case kZoomOutControl:
				...
				UpdateImageZoomControls (
								theWindow, displaySpecsPtr->magnification, true);
				break;
			...
			case kLegendPaletteControl:
            {
					...
					SInt16 ancillaryInfoFormat = ((FileInfoPtr)*gActiveImageFileInfoH)->ancillaryInfoformat;
					UInt32 numberClasses = ((FileInfoPtr)*gActiveImageFileInfoH)->numberClasses;
					Handle classDescriptionH = ((FileInfoPtr)*gActiveImageFileInfoH)->classDescriptionH;
					UInt32 colorTableOffset = ((FileInfoPtr)*gActiveImageFileInfoH)->colorTableOffset;
					...
					if (gAppearanceManagerFlag)
						...
						if (popUpMenuHandle != NULL)
							...
							if (selection > 0)
								errCode = GetBevelButtonMenuValue (theControl, (MenuItemIndex*)&paletteSelection);
						...
						DrawPopupControl (theControl, 10000);
						...
						paletteSelection = StandardControlPopUpMenu (theControl, 
																					gPopUpPaletteMenu, 
																					paletteSelection, 
																					kPopUpPaletteMenuID,
																					kCheckItem);
				...
				
{
	...
	if (gOSXFlag)			
		...
		maxControlValue = GetControlMaximum (gCurrentControlHandle);
	...
	while ( StillDown() ) 
		...
		if (refCon == kVertical)
			{
			scrollStep = (SInt32)(linesPerControlStep * (mousePoint.v - gOldThumbScrollPoint.v));
			...
			{  
			scrollStep = (SInt32)(linesPerControlStep * (mousePoint.h - gOldThumbScrollPoint.h));
		...
		if (gOSXFlag)
			...
			SetControlValue (gCurrentControlHandle, (SInt16)origin);
				...
				
{
	...
	if (gOSXFlag)
		...
		lastOrigin = GetControlValue (gCurrentControlHandle);
...

{
	...
	if (h != 0 || v != 0)
		...
		if (gOSXFlag)			
			...
			SetControlValue (theControl, (SInt16)origin);
...
//	Rect* 								rect = &(*textH)->tRect;
	...
	if (gPresentCursor == kIBeam)
		...
		if (mousePoint.v < rect.top)
			OutputWScroll (
					((WindowInfoPtr)*gActiveWindowInfoH)->verticalControl, kControlUpButtonPart);
			
		else if (mousePoint.v > rect.bottom)
			OutputWScroll (
				((WindowInfoPtr)*gActiveWindowInfoH)->verticalControl, kControlDownButtonPart);
	
		if (mousePoint.h < rect.left) 
			OutputWScroll (
				((WindowInfoPtr)*gActiveWindowInfoH)->horizontalControl, kControlUpButtonPart);
			
		else if (mousePoint.h > rect.right) 
			OutputWScroll (
				((WindowInfoPtr)*gActiveWindowInfoH)->horizontalControl, kControlDownButtonPart);
...
*/
	...
	SetControlProperty (theControl, (UInt32)gCreator, kScrollStepTag, sizeof(step), &step);
...

{
	...
	if (controlVHandl != NULL && controlHHandl != NULL)
		...
		imageLines = (SInt32)((DisplaySpecsPtr)*displaySpecsH)->imageDimensions[kVertical];
		...
		if (gOSXFlag)
			...
			SetControlMaximum (controlVHandl, (SInt16)nVOrigin);
		...
		if (nVOrigin <= vOrigin)
			...
			if (nVOrigin == 0)
				...
				if (gOSXFlag)
					...
					SetControlMaximum (controlVHandl, (SInt16)0);
			...
			if (gOSXFlag)
				...
				SetControlValue (controlVHandl, (SInt16)nVOrigin);
		...
		imageColumns = (SInt32)((DisplaySpecsPtr)*displaySpecsH)->imageDimensions[kHorizontal];
		...
		if (gOSXFlag)
			...
			SetControlMaximum (controlHHandl, (SInt16)nHOrigin);
		...
		if (nHOrigin <= hOrigin)
			...
			if (nHOrigin == 0)
				...
				if (gOSXFlag)
					...
					SetControlMaximum (controlHHandl, 0);
			...
			if (gOSXFlag)
				...
				SetControlValue (controlHHandl, (SInt16)nHOrigin);
...
//		std::sprintf( (char*)&gTextString, "X%4.*f", 
...
//		SetSmartScrollInfo (bar, visible, total);
		...
		if (scrollToSelStart)
			...
			SInt32 horizontalScrollSetting = 
				weViewRect.left - weDestRect.left - LCGetValue (windowInfoPtr->horizontalControl);
					
			SInt32 verticalScrollSetting = 
				weViewRect.top - weDestRect.top - LCGetValue (windowInfoPtr->verticalControl);
	...
	if (controlHandle != NULL)
		...
		if (gAppearanceManagerFlag)
			visibleFlag = IsControlVisible (controlHandle);
				...
				visibleFlag = (*controlHandle)->contrlVis;
		#endif	// !TARGET_API_MAC_CARBON
