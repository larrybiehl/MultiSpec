...
//											handle;
	...
	#if !TARGET_API_MAC_CARBON	
		JmpHandle							legendListDefinitionHandle;
		JmpControlHandle					controlDefinitionHandle;
	#endif	// !TARGET_API_MAC_CARBON
	...
	Boolean								powerPCExpectedFlag,
											processor68881ExpectedFlag,
											hasFPUFlag,
											longRowBytesFlag,
											returnFlag;
...
//											markCharOpenImage = 0x8D;
		...
		SInt32								minimumHeapSize = 350000,
												minimumSpaceSize = 100000;
												
		SysEnvRec							sysEnvRec;
		
		SInt16								QD32Trap = 0xab03,
												WNETrapNum = 0xa860;
	#endif	// !TARGET_API_MAC_CARBON
	
	
	#if !TARGET_API_MAC_CARBON	
				// Get memory that will be used later for the 'About dialog box.		
				
		dialogHandle = MNewHandle ( sizeof(DialogRecord) );
	...
	gSpareCodeMemoryH = UnlockAndDispose (gSpareCodeMemoryH);
			...
			// Set the processor68881ExpectedFlag.												

	processor68881ExpectedFlag = TRUE;
	powerPCExpectedFlag = FALSE;

	#ifdef _MC68881_
		processor68881ExpectedFlag = TRUE;
	#else		// not _MC68881_ 
		processor68881ExpectedFlag = FALSE;
	#endif

	#ifdef powerc
		processor68881ExpectedFlag = FALSE;
		powerPCExpectedFlag = TRUE; 
	#endif
...
//			gNavServicesExistsFlag = FALSE;
	...
	#else		
		gHasWaitNextEvent = TrapAvailable (WNETrapNum);
	#endif	// TARGET_API_MAC_CARBON else...
	...
	if (gGestaltAvailableFlag)
		...
		if (gGestaltAvailableFlag)
			{
			errCode = Gestalt (gestaltFPUType, &gestaltResponse);
			hasFPUFlag = (gestaltResponse != 0);
						
			}		// end "if (gGestaltAvailableFlag)" 
		
		else		// !gGestaltAvailableFlag 
			{
			errCode = SysEnvirons (1, &sysEnvRec);
			hasFPUFlag = sysEnvRec.hasFPU;
	...
	if (errCode != noErr)
			...
			// Make certain that a math coprocessor is present if the 				
			// compiled software uses a coprocessor. 										
			
	if (processor68881ExpectedFlag && !hasFPUFlag)
		{
		if (gSystemArchitectureCode == kPPC || gSystemArchitectureCode == kIntel)
			returnCode = StopAlert (1167, NIL);
		
		else		// gSystemArchitectureCode == k68K
			returnCode = StopAlert (1150, NIL);
			
																				ExitToShell ();
		
		}		// end "if (processor68881ExpectedFlag && !hasFPUFlag)" 
...
//			if (Gestalt (gestaltFontMgrAttr, &gestaltResponse) == noErr)
		...
		if (errCode == noErr && 
								(gestaltResponse & (1 << gestaltDragMgrPresent)) == 0)
...
//			gSupportsHFSPlusVols = TRUE; 
			gHasHFSPlusAPIs = TRUE; 
				...
				// Could not get this to work under OS9 even if gSupportsHFSPlusVols
				// was true.
...
//	InsertMenu (gMultiSpecMenus[kPaletteM], -1);		// insert submenu in options menu 
	...
	gCursorPtr[kSpin] = (CursPtr)GetHandlePointer (
													(Handle)cursorHndl, kLock, kMoveHi);
	...
	GetIndPattern (&gOutlinePenPattern, kLinePatternID, 1);
			...
			// Get the address mask to use.													
			
	gAddressMask = (UInt32)StripAddress ( (Ptr)-1L );
...
//	gUpdatePaletteMenuItemsFlag = TRUE;
	...
	gTextCR[2]  = 0;
		...
		(**legendListDefinitionHandle).jmpAddr = 
													NewListDefProc (LegendListDefinition);
		...
		controlDefinitionHandle = 
										(JmpControlHandle)Get1Resource ('CDEF', 131);
		...
		(**controlDefinitionHandle).jmpAddr = 
											NewControlDefProc (ControlDefinition);
	...
	gInverseMatrixMemory.ipvotPtr = NULL;
			...
			// Initialize the file types that MultiSpec is allowed to 				
			// recognize.																			
...
//	gDrawBitsPerDataValuePopUpPtr = NewUserItemUPP(DrawBitsPerDataValuePopUp);
//	if (gDrawBitsPerDataValuePopUpPtr == NULL)
//																				ExitToShell ();
																				
//	gDrawBytesPerDataValuePopUpPtr = 
//											NewUserItemUPP(DrawBytesPerDataValuePopUp);
//	if (gDrawBytesPerDataValuePopUpPtr == NULL)
//																				ExitToShell ();
	...
	InitializeDoubleVariables ();
		...
		ptr = (Ptr)GetHandlePointer (itlHandle, kNoLock, kNoMoveHi);
	...
	#ifndef __XCODE__ 
		#ifndef powerc
					// Display a note if the compiled code does not take advantage 	
					// of a math coprocessor and one is available to use. 				
				
			if ( (!processor68881ExpectedFlag && hasFPUFlag) ||
					(!powerPCExpectedFlag && gSystemArchitectureCode == kPPC) )
				returnCode = StopAlert (1151, NIL);
		#endif
...
//			}
	...
	if (userInterfaceValue != NULL)
      {
		returnFlag = CFNumberGetValue (userInterfaceValue, kCFNumberSInt16Type, &interfaceValue);
...

{
	...
	if ( !abortedFlag )
		...
		InitializeWindowInfoStructure (
				(Handle)windowInfoPtr, kPointer, fileInfoHandle, kNoImageType, kOutputWindowType);
			
			// Initialize items specific for text window information					
			// structure.																			
			...
			if (gOSXFlag)
				...
				if (status == noErr)
					status = InstallEventHandler (GetWindowEventTarget (theWindow), 
						...
						// Install a handler to update the UI when this window has a mouse click in
						// the content portion of the window.
...

{
	...
	if (theWindow != NULL)
		...
		if (windowInfoHandle != NULL)
			...
			windowInfoPtr->verticalControl = NewControl (
															theWindow, 
															&gTempRect, 
															"\p", 
															visible, 
			...
			windowInfoPtr->horizontalControl =
...
//		kTXNReadOnlyMask+kTXNMonostyledTextMask+kTXNWantHScrollBarMask+
	...
	if (status == noErr)
		...
		status = TXNSetTXNObjectControls (gTextObject, FALSE, 1, iControlTags, iControlData);
...
//			Tabs caused problems in the output text window. After there are so many, the text
//			in the window. It is truncated when displaying although the data are still there
//			for copying. They will reappear if one scrolls with that text hidden. The text
//			will go away again as soon as one changes the size of the text window.
//			Found note about this problem on the web. It is a problem in mlte. It has a problem
//			after 20 or number of tabs. Have not found a solution other than converting to cocoa.
//			2/25/2012
	if (status == noErr)
		{		
...
//		gOutputAutoScrollActionPtr = (WEScrollUPP)NewRoutineDescriptor(
//			(ProcPtr)TextScrolled, 
//			kPascalStackBased
//					| STACK_ROUTINE_PARAMETER(1,SIZE_CODE(sizeof(WEReference))), 
//					kPowerPCISA);
