...
//								after LARSYS that ran on an IBM VM system.
//
					...
					for (index=numberControls; index>=1; index--)
						...
						/*
							...
							if (IsControlVisible (controlHandle))
								ValidWindowRect (
													window, 
													GetControlBounds (controlHandle, &gTempRect));
...

{
	...
	Handle								imageBaseAddressH;
			...
			// Note that windowInfoHandle has already been locked by calling routine.																				
	...
	UpdateImageZoomControls (window, magnification, TRUE);
 	...
 	if (windowInfoPtr->coordinateHeight > 0)
		...
		gCoordinateColumnValue = 0;
			...
			// If graph selection window is open, then indicate that the parameters 
			// for the window need to be updated.							
	...
	*/
	...
	if (callTitleUpdateFlag)
		...
		//	{
			...
			DrawSideBySideTitles (
									windowInfoHandle, window, &nonImageRect, kToImageWindow);
					...
					if (gEventRecord.modifiers&cmdKey)
						...
						if (gTheActiveWindow)
							...
							if (gTheActiveWindow == gOutputWindow)
								...
								else if (theChar == kEnterCharCode || 
																			theChar == kReturnCharCode)
									DoStatisticsAddFieldEvent ();
									
								else if (theChar == kLeftArrowCharCode || 
		  		...
		  		case autoKey:
					...
					if (gTheActiveWindow != NULL)
						...
						if (gTheActiveWindow == gActiveImageWindow)
							...
							if (CGEventSourceKeyState (eventSource, 0x7B) || 
														CGEventSourceKeyState (eventSource, 0x7C))
								{
...

{
	...
	if (TrackBox (window, gEventRecord.where, partCode))
		...
		GetMaximumSizeForDevicePointIsIn (&gEventRecord.where, &gViewRect);										
      		...
      		// Update the standard state to be the size of the window that allows 
				// the entire image to be viewed or if that is larger than the screen 
				// then the entire screen for the device that the cursor is in. 
				
		GetWindowStandardState (window, &idealStandardState);
		gTempRect = idealStandardState;
		GetWindowUserState (window, &userStateRect);
				
				// Determine whether the top left corner of the user rectangle is within 
				// the current device.  If so use this as the top left for the standard 
				// state.  If not	use the upper left of the 'new' current device.					
		...
		if (displaySpecsPtr != NULL)
			...
			idealStandardState.right = MIN (
										gViewRect.right,
										idealStandardState.left + (SInt16)gTempLongPoint.h);
											
			idealStandardState.bottom = MIN (
										gViewRect.bottom,
										idealStandardState.top + (SInt16)gTempLongPoint.v);
											
					// Now force the upper left if possible to enclose the image if needed.														
				
			idealStandardState.left = MAX (
										gViewRect.left,
										idealStandardState.right - (SInt16)gTempLongPoint.h);
											
			idealStandardState.top = MAX (
										gViewRect.top,
...
//							a given size and projection, displayed as channels 4, 4, 2 and 
//							saved to a geotiff file.
//
//	Parameters in:				
//
//	Parameters out:				
...
{	
	...
	*/	
	...
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p25_1latest_2016_mint_20170423");
	...
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p26_1latest_2016_mint_20170423");
	gConfirmReplacementAlertFlag = FALSE;
	menuData = (kReformatMenuID<<16) + kReformatChangeImageRequest;
	Menus (menuData);
	gConfirmReplacementAlertFlag = TRUE;
	...
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p27_1latest_2016_mint_20170423");
	gConfirmReplacementAlertFlag = FALSE;
	menuData = (kReformatMenuID<<16) + kReformatChangeImageRequest;
	Menus (menuData);
	gConfirmReplacementAlertFlag = TRUE;
	...
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p28_1latest_2016_mint_20170423");
	gConfirmReplacementAlertFlag = FALSE;
	menuData = (kReformatMenuID<<16) + kReformatChangeImageRequest;
	Menus (menuData);
	gConfirmReplacementAlertFlag = TRUE;
	...
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p29_1latest_2016_mint_20170423");
	gConfirmReplacementAlertFlag = FALSE;
	menuData = (kReformatMenuID<<16) + kReformatChangeImageRequest;
	Menus (menuData);
	gConfirmReplacementAlertFlag = TRUE;
	...
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p30_1latest_2016_mint_20170423");
	gConfirmReplacementAlertFlag = FALSE;
	menuData = (kReformatMenuID<<16) + kReformatChangeImageRequest;
	Menus (menuData);
	gConfirmReplacementAlertFlag = TRUE;
	...
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p31_1latest_2016_mint_20170423");
	gConfirmReplacementAlertFlag = FALSE;
	menuData = (kReformatMenuID<<16) + kReformatChangeImageRequest;
	Menus (menuData);
	gConfirmReplacementAlertFlag = TRUE;
	...
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p32_1latest_2016_mint_20170423");
	gConfirmReplacementAlertFlag = FALSE;
	menuData = (kReformatMenuID<<16) + kReformatChangeImageRequest;
	Menus (menuData);
	gConfirmReplacementAlertFlag = TRUE;
	...
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p33_1latest_2016_mint_20170423");
	gConfirmReplacementAlertFlag = FALSE;
	menuData = (kReformatMenuID<<16) + kReformatChangeImageRequest;
	Menus (menuData);
	gConfirmReplacementAlertFlag = TRUE;
	...
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p34_1latest_2016_mint_20170423");
	gConfirmReplacementAlertFlag = FALSE;
	menuData = (kReformatMenuID<<16) + kReformatChangeImageRequest;
	Menus (menuData);
	gConfirmReplacementAlertFlag = TRUE;
	...
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p35_1latest_2016_mint_20170423");
	gConfirmReplacementAlertFlag = FALSE;
	menuData = (kReformatMenuID<<16) + kReformatChangeImageRequest;
	Menus (menuData);
	gConfirmReplacementAlertFlag = TRUE;
	...
	gMultiSpecWorkflowInfo.thresholdValue = 25;
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p25_3earliest_2016_mint_20170423");
	gConfirmReplacementAlertFlag = FALSE;
	menuData = (kReformatMenuID<<16) + kReformatChangeImageRequest;
	Menus (menuData);
	gConfirmReplacementAlertFlag = TRUE;
	...
	gMultiSpecWorkflowInfo.thresholdValue = 26;
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p26_3earliest_2016_mint_20170423");
	gConfirmReplacementAlertFlag = FALSE;
	menuData = (kReformatMenuID<<16) + kReformatChangeImageRequest;
	Menus (menuData);
	gConfirmReplacementAlertFlag = TRUE;
	...
	gMultiSpecWorkflowInfo.thresholdValue = 27;
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p27_3earliest_2016_mint_20170423");
	gConfirmReplacementAlertFlag = FALSE;
	menuData = (kReformatMenuID<<16) + kReformatChangeImageRequest;
	Menus (menuData);
	gConfirmReplacementAlertFlag = TRUE;
	...
	gMultiSpecWorkflowInfo.thresholdValue = 28;
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p28_3earliest_2016_mint_20170423");
	gConfirmReplacementAlertFlag = FALSE;
	menuData = (kReformatMenuID<<16) + kReformatChangeImageRequest;
	Menus (menuData);
	gConfirmReplacementAlertFlag = TRUE;	
	...
	gMultiSpecWorkflowInfo.thresholdValue = 29;
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p29_3earliest_2016_mint_20170423");
	gConfirmReplacementAlertFlag = FALSE;
	menuData = (kReformatMenuID<<16) + kReformatChangeImageRequest;
	Menus (menuData);
	gConfirmReplacementAlertFlag = TRUE;
	...
	gMultiSpecWorkflowInfo.thresholdValue = 30;
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p30_3earliest_2016_mint_20170423");
	gConfirmReplacementAlertFlag = FALSE;
	menuData = (kReformatMenuID<<16) + kReformatChangeImageRequest;
	Menus (menuData);
	gConfirmReplacementAlertFlag = TRUE;
	...
	gMultiSpecWorkflowInfo.thresholdValue = 31;
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p31_3earliest_2016_mint_20170423");
	gConfirmReplacementAlertFlag = FALSE;
	menuData = (kReformatMenuID<<16) + kReformatChangeImageRequest;
	Menus (menuData);
	gConfirmReplacementAlertFlag = TRUE;
	...
	gMultiSpecWorkflowInfo.thresholdValue = 32;
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p32_3earliest_2016_mint_20170423");
	gConfirmReplacementAlertFlag = FALSE;
	menuData = (kReformatMenuID<<16) + kReformatChangeImageRequest;
	Menus (menuData);
	gConfirmReplacementAlertFlag = TRUE;
	...
	gMultiSpecWorkflowInfo.thresholdValue = 33;
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p33_3earliest_2016_mint_20170423");
	gConfirmReplacementAlertFlag = FALSE;
	menuData = (kReformatMenuID<<16) + kReformatChangeImageRequest;
	Menus (menuData);
	gConfirmReplacementAlertFlag = TRUE;
	...
	gMultiSpecWorkflowInfo.thresholdValue = 34;
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p34_3earliest_2016_mint_20170423");
	gConfirmReplacementAlertFlag = FALSE;
	menuData = (kReformatMenuID<<16) + kReformatChangeImageRequest;
	Menus (menuData);
	gConfirmReplacementAlertFlag = TRUE;
	...
	gMultiSpecWorkflowInfo.thresholdValue = 35;
	CopyPToP (gMultiSpecWorkflowInfo.defaultName, 
					(UCharPtr)"\p35_3earliest_2016_mint_20170423");
	gConfirmReplacementAlertFlag = FALSE;
	menuData = (kReformatMenuID<<16) + kReformatChangeImageRequest;
	Menus (menuData);
	gConfirmReplacementAlertFlag = TRUE;
...
//
//	Software purpose:	This routine handles dragging of a window. The toolbox call for 
//							dragging will only be used if sufficient memory is available.
//
//	Parameters in:				
//
//	Parameters out:				
...

{
	...
	Boolean								cancelledFlag = FALSE;
			...
			// Release the spare memory in case it may be needed for saving files.																				
	...
	if (gTheActiveWindow == gActiveImageWindow)
		...
		if (windowType == kThematicWindowType)
			...
			if (gActiveOffscreenMapExists && 
					(gEventRecord.modifiers & shiftKey) &&
...

{
	...
	/* Check this (Larry Biehl 7/23/2018)
	#if TARGET_API_MAC_CARBON
		Rect									availableRect;
	...
	if (currentDevice)
		...
		*maximumRectPtr = (*currentDevice)->gdRect;
		...
		//#if TARGET_API_MAC_CARBON
			...
			status = 
					GetAvailableWindowPositioningBounds (currentDevice, maximumRectPtr);
...
{	
	...
	if (displaySpecsPtr != NULL)
		...
		displaySpecsPtr->origin[kHorizontal] = savedOrigin[kHorizontal];
				...
				// Adjust the size needed to allow for scroll bars and one pixel extra.													
...

{
   ...
   EraseRect (&scrollBar);
  	InvalWindowRect (theWindow, &scrollBar);
			...
			// coordinate view above the scroll bar if new window is wider than old one																
	...
	if (width > rect.right)
   	...
   	EraseRect (&scrollBar);
		...
		scrollBar.bottom = 
  		...
  		if (((WindowInfoPtr)*windowInfoHandle)->titleHeight > 0)
			...
			scrollBar.left = 0;
			scrollBar.bottom = 
							scrollBar.top + ((WindowInfoPtr)*windowInfoHandle)->titleHeight;
   		EraseRect (&scrollBar);
  			InvalWindowRect (theWindow, &scrollBar);
   	...
   	EraseRect (&scrollBar);
  		InvalWindowRect (theWindow, &scrollBar);
			...
			// Invalidate horizontal scroll area of old window if new window is taller 
			// than old one																
...

{
	...
	if (gMemoryTypeNeeded == 0)
		...
		if (gTextMemoryShortCode < 2 && !gMemoryAlertDisplayFlag)
			...
			numberChars = InsertCommasInNumberString ((char*)&tempString[1], 
																	numberChars, 
																	-1,
																	numberChars);
	...
	if (gMemoryTypeNeeded == 1 || gMemoryTypeNeeded == -1)
		...
		gMemoryTypeNeeded = -1;
				...
				// Warn the user that memory is low. Spare memory for the warning 
				// message has already been released.							
		...
		//#endif	// !TARGET_API_MAC_CARBON																						return (1);
			...
			// If this is memory needed to expand the spare handles, then present a 
			// caution alert warning the user that memory is low.		
...
//
//	Software purpose:	This routine sends a command to the Edit-Clear Selection 
//							Rectangle menu.
//
//	Parameters in:				
//
