...
//
//	Software purpose:			The purpose of this routine is to handle a mousedown in 
//									the channels pop up menu location.
//
//	Parameters in:					
//
//	Parameters out:				
...
//
//	Software purpose:	The purpose of this routine is to draw the channel prompt and 
//							channel set selection for channel pop up menu.  This routine 
...

{
	...
	if (drawStringFlag)
		...
		if (popUpMenuHandle != NULL)
			...
			osStatus = CopyMenuItemTextAsCFString (popUpMenuHandle,
																  localMenuItem,
																  &cfStringRef);
		...
		else	// popUpMenuHandle == NULL
			{
					...
					// If menuItem is 1, then draw text is for the project changes popup 
					// menu in the statistics dialog window.
		...
		HIRect			theRect = {popUpBox.left+4, 
										 ...
										 popUpBox.right-popUpBox.left-25,		// Allow for 
																							// down arrow
		...
		if (osStatus == noErr)
			osStatus = CreateCGContextForPort (
										GetWindowPort (GetDialogWindow (dialogPtr)), &context);
...

{
	...
	EraseRect (&popUpBox);
	menuData = PopUpMenuSelect (popUpMenu, popUpLoc.v, popUpLoc.h, currentSelection);
