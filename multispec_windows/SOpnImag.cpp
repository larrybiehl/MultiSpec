	...
	if (returnCode == 2 && /* gMultipleImageFileCode == 3 && */ linkOffsetIndex > 0)
		...
		if (fileInfoPtr->numberHdfDataSets > gLastHDFDataSetSelection)
			...
			if (hdfDataSetsPtr[gLastHDFDataSetSelection].name[1] == '*')
				...
				windowFileInfoPtr = (FileInfoPtr) GetHandlePointer(
                        windowFileInfoHandle,
                        kNoLock,
                        kNoMoveHi);
	...
	for (line = 1; line<=lineEnd; line++)
		...
		if (fileInfoPtr->numberBytes == 1)
			...
			for (index = 0; index<fileInfoPtr->numberColumns; index++)
				...
				// Allow some other things to occur. Display message if it will take a long time.
		...
		if (numberOutputClasses > 0)
			...
			if (numberOutputClasses == oldNumberClasses)
				...
				if (fileInfoPtr->classDescriptionH != NULL && fileInfoPtr->descriptionsFlag)
					{
					oldClassNamePtr = (HUCharPtr) GetHandlePointer (fileInfoPtr->classDescriptionH, kNoLock, kNoMoveHi);
			...
			if (!classNamesCopiedFlag)
				...
				if (newDescriptionHandle != NULL)
					...
					classNamePtr = (HUCharPtr) GetHandlePointer(
                            newDescriptionHandle, kLock, kNoMoveHi);
					...
					if (fileInfoPtr->classDescriptionH != NULL && fileInfoPtr->descriptionsFlag)
						...
						for (index = 0; index < numberOutputClasses; index++)
							...
							oldIndex = 0;
							oldClassNamePtr = (HUCharPtr) GetHandlePointer(
                                    fileInfoPtr->classDescriptionH, kNoLock, kNoMoveHi);
				...
				UInt32								supportFileType)
{
	...
	imageFilePathPtr = (UInt8*)GetFilePathPPointer(imageFileStreamPtr);
	supportFilePathPtr = (UInt8*)GetFilePathPPointer(supportFileStreamPtr);
	imageFileNameCPtr = (UInt8*)GetFileNameCPointer(imageFileStreamPtr);
		...
		if (strPtr != NULL)
			...
			if (numberCharacters > 0)
				...
				stringPtr[numberCharacters + 1] = 0;
						...
						// Set numberCharacters so that the comma will be skipped at the end
						// if needed when set up the return address for the string buffer.
				...
				FileInfoPtr							fileInfoPtr)
{
	...
	if (fileInfoPtr->instrumentCode == 0)
		...
		fileNamePtr = (UCharPtr)GetFileNameCPointer (fileInfoPtr);
			...
			SInt16									formatOnlyCode)
{
	...
	if (fileType != 0) {
		...
		fileStreamPtr = GetFileStreamPointer(fileInfoPtr);
		imageFileNamePtr = (UInt8*)GetFilePathPPointer (fileStreamPtr);
    ...
    if (inputStringPtr != NULL) {
        ...
        if (mapProjectionInfoPtr != NULL) {
            ...
            mapProjectionInfoPtr->gridCoordinate.projectionCode =
                    GetProjectionNameInfo (&mapInfoStrings[0][1],
                    &mapProjectionInfoPtr->gridCoordinate.referenceSystemCode);
            ...
            if (mapProjectionInfoPtr->gridCoordinate.projectionCode == kTransverseMercatorCode) {
            ...
            if (returnCode == 1) {
                ...
                mapProjectionInfoPtr->planarCoordinate.xMapCoordinate11 += (1.5 - xReferencePixel) * xPixelSize;
                mapProjectionInfoPtr->planarCoordinate.yMapCoordinate11 -= (1.5 - yReferencePixel) * yPixelSize;
            ...
            if (string >= 8) {
                if (mapProjectionInfoPtr->gridCoordinate.referenceSystemCode >= kUTM_NAD27RSCode &&
                    ...
                    if (CompareStringsNoCase (&mapInfoStrings[8][1], (UCharPtr)"south", 5) == 0)
                    ...
                    if (CompareStringsNoCase (&mapInfoStrings[9][1], (UCharPtr)"WGS-84", 6) == 0)
                        mapProjectionInfoPtr->geodetic.datumCode = kWGS84Code;

                }// end "if (mapProjectionInfoPtr->gridCoordinate.referenceSystemCode == ..."

                else if (mapProjectionInfoPtr->gridCoordinate.projectionCode != kNotDefinedCode) {
                    if (CompareStringsNoCase (&mapInfoStrings[7][1], (UCharPtr)"WGS-84", 6) == 0)
                        mapProjectionInfoPtr->geodetic.datumCode = kWGS84Code;
                    ...
                    } // end "mapProjectionInfoPtr->gridCoordinate.projectionCode == kSinusoidalCode"

                } // end "else if (...->gridCoordinate.code != kNotDefinedCode)"

                if (xPixelSize != 0 || yPixelSize != 0) {
                    if (CompareStringsNoCase (&mapInfoStrings[8][1], (UCharPtr)"units=Degrees", 13) == 0)
                        mapProjectionInfoPtr->planarCoordinate.mapUnitsCode = kDecimalDegreesCode;

                    else if (CompareStringsNoCase (&mapInfoStrings[8][1], (UCharPtr)"units=Meters", 12) == 0)
                        mapProjectionInfoPtr->planarCoordinate.mapUnitsCode = kMetersCode;

                    else if (CompareStringsNoCase (&mapInfoStrings[8][1], (UCharPtr)"units=Feet", 10) == 0)
            ...
            if (mapProjectionInfoPtr->gridCoordinate.referenceSystemCode == kUserDefinedRSCode) {
                ...
                if (endMapInfoStringPtr != NULL) {
                    // The projection info line was found.  Now read the 10 strings that 
                    // should be in this line that are separated by commas.
                    ...
                    if (string >= 7 && mapProjectionInfoPtr->gridCoordinate.projectionCode ==
                    ...
                    else if (string >= 6 && mapProjectionInfoPtr->gridCoordinate.projectionCode ==
                            kSinusoidalCode) {
                        returnCode = sscanf((char*) &mapInfoStrings[1][1],
                                "%lf",
                                &radiusSpheroid);
            ...
            } // end "if (...->gridCoordinate.referenceSystemCode == kUserDefinedReferenceSystemCode)"
...
Boolean ReadERDASClassNames(
				...
				UCharPtr								classNamePtr)
 {
    ...
    if (errCode == noErr) {
        /*															
								 // Removed this code on 5/11/2004.  Decided that the wrong trailer files
								 // should now be gone or should be replaced.

								 // First check if a histogram exists in the .trl file.				
								 // Note.  Discovered on 4-3-91 that above 7*128 bytes should 		
								 // always be in the trailer file.  The presence of 'TRAILER' or	
								 // 'TRAIL74' just indicates whether the histogram is valid.			
								 // The following will try to determine whether these bytes were	
								 // left out based on the size of the file.								
			...
			filePosOffset = classNameStart;
			if (errCode == noErr)
					  {  
					  if ( strncmp ((char*)&gTextString, erdasTRLHeader, 5) != 0)
								 {
													  // Histogram part may not be present.  Do a quick check to 	
													  // see if it is.  Assume if there is not enough space in		
													  // the file for the histogram and the class names, then		
													  // start reading the class names at the location where the	
													  // histogram would normally be.										
	
								 errCode = GetSizeOfFile (supportFileStreamPtr, &fileSize);

								 if (fileSize < classNameStart + fileInfoPtr->numberClasses * 32)
											filePosOffset = histogramStart;
        ...
        SInt16 formatOnlyCode)
 {
    ...
    if (formatOnlyCode != kLoadHeader) {
        ...
        if (version != 0) {
            ...
            fileInfoPtr->numberChannels =
                    GetShortIntValue((char*) &headerRecordPtr[8]);
    ...
    if (version != 0) {
        ...
        horizontalPixelSize =
                ConvertATRealtoReal((char*) &headerRecordPtr[120]);

        verticalPixelSize =
                ConvertATRealtoReal((char*) &headerRecordPtr[124]);
        ...
        if (tempInt <= 20) {
            ...
            if (fileInfoPtr->mapProjectionHandle != NULL) {
                ...
                else if (mapProjectionInfoPtr->gridCoordinate.referenceSystemCode == kStatePlaneNAD27RSCode)
			...
			SInt16									formatOnlyCode)
{
   ...
   if (headerRecordPtr != NULL && fileInfoPtr != NULL) 
      ...
      GetCopyOfPFileName (fileStreamPtr, gTextString);
	...
	if (fileType != 0) 
      ...
      if (fileType == kLASType1) 
        ...
        if (errCode == noErr && fileType == kLASType2) {
            ...
            fileNamePtr = (FileStringPtr)GetFilePathPPointer(fileStreamddrPtr);
        ...
        if (errCode == noErr) {
            ...
            fileInfoPtr->numberLines =
                    GetLongIntValue((char*) &headerRecordPtr[79]);


            // Get the number of columns in the image.										

            fileInfoPtr->numberColumns =
                    GetLongIntValue((char*) &headerRecordPtr[83]);
            ...
            fileInfoPtr->startLine =
                    GetLongIntValue((char*) &headerRecordPtr[95]);

            // Get the start column number					

            fileInfoPtr->startColumn =
                    GetLongIntValue((char*) &headerRecordPtr[99]);
            ...
            if (errCode == noErr) {
                ...
                if (nextSet == 2) {
                    ...
                    if (strPtr != NULL) {
                        ...
                        strPtr += stringLength;
                        sscanCode =
                                sscanf(strPtr, "%ld\r", &fileInfoPtr->numberClasses);
                ...
                if (nextSet == 3) {
                    ...
                    if (strPtr != NULL) {
                        ...
                        if (strPtr2 != NULL) {
                            ...
                            if (sscanCode == 6) {
                                // Get the number lines and columns in the 		
                                // image.													
...
void ReadLARSYSChannelDescriptionsAndValues(
        FileInfoPtr fileInfoPtr)
 {
    ...
    if (errCode == noErr) {
        // Set position mark in file to the start of the channel				
        // descriptions.																	
        ...
        FileInfoPtr								fileInfoPtr)
{
    ...
    if (fileInfoPtr != NULL) 
		...
		if (errCode == noErr) 
			...
			gCharBufferPtr1 = (CharPtr)CheckHandleSize (
												  &gCharBufferHandle1,
												  &continueFlag,
												  &changedFlag,
												  count + 1);
				...
				UCharPtr								classNamePtr)
 {
    ...
    while (classIndex < fileInfoPtr->numberClasses && errCode == noErr) {
        ...
        if (errCode == noErr) {
            ...
            DisplayAlert(
                kErrorAlertID, kCautionAlert, kAlertStrID, IDS_Alert5, 0, NULL);
...
#	endif
			...
			if (CompareSuffixNoCase ((char*)"\0.img", 
				...
				if (CheckForPDS_LBL_File (fileInfoPtr, localHeaderRecord))
					...
					if (MGetString(gTextString2, kFileIOStrID, IDS_FileIO152))		// PDS_VERSION_ID
						{
						CopyPascalStringToC((ConstStr255Param) &gTextString2, (char*) &gTextString2);
						stringCompare = strncmp ((char*)&localHeaderRecord[0], (char*) &gTextString2, 14);		
		...
		if (fileType < 0)
			{
				...
				if (stringCompare == 0)
					{
							// This is a PDS type of file. Check if the version is PDS3. If so then
							// allow gdal to read the file if gdal library is available.
					...
					if (tReturnCode == 0)
						{
						stringCompare = CompareStringsNoCase (&gTextString2[1], (UCharPtr)"pds3", 4);
						...
						if (stringCompare == 0)
							...
							stringCompare = 1;
									...
									// If LBL file found to go with selected image file, then need to change
									// the file being used to the LBL file so that gdal will handle it correctly.
	...
	if (fileType != 0) 
		...
		fileInfoPtr->ancillaryInfoformat = kPDSType;
				...
				// seen PDS files that have seen have a 512 to 25K bytes header. I have not seen
				// the specs for the PDS format.						
        ...
        while (notFound && errCode == noErr) {
            ...
            if (errCode == noErr) {
                ...
                if (nextSet == 1) {
                    ...
                    recordBytes = GetFileHeaderValue(
                            kFileIOStrID,
...
SInt16 ReadSunScreenDumpHeader(
        ...
        SInt16 formatOnlyCode)
 {
    ...
    if (fileType != 0) {
        ...
        // Load the rest of the information for SUN screen dump 				
        // formatted files.																
...
SInt16 ReadTGAHeader(
        ...
        SInt16 formatOnlyCode)
 {
    ...
    if (headerRecordPtr != NULL && fileInfoPtr != NULL) {
        ...
        // Image Type field should be 2 or 3 for MultiSpec to
        // recognize.

        if (headerRecordPtr[2] != 0x02 && headerRecordPtr[2] != 0x03)
            fileType = 0;

        // If the Color Map Type field is '0' then bytes 4-8 should
        // be 0.
...
//	Revised By:			Larry L. Biehl			Date: 06/18/2018	
