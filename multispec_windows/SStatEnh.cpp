...
//									All rights reserved.
//
//	File:						SSttatisticsEnhancement.cpp
//
      ...
      xloglPtr[iount] = 0.0;
    	...
    	for (i=0; i<numberClasses; i++)
        	...
        	if (priorsPtr[i] > threshold2)
				...
				continueFlag = GetClassSumsSquares (
...
//		   buffer4BytePtr = (HSInt32Ptr)inputBufferPtr2;
...
//											numberLPixels,
	...
	if ( lContBlock > (bytesNeeded + kMinFreeMemory) )
		...
		bytesNeeded = numberChannels * sizeof(double);
//		bytesNeeded = numberChannels * sizeof(SInt32);
...
//		numberULPixels = MIN(numberULPixels, numberLPixels);
		...
		numberPixels = GetNumberPixelsInArea (
...

{
	...
	classPtr = (UInt16*)GetHandlePointer (
											statEnhanceSpecsPtr->classHandle,
											kNoLock,
											kNoMoveHi);
	
			// List the soft outlier threshold and threshold value if		
			// used.												
	...
	if (statEnhanceSpecsPtr->softThresholdCode == kUsePercentThreshold)
		continueFlag = ListSpecifiedStringNumber ( 
			...
			// List the hard outlier threshold and threshold value if		
			// used.					
...

{
	...
	if (gProjectInfoPtr->statisticsEnhanceSpecsH == NULL)
		...
		if ( gProjectInfoPtr->statisticsEnhanceSpecsH )
			...
			gStatEnhanceSpecsPtr = (StatEnhanceSpecsPtr)GetHandlePointer (
					...
					// Initialize the statistics enhancement specification 			
					// structure.																	
	...
	if (continueFlag)
		...
		classPtr = (SInt16*)CheckHandleSize ( 
										&gStatEnhanceSpecsPtr->classHandle,
										&continueFlag, 
										&changedFlag, 
										bytesNeeded);
...

{
	...
	classInfoPtr = GetClassInfoStructure	(
												(UInt16)gStatEnhanceSpecsPtr->numberClasses);	
...
//		gStatEnhanceSpecsPtr->unlabelDataPtr = (HSInt32Ptr)
//							MNewPointer(numberPixels * numberChannels * sizeof(SInt32));
...
//			gStatEnhanceSpecsPtr->labeledDataPtr = (HSInt32Ptr)
//					MNewPointer(numberClasses * 5 * numberChannels * sizeof(SInt32));
...
// 											kForce4Bytes,
	...
	if (continueFlag)
		...
		numberPixels = GetFieldDataValues (	
												&gAreaDescription, 
												fileIOInstructionsPtr, 
												gProjectInfoPtr->channelsPtr, 
												(SInt16)numberChannels, 
	...
	gStatEnhanceSpecsPtr->classTypicalityPtr = (TypicalitySummaryPtr)CheckAndDisposePtr (
		...
		if ( LoadStatisticsEnhanceSpecs () )
			...
			if ( StatisticsEnhanceDialog () )
				...
				classPtr = (UInt16*)GetHandlePointer (
						...
						// Update statistics for project if needed. Remember, do
						// not allow enhanced statistics as the base unless the
...
//
//	Coded By:			Larry L. Biehl			Date: 11/19/1993
//	Revised By:			Larry L. Biehl			Date: 03/20/2014
