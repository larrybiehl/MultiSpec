...
//#include "SMemUtil.cpp"
//#include "SDlgUtil.cpp"
//#include "cstubs.cpp"
#endif
			...
			if (gProjectInfoPtr != NULL)
				...
				if (gTransformationMatrix.createdByCode < 16)
					...
					if (gTransformationMatrix.numberChannels <= 
						...
						{ 			
						for (index=0; index<(UInt32)gTransformationMatrix.numberChannels; index++)
							{
...

{
	...
	if (gProcessorCode == kClusterProcessor)
		...
		if (fileIOInstructionsPtr->fileInfoPtr->noDataValueFlag)
			...
			// should occur for a command-. and status information. This is not needed called
			// for updating train or test area statistics. These have been set in the calling
			// routine.
...
//							with what is now used in the isodata cluster processor to determine
//							whether data values are to be thresholded.
//
			...
			for (channel=0; channel<numberChannels; channel++)
				...
				if (nonSatClusterDistance > 0)
					...
					if (tailDifference > 3 * nonSatClusterDistance)
						minThresholdValuesPtr[channel] = histogramSummaryPtr[channelIndex].minNonSatValue;
						
					else		// tailDifference <= 3 * nonSatClusterDistance
						minThresholdValuesPtr[channel] = histogramSummaryPtr[channelIndex].minValue;

					tailDifference = histogramSummaryPtr[channelIndex].maxValue - 
														histogramSummaryPtr[channelIndex].maxNonSatValue;
														
					if (tailDifference > 3 * nonSatClusterDistance)
						maxThresholdValuesPtr[channel] = histogramSummaryPtr[channelIndex].maxNonSatValue;
						
					else		// tailDifference <= 3 * nonSatClusterDistance
						maxThresholdValuesPtr[channel] = histogramSummaryPtr[channelIndex].maxValue;
						
					}		// end "if (nonSatClusterDistance > 0)"
					
				else		// noSatClusterDistance == 0
					{
							// Min and max non-saturated values are the same. Will check if distance from min and 
							// min non-saturated is very different from min and max non-saturated and vice versa.
...

{
	...
	if (eigenVectorPtr && tempMatrixPtr)
		...
		if (squareOutputMatrixFlag == kTriangleOutputMatrix)					
			SquareToTriangularMatrix (
						covariancePtr, covariancePtr, numberFeatures );
...
//				fFormatDecimalDigits = MAX (0, fFormatDecimalDigits);
				...
				}		// end "if (value >= kMinValueToListWith_f && value <= kMaxValueToListWith_f)"
...
//				sprintf (stringPtr,  "\t%12.4f", covariancePtr[index]);
...
//				sprintf (stringPtr,  "\t%9.2f", covariancePtr[index+numberComponents]);
				numberChars = LoadRealValueString (stringPtr,
...
{ 
	...
	if (createByCode < 16)
		continueFlag = ListEigenTransformationInformation (
														covariancePtr, 
														eigenVectorPtr, 
														numberIterations, 
														smallestElement, 
		...
		continueFlag = ListOffsetGainTransformationInformation (
														covariancePtr, 
														eigenVectorPtr,
														numberChannels,
														listEigenvalueFlag);
...

{
	...
	if (numberOutputChannels == 0 || !inputTriangleMatrixPtr ||
								numberInputChannels == 0 || !outputSquareMatrixPtr)  
																						return;
		...
		for(j=0; j<inputSize1; j++) 
	   	...
	   	lIn1Ptr++;
	    	inputMatrixPtr++;
				...
				// decision boundary. It will only work if the data values
				// are integer multiples. I do not understand it yet.
				// 7-17-96.
				
//		dataValue = (SInt32)(doubleValue+.5);
//		*outputVectorPtr = (float)dataValue;
		*outputVectorPtr = doubleValue;
//		*outputVectorPtr = (float)doubleValue - *transformFeatureMeansPtr;		
		outputVectorPtr++;
