	...
	if (continueFlag)
		...
		while (noChangeCount < (UInt32)final_features) 
			...
			if (firstTimeFlag || group_bands[i] > 1)
				{
						// Only go through this loop if the we have been through it
						// one time and the number of bands in the
						// group is more than 1. If only 1 then nothing changes.
...
*/
	...
	try
		{
		continueFlag = First_stage_matrix(
								group_of_bands,
   							last_group_of_bands,  
								final_num_features,
	          				number_of_features, 
...
//								(Str255*)"\pTD Number feature groups:");
		...
		continueFlag = First_stage_topDownAlgorithm (
												final_num_features, 
			    				 				feature_type,
...
//			last_group_of_bands = ivector(1, max_number_features);
		...
		if (projectionPursuitSpecsPtr->initialGroupingCode == kLastGrouping)
			...
			if (projectionPursuitSpecsPtr->algorithm != kPPFeatureSelection)
				...
				if (gProjectInfoPtr->covarianceStatsToUse == kOriginalStats ||
					...
					if (min_num_samples > 1)
						newInitialNumberFeatures = MAX(initialNumberFeatures,
			...
			SetupGroupingVector (
								group_of_bands, number_of_features, final_num_features);
...
//				final_num_features = max_number_features;
		...
		if (continueFlag)
			...
			if (continueFlag && gFeatureExtractionSpecsPtr->listTransformationFlag)
				...
				continueFlag = ListSpecifiedStringNumber ( 
										kFeatureExtractStrID, 
										IDS_FeatureExtract33, // "%s Preprocessing Channel Transformation Matrix"
										(unsigned char*)&gTextString,
										resultsFileStreamPtr, 
										gOutputForce1Code,
										continueFlag );
		...
		if (continueFlag && 
			...
			{
			LoadDItemStringNumber (	kFeatureExtractStrID, 
											IDS_FeatureExtract65, // "\pProjection Pursuit - Numerical Optimization"
											gStatusDialogPtr, 
											IDC_Status21, 
											(Str255*)&gTextString );
...

{
	...
	bytesNeeded = 
			2 * projectionPursuitSpecsPtr->finalNumberFeatures * sizeof(double);
	if (continueFlag)
...
}		// end "CopyTransformationToVector"
