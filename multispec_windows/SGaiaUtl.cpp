...

{
	...
	while (classIndex <= numberGAIAClasses)
		{
		/*		CopyPToP ( classNamePtr, (char*)"\0Class \0");
		 NumToString ( (SInt32)classIndex, gTextString);
		 numberLength = (SInt16)MIN(gTextString[0], 25);
		 BlockMoveData ( (char*)&gTextString[1], &classNamePtr[7], numberLength);
...
{    
	...
	if (errCode == noErr)
		...
		if (gImageFileInfoPtr && gImageFileInfoPtr->channelDescriptionH && 
			...
			{
			HLock (gImageFileInfoPtr->channelDescriptionH);
	...
	if (errCode == noErr)
		...
		for (classNumber=0; classNumber<numberGAIAClasses; classNumber++)
			...
			continueFlag = GetGAIAClassMeans (
									classChannelStatsPtr, 
									numberChannels, 
									channelListPtr, 
									classNumber);
	...
	if (errCode == noErr)
		...
		for (index=1; index<=numberGAIAClasses; index++)
			{	
			errCode = MWriteData(fileStreamPtr, &count, tempHeaderRecordPtr, kErrorMessages);
