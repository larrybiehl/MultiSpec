...
// 		ComputeEigenvectors (finds eigenvectors of the sum of all the class pair feature matrices)
// 		
...
extern void 	FindEigenvaluesOfUpperHessenbergMatrix (
						double*								inputMatrixPtr, 
						UInt32								n, 
						double* 								wr, 
						double* 								wi,
...
extern SInt32 		MatlabEigFunction (
							double*									inputMatrixPtr,
							double*									eigenvectorPtr,
							double*									eigenvaluePtr,
							UInt32									numberFeatures);
...
{				
	...
	if (*ERROR_FLAG == 0)
		...
		gFeatureExtractionSpecsPtr->threshold1 = 
													-gFeatureExtractionSpecsPtr->threshold1;
		...
		gFeatureExtractionSpecsPtr->threshold2 = 
													-gFeatureExtractionSpecsPtr->threshold2;
		...
		if (*ERROR_FLAG == 0)
			...
			numberClassPairs = (SInt32)GetNumberOfCombinations (
																		(SInt32)no_class, 
																		2, 
																		1, 
																		FALSE,
...
//			continueFlag = ListSpecifiedString ( (SInt16)i, NULL, gOutputForce1Code );  
			...
			for (j=i+1; j<no_class; j++)
				...
				if (classPairWeightPtr[classPairCount-1] > 0)
					...
					if (countOKFlag && *ERROR_FLAG == 0)
						pixelPairs = FS_sub_find_edbfm_2_class(
																	class_info,
											  						class_index,
											  						2,
											  						no_feature,
					...
					symbolIndex1 = 0;
					if (countMahDistance[2] < (SInt32)gFeatureExtractionSpecsPtr->minimumThresholdNumber)
						{
...
//									(SInt32)gFeatureExtractionSpecsPtr->minimumThresholdNumber;
					...
					symbolIndex2 = 0;
					if (countMahDistance[3] < (SInt32)gFeatureExtractionSpecsPtr->minimumThresholdNumber )
						{
...
//									(SInt32)gFeatureExtractionSpecsPtr->minimumThresholdNumber;
		  			...
		  			if (gFeatureExtractionSpecsPtr->optimizeClassFlag) 
						...
						if (countOKFlag && *ERROR_FLAG == 0)
		   				numberFeatures = FS_optimize_2_class (
		   										class_info,
				   								class_index,
				   								2,
				   								no_feature,
		...
		if (*ERROR_FLAG == 0 || *ERROR_FLAG == 590)
			...
			BlockMoveData (
				edbfm_all, eigen_values, (SInt32)no_feature*sizeof(double) );
	 		...
	 		for (j=0; j<no_feature; j++)
	 			{
	  			for (k=0; k<no_feature; k++)
	   			{
	    			*(Sw+j*no_feature+k) += 
	    						*((class_info+i)->cov+j*no_feature+k) * weight;
								...
								
{
	...
	accuracyThreshold = 
					accuracy * gFeatureExtractionSpecsPtr->optimizeThreshold/100.;
				...
				
{
	...
	if (no_points > 0)
		...
		for (i=0; i<no_points; i++)
			{
			rejectedFlag = FS_sol_bnd_line_2(
									mean_icov_diff,
									icov_diff,
									c,
									*(point1_array+i),
									...
									
{
	...
	if (*ERROR_FLAG == 0)
		...
		memoryBlockPtr += numberBytes;
			...
			array_index[0] = (long*)memoryBlockPtr;
			memoryBlockPtr += numberBytes;
			numberBytes = (UInt32)(sizeof(long)*samplesInOtherClass);
			array_index[1] = (long*)memoryBlockPtr;
			memoryBlockPtr += numberBytes;
		#else
		...
		for (i=0; i<no_class; i++)
			...
			for (k=0; k<numberSamples; k++)
				{		 
				if ( (cl_res+i)->mah_dis[k]-tmp < 
												(cl_res+i)->mah_dis_the_other[k] )
		  			{
		  			error++;
...
{							
	...
	for (i=0; i<numberClasses; i++)
		{
		maxNumberSamplesInOneClass = MAX (
							maxNumberSamplesInOneClass, 
		...
		if (i <= m && i != n) 
			...
			if (scale) 
				...
				for (j=l;j<=m;j++) 
					...
					for (k=l;k<=n;k++) a[j][k] += s*rv1Ptr[k];
