...
//									All rights reserved.
//
//	File:						SGrafVew.cpp : implementation file 
//
//	Functions in file:	
//
//	Brief description:	This file contains functions that relate to the graph view class.
//
//	Include files:			"SGrafVew.h"
//								"SMulSpec.h"
//
//	Language:				C
//
...
#			if defined multispec_win	
				left = 95 - textWidth - 1;
				top  = (SInt16)(graphWindowRect.bottom - graphWindowRect.top - 7);
			...
			if (!printCopyFlag)
				{
					...
					graphRecordPtr->pDC->MoveTo (0, 
							graphRecordPtr->clientRect.bottom-kSBarWidth-2);
...
Boolean 
CMGraphView::FinishGraphRecordSetUp(
...
{  
	...
	if (graphRecordPtr != NULL)
		...
		if (continueFlag && 
			...
			continueFlag = LoadGraphXVector (graphRecordPtr, channelListPtr, vectorLength);
...
void 
CMGraphView::IntializeGraphRecord (void)
...
void 
CMGraphView::SetCheckIOMemoryFlag (
				Boolean				flagSetting)
...
int 
CMGraphView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{                  
...
void 
CMGraphView::OnInitialUpdate(void)
...
//	CMGraphDoc* graphDocCPtr = (CMGraphDoc*)GetDocument();
...
void 
CMGraphView::MoveGraphControls (void)

{
	...
	if (pushButton != NULL)
		...
		left = 95;  
		top = graphWindowRect.bottom - graphWindowRect.top - 20;
...
void 
CMGraphView::OnUpdateFileSaveAs(
				CCmdUI* 		pCmdUI)
{                                                                                                                     
	pCmdUI->Enable(FALSE);
...
void 
CMGraphView::OnPrint(CDC* pDC, CPrintInfo* pInfo)
...
void 
CMGraphView::OnUpdateEditCopy(CCmdUI* pCmdUI) 
{
...
void CMGraphView::OnNextChannel() 
{
	...
	if ( ((CMultiSpecApp*)AfxGetApp())->GetZoomCode() == 
															IDC_NextChannel )
		{
...
void CMGraphView::OnSize(UINT nType, int cx, int cy) 
{
//	CButton*				pushButton;
...
//	if (pushButton != NULL && pushButton->IsWindowVisible())
		MoveGraphControls ();
...
void CMGraphViewButton::OnLButtonDown (UINT nFlags, CPoint point)
...
{                                                      
	...
	if (m_buttonID == IDC_NextChannel || 
						m_buttonID == IDC_PreviousChannel)
		{
		if ( GetState() & 0x0004 )
			{
...
void 
CMGraphViewButton::OnMouseMove(UINT nFlags, CPoint point)
...
void CMGraphViewButton::OnLButtonUp(UINT nFlags, CPoint point)
{
	CWnd*			captured;
...
void CMGraphViewButton::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{     
	if (nChar == 0x10)
		{                
...
void CMGraphViewButton::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags)
{    
	if (nChar == 0x10)
		{                
//		CMLegendView* legendViewCPtr = (CMLegendView*)GetParent(); 
...
void 
CMGraphViewButton::OnRButtonDown(UINT nFlags, CPoint point)
...
void CMGraphViewButton::OnRButtonUp(UINT nFlags, CPoint point)
{  
	if (GetCapture() == this)                                                            
		((CMultiSpecApp*)AfxGetApp())->SetControlDelayFlag (TRUE); 
