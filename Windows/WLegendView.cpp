...
//
//	File:						WLegendView.cpp : implementation file
//
...
//								CMLegendView, CMButton, and CMComboBox classes.
//
//------------------------------------------------------------------------------------
...
void CMLegendView::AdjustLegendListLength (void)

{
	...
	if (windowType == kThematicWindowType)
		...
		m_numberDisplayedListItems =
									(SInt16)((legendRect.bottom - 17 - listRect.top)/height);
...
void CMLegendView::DoDataExchange (
				CDataExchange* 					pDX)
...
void CMLegendView::OnActivateView (
				BOOL 									bActivate,
				CView* 								pActivateView,
				CView* 								pDeactiveView)
...
LRESULT CMLegendView::OnDoRealize (
				WPARAM 								wParam,
				LPARAM 								lParam)
...
BOOL CMLegendView::OnEraseBkgnd (
				CDC* 									pDC)
...
void CMLegendView::OnInitialUpdate (void)
...
void CMLegendView::OnScrollDown ()
...

{
	...
	m_paletteSelection = m_legendPaletteCombo.FindStringExact (-1, _T("Palette"));
   ...
   SInt16								lastComboPaletteSelection,
	...
	if (paletteSelection == kFalseColors)
		...
		if (FalseColorPaletteDialog ())
			...
			paletteSelection =
							(SInt16)m_legendPaletteCombo.GetItemData (m_paletteSelection);
	...
	lastPaletteSelection =
					(SInt16)m_legendPaletteCombo.GetItemData (lastComboPaletteSelection);
...
void CMButton::OnMouseMove (UINT nFlags, CPoint point)
...

{
   ...
   legendWidth = gActiveImageViewCPtr->GetLegendWidth ();
	int left =
			(int)(legendWidth * gActiveImageViewCPtr->m_printerTextScaling - size.cx) /2;
	left = MAX (0,left);
