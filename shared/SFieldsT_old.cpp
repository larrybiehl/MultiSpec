...
//								void 				ENVI_ASCII_ROI_ToThematicFileControl
...
//	Revised By:			Larry L. Biehl			Date: 03/13/2014	
		...
		while ((fieldNumber != -1) && continueFlag)
			...
			if (requestedFieldType & fieldIdentPtr->fieldType)
				...
				if (pointType != kClusterType)
					...
					numberFieldColumns = 
						(fieldColumnEnd - fieldColumnStart + columnInterval)/
																					columnInterval;
					...
					if (pointType == kMaskType)
						...
						savedMaskBufferPtr = (HUInt16Ptr)GetHandlePointer(
												areaDescription.maskInfoPtr->maskHandle,
												kLock,
												kNoMoveHi);
	...
	
{
	...
	inputOffScreenMapHandle = 
							(PixMapHandle)inputWindowInfoPtr->offScreenMapHandle;
	...
	if (continueFlag)
		...
		sprintf( (char*)&gTextString, 
					"  %hd classes were generated (including a background class).\r",
					(short int)(*thePictInfo.thePalette)->pmEntries+1);
		continueFlag = ListString ((char*)&gTextString, 
											strlen((char*)gTextString), 
											gOutputTextH );
		...
		if (continueFlag && outFileInfoPtr->format == kGAIAType)
			...
			if (continueFlag)
				InitializeGAIALineBytes (
								(unsigned short int*)gOutputBufferPtr, 
	...
	
{
	...
	if (continueFlag)
		...
		outFileInfoPtr = (FileInfoPtr)GetHandlePointer(
												outputFileInfoHandle, kLock, kNoMoveHi);
	...
	if (continueFlag)
		...
		if (continueFlag)
			continueFlag = CovertMultispectralToThematic (
											inputWindowInfoPtr, 
											outFileInfoPtr, 
											reformatOptionsPtr);
...

{
	...
	DrawPopUpMenuItem (dialogPtr, itemNumber, 
						gPopUpDiskFileFormatMenu, gOutputFormatCode, TRUE);
...

{
	...
	if (continueFlag)
		...
		if (continueFlag)													
			...
			StringPtr fileNamePtr = GetFileNameCPointer (inputFileStreamPtr);
			sprintf(	(char*)&gTextString, 
						"    ROI text file = '%s'%s", 
						fileNamePtr,
						gEndOfLine);
			...
			
{
	...
	if (continueFlag)
		...
		if (continueFlag)
			{
			if (CompareStringsNoCase (*inputStringPtrPtr, (char*)"; ENVI Output of ROIs:", 19) == 0)
//			if (strncmp (*inputStringPtrPtr, (char*)"; ENVI Output of ROIs", 19) == 0)
	...
	if (*asciiFormatCodePtr == 1)
		...
		returnCode = sscanf (*inputStringPtrPtr,
			...
			
{
	...
	while (notDoneFlag)
		...
		if (continueFlag)
			...
			stringCompare = strncmp (inputStringPtr, (char*)roiName, 8);
			...
			else if (strncmp (inputStringPtr, (char*)"ROI Color", 9) == 0)
				{
				returnCode = sscanf (inputStringPtr, 
										"ROI Color: (%hd,%hd,%hd)",
										&red, 
										&green,
										&blue);
			...
			else if (strncmp (inputStringPtr, (char*)roiPixel, 1) == 0)
				{
				returnCode = sscanf (inputStringPtr, 
										"(%ld ) , (%ld)",
										&column, 
	...
	if (pointsPerClassVector != NULL)
		...
		for (index=0; index<numberClasses; index++)
			...
			while (continueFlag && lineNotFoundFlag)
				...
				if (errCode == noErr)
					{
					if (CompareStringsNoCase (inputStringPtr, (char*)"; ROI name:", 11) == 0)
						{
						ENVI_ROIToThematicGetClassName (inputStringPtr, classNamePtr);
						classNamePtr += 32;
			...
			if (continueFlag)
				...
				if (continueFlag && 
						CompareStringsNoCase (inputStringPtr, (char*)"; ROI rgb value:", 16) == 0)
					{
					returnCode = sscanf (inputStringPtr, 
			...
			if (continueFlag)
				...
				if (continueFlag &&
							CompareStringsNoCase (inputStringPtr, (char*)"; ROI npts:", 11) == 0)
					{
					returnCode = sscanf (inputStringPtr, 
													"; ROI npts: %ld",
...

{
	...
	if (continueFlag)
		...
		//outFileInfoPtr->bandInterleave		= kBIL;    			// kBIL  
		//outFileInfoPtr->numberHeaderBytes 	= 128;				// Assuming ERDAS .GIS format for now.
		outFileInfoPtr->numberTrailerBytes 	= 0;
		outFileInfoPtr->numberPreLineBytes 	= 0;
		outFileInfoPtr->numberPostLineBytes = 0;
		outFileInfoPtr->numberPreChannelBytes 	= 0;
...
//	Revised By:			Larry L. Biehl			Date: 03/28/2003
