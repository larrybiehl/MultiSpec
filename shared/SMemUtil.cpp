...
//				outputPtr = GetHandlePointer (handle, kLock, kMoveHi); // changed to kNoMoveHi on 3/17/2013 to test problem with OSX 8
				outputPtr = GetHandlePointer (handle, kLock, kNoMoveHi);
				
			
		#else // if defined multispec_lin
			Handle	newHandle;
...
//	Revised By:			Larry L. Biehl			Date: 12/16/2008
// For now, just returning TRUE for multispec_lin
Boolean CheckMemoryForColorPicker (
				Str255*								stringPtr, 
				SInt16								stringNumber)
	...
	if (inputHandle != NULL && outputHandle != NULL)
		...
		if (sizeInputHandle == sizeOutputHandle)
			...
			if (sizeInputHandle > 0)
				{
				Ptr inputPointer = GetHandlePointer (inputHandle, kNoLock, kNoMoveHi);
				Ptr outputPointer = GetHandlePointer (outputHandle, kNoLock, kNoMoveHi);
...
{  
	...
	if (sizeInputHandle > 0)
		...
		if (continueFlag)
			{ 
			inputPointer = GetHandlePointer (inputHandle, kNoLock, kNoMoveHi);
...
{  
   ...
   sizeInputPointer = getsize((void *)inputPointer);
	...
	if (sizeInputPointer > 0)
		...
		if (continueFlag)
			{ 
			outputPointer = GetHandlePointer (*outputHandlePtr, kNoLock, kNoMoveHi);
         #ifndef multispec_lin
...
{                                
	...
	if (forceOutputByteCode == kDoNotForceBytes)
		...
		numberOutBytes = 2;
	else if (forceOutputByteCode == kForce4Bytes || forceOutputByteCode == kForceReal4Bytes)
		numberOutBytes = 4;
...
#	endif
	...
	UInt32 longestContBlock = ::GlobalCompact(0);
	...
	SInt16 itemHit = AfxMessageBox((LPCTSTR)A2T((LPCSTR)gTextString), MB_ICONSTOP | MB_OK, 0);
...
//	Revised By:			Larry L. Biehl			Date: 03/1277/2017	
...
//		rlimit								resourceLimits;
				...
				// It is difficult to determine the amount of memory available in OSX. There
				// are no routine that I have found to give an estimate directly. Suggestions
				// are to use Gestalt-gestaltPhysicalRAMSize but this is only valid when
				// memory available is 2 Gig or less. sysctl has also been suggested. I don't
				// know how to use this yet. Knowing physical RAM is just a start because one
				// needs to know how much the application is taking up.
				// The approach being used is to turn the memory flag off and just make requests
				// at 100 megabytes at a time less until an amount is found that works. I will
				// start checking at 1.1 gigabytes of RAM for now. 1 gig seems to be the starting
				// point for the machine being used now ... Mac Pro with 8 gig of ram.
...
//		*lContBlockPtr = GetFreeSpace(0);
		GlobalMemoryStatus(&memoryStatus);
		*lContBlockPtr = memoryStatus.dwAvailPageFile;
		*lContBlockPtr = MIN(*lContBlockPtr, (UInt32)(1.5*memoryStatus.dwAvailPhys));
		freeMemory = memoryStatus.dwAvailPageFile;
	//#else
	//	*lContBlockPtr = ::GlobalCompact(0);
	//	freeMemory = GetFreeSpace(0);
	//#endif
...
//			#if !defined _WIN32                                           
//				UInt32 selector_handle = GlobalHandle ( SELECTOROF( (LPSTR)pointer ) );
//				handle = (Handle)LOWORD(selector_handle);
//			#else
//				UInt32 selector_handle = (UInt32)GlobalHandle ( 
//														(LPCVOID)(HIWORD( (LPSTR)pointer )) );
...
//	Revised By:			Larry L. Biehl			Date: 04/07/1995	
// 
	...
	if (CheckIfBytesRequestedAreWithinLimit (numberBytes))
		{	     
			...
			if (handle != NULL)
				...
				HPtr hptr = (HPtr)GetHandlePointer (handle, kNoLock, kNoMoveHi);
...
{       
	...
	if (CheckIfBytesRequestedAreWithinLimit (bytesNeeded))
		{	 
			...
			if (*handlePtr != NULL)
				{
				Handle newHandle = ::GlobalReAlloc (*handlePtr, (SIZE_T)bytesNeeded, GMEM_MOVEABLE);
...
{   
	...
	//			#if !defined _WIN32                                           
	//				UInt32 selector_handle = GlobalHandle ( SELECTOROF( (LPSTR)*pointerPtr ) );
	//				handle = (Handle)LOWORD(selector_handle); 
	//			#else
	//				UInt32 selector_handle = (UInt32)GlobalHandle ( 
	//																(LPCVOID)(HIWORD( (LPSTR)*pointerPtr )) );
					handle = GlobalHandle ((LPSTR)*pointerPtr);
	//			#endif	// !defined _WIN32  
				
				Ptr newPointer = (Ptr)::GlobalLock( ::GlobalReAlloc (handle, (SIZE_T)bytesNeeded, GMEM_MOVEABLE) );
...
{    
	UInt32				sizeSpare = 0;
		...
		if (gSpareWarnMemoryH != NULL)
			sizeSpare = ::GlobalSize (gSpareWarnMemoryH);
...
void *memorycopy(void *output, void *input){
    //void *temp;
    //memcpy(temp,(void *)((onebyte *)input-memsize),(size_t)(getsize(input)+memsize));
    //output = (void *)((onebyte *)temp + memsize);
    //return output;
   int input_memsize = getsize(input); 
   memcpy((void *)((onebyte *)output-memsize),(void *)((onebyte *)input-memsize),(size_t)(input_memsize+memsize));
    //output = (void *)((onebyte*)output + memsize);
}
#endif
