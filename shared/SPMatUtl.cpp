...
//
//	Software purpose:	The purpose of this routine is to find the maximum diagonal element
//							in the input double square matrix and return that maximum value.
//
//	Parameters in:		None
//
//	Parameters out:	None
	...
	if (classNamesPtr[classStorage].numberOfTrainFields > 0)
		...
		if (gFeatureExtractionSpecsPtr != NULL && (optionsCode & kComputeColumnInterval))
			...
			gAreaDescription.columnInterval = 
													MAX(gAreaDescription.columnInterval, 1);
		...
		while (fieldNumber != -1 && continueFlag)
			{
					// Make certain that field is training field
					// Do not include the field if it has not been included in the statistics.						
...
/*	
	...
	InitializeAreaDescription (&maskAreaDescription, 
		...
		if (errCode != kSkipLine)
			...
			for (sample=1; sample<=numberSamples; sample++)
				...
				if (checkIfTrainSampleFlag)
							...
							// option, the user does not want to use any samples that are being
							// used for training. This option is used by the Statistics 
						...
						if (transformDataFlag)
							...
							if (doubleDataValuePtr != NULL)
								...
								for (channel=0; channel<numberChannels; channel++)
									doubleDataValuePtr[channel] = (double)bufferValuePtr[channel];
...

{
	...
	stopFlag = FALSE;
			...
			// Get smallest pivot value allowed.  It will be based on the number of unique 
			// digits in a double value. I.E. Max diagonal element in input matrix less the 
