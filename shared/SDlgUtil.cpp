...
//      wxTextCtrl* textctrl = (wxTextCtrl*)dialogPtr->FindWindow(itemNumber);
      ...
      wxTextCtrl* textctrl = wxDynamicCast(dialogPtr->FindWindow(itemNumber), wxTextCtrl);
      if(textctrl != NULL) {
         stringValue = textctrl->GetValue();
      }else{
         wxComboBox* combobox = wxDynamicCast(dialogPtr->FindWindow(itemNumber), wxComboBox);
...

{
	...
	if (gStatusDialogPtr != NULL && gFromToolParameterFileFlag)
		{
				// This implies that the histogram function is being called within a tool parameter
				// process and the status dialog box is being displayed for that. It is not needed
				// now. Close the status dialog box and get a new one.
...
//						statusDialogPtr, 7, &gStatusGraphicsBox, gDrawGraphicsBoxPtr);
	...
	#if defined multispec_lin
		//CShortStatusDlg* statusDialogPtr2 = new CShortStatusDlg(statusInfoID, (wxWindow *)gActiveImageViewCPtr->m_frame);
		CShortStatusDlg* statusDialogPtr2 = new CShortStatusDlg(statusInfoID, (wxWindow*)GetMainFrame());
	...
	
{
	...
	channelsPtr = (UInt16*)GetHandlePointer (
													featureHandle, kNoLock, kNoMoveHi);
	...
	if (localClassAreaPtr != NULL && classAreaHandle != NULL)
		{
		UInt16* classAreaPtr = (UInt16*)GetHandlePointer(
													classAreaHandle, kNoLock, kNoMoveHi);
	...
	if (localSymbolPtr != NULL && symbolsHandle != NULL)
		{
		char* symbolsPtr = (char*)GetHandlePointer(
													symbolsHandle, kNoLock, kNoMoveHi);
...
//		sprintf( 
//			(char*)&gTextString[1], "%.*f", decimalPlaces, realValue);
...
/*
							osErr = SetControlData (theControl,
																...
																CFStringGetLength(cfStringRef),
																CFStringGetCStringPtr(cfStringRef,kCFStringEncodingMacRoman));
*/
							if (validControlFlag)
...

{
		...
		if (initializeLineColumnValuesFlag)
			...
			if (lineColumnSetNumber == 1)
				...
				if (intervalEditBoxesExistFlag)
					dialogPtr->LoadDItemValue (IDC_LineInterval, dialogSelectAreaPtr->lineInterval);
				
						// Enter start, end and interval for columns.													
				
				dialogPtr->LoadDItemValue (IDC_ColumnStart, dialogSelectAreaPtr->columnStart);
				dialogPtr->LoadDItemValue (IDC_ColumnEnd, dialogSelectAreaPtr->columnEnd);
            
				if (intervalEditBoxesExistFlag)
					dialogPtr->LoadDItemValue (IDC_ColumnInterval, dialogSelectAreaPtr->columnInterval); 
				...
				//if (intervalEditBoxesExistFlag)
				//	dialogPtr->LoadDItemValue (IDC_LineInterval2, dialogSelectAreaPtr->lineInterval);
				
						// Enter start, end and interval for columns.													
				dialogPtr->LoadDItemValue (IDC_ColumnStart2, dialogSelectAreaPtr->columnStart);
				dialogPtr->LoadDItemValue (IDC_ColumnEnd2, dialogSelectAreaPtr->columnEnd);
				
						// So far no intervals for set 2
				//if (intervalEditBoxesExistFlag)
				//	dialogPtr->LoadDItemValue (IDC_ColumnInterval2, dialogSelectAreaPtr->columnInterval); 
				...
				//if (intervalEditBoxesExistFlag)
				//	dialogPtr->LoadDItemValue (IDC_LineInterval3, dialogSelectAreaPtr->lineInterval);
				
						// Enter start, end and interval for columns.													
				dialogPtr->LoadDItemValue (IDC_ColumnStart3, dialogSelectAreaPtr->columnStart);
				dialogPtr->LoadDItemValue (IDC_ColumnEnd3, dialogSelectAreaPtr->columnEnd);

						// So far no intervals for set 3
				//if (intervalEditBoxesExistFlag)
				//	dialogPtr->LoadDItemValue (IDC_ColumnInterval3, dialogSelectAreaPtr->columnInterval); 
...
//								gProjectInfoPtr->numberStatisticsChannels);
		...
		if (channelsHandle != NULL)
			{
			SInt16* channelPtr = (SInt16*)GetHandlePointer(
													channelsHandle,
													kNoLock,
													kNoMoveHi); 
	...
	if (symbolsHandle != NULL)
		...
		*symbolSetPtr = symbolSelection;
		...
		unsigned char* symbolsPtr = (unsigned char*)GetHandlePointer(
													symbolsHandle,
													kNoLock,
													kNoMoveHi);													
...
//		SysBeep(1); 
   ...
   #if defined multispec_lin
    // For now do nothing as in windows
   #endif
...
{  
	...
	Boolean returnFlag = dialogPtr->SetWindowPos (
										NULL, 
										newLeft, 
										newTop, 
										0, 
...
{  
	...
	if (numberSelectedFeatures < numberInputChannels)
		...
		channelSubsetsHeaderPtr = (SInt32*)CheckHandleSize ( 
														&gChannelSubsetsH,
														&returnFlag,  
														&changedFlag, 
														bytesNeeded);
...
//		CheckDlgButton(itemNumber, setting);
   ...
   #if defined multispec_lin
		wxCheckBox* buttonPtr = wxDynamicCast(dialogPtr->FindWindow(itemNumber), wxCheckBox);
		...
		if (buttonPtr != NULL)
			...
			{
			wxRadioBox* buttonPtr = wxDynamicCast(dialogPtr->FindWindow(itemNumber), wxRadioBox);
			if (buttonPtr != NULL)
				{
         ...
         wxRadioButton* buttonPtr2 = wxDynamicCast(dialogPtr->FindWindow(itemNumber), wxRadioButton);
         if (buttonPtr2 != NULL)
				{
...

{
	if (dialogPtr != NULL)
		{
		if (MGetString ((UCharPtr)stringPtr, strListID, stringNumber))
			SetDLogControlTitle ( dialogPtr, itemNumber, (CharPtr)stringPtr);
...
//	   		gStatusDialogPtr->MShowDialogItem(gStatusDialogPtr, IDCANCEL);
      ...
      //gStatusDialogPtr->SetSizerAndFit(((CShortStatusDlg*)gStatusDialogPtr)->bSizer35); 
