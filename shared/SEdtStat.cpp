...

{
	...
	if (gProjectInfoPtr->statsWindowMode == 3 &&
		...
		LSetCell (
			(char*)&gTextString[1], (short int)gTextString[0], cell, gStatisticsListHandle);
		...
		#if defined multispec_mac
//			InvalRect ( &((ListPtr)*gStatisticsListHandle)->rView );
			InvalWindowRect (gProjectWindow, GetListViewBounds(gStatisticsListHandle, &gTempRect));
		#endif	// defined multispec_mac
...
//		CheckEnhancedStatistics ();
		...
		for ( classIndex=removeClass;
			...
			classListComboBoxPtr = (CComboBox*)gProjectWindow->GetDlgItem(IDC_ClassList);				
...

{
	...
	if ( 	(removePoint >= 0) && 
		...
		gProjectInfoPtr->changedFlag = TRUE;
			...
			// Note that the first two points in the list represent the bounding rectangle
			// around the polygon.				
...
//			InvalRect ( &((ListPtr)*gStatisticsListHandle)->rView );
		...
		GetPolygonLabelPoint (&gProjectInfoPtr->fieldIdentPtr[currentField],
		...
		if (itemHit > 2)
			...
			if	(itemHit == 1)      // User selected OK for information 
				...
				if (classFieldFlag == 2)
					...
					returnCode = CheckForDuplicateClassName (classStorage, &gTextString);
					if ( returnCode != 0 )
						{
				...
				if (classFieldFlag == 3)
					...
					returnCode = CheckForDuplicateFieldName (currentField, &gTextString);
					if ( returnCode > 0 )
						{
						HiliteControl ((ControlHandle)okHandle, 255);
...
//
//	Software purpose:	The purpose of this routine is to initialize the edit class/field
//							dialog parameters. 
//
//	Parameters in:		None
//
...
{  
	...
	if (classFieldFlag == 2)
		...
		LoadDItemString (dialogPtr, IDC_ClassName, &gTextString);
	...
	if (classFieldFlag == 3)
		...
		LoadDItemString (dialogPtr, IDC_FieldName, &gTextString);
		...
		*selectedItemPtr = IDC_FieldName;  
		...
		gTextString[0] = sprintf ((char*)&gTextString[1],
											"Number pixels: %s",
											(char*)&gTextString2);
		LoadDItemString (dialogPtr, IDC_FieldNumberPixels, &gTextString);
...

{
	...
	while (nextFieldNumber != -1)
		...
		nextFieldNumber = 
					gProjectInfoPtr->fieldIdentPtr[nextFieldNumber].nextField;
...
//				InvalRect ( &((ListPtr)*gStatisticsListHandle)->rView ); 
		...
		MHiliteControl (
					gProjectWindow, gProjectInfoPtr->toProjectControlH, 0);
