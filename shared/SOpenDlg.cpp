				...
				Boolean								firstFileStreamLoadedFlag)
{
	...
	if (windowInfoHandle != NULL && fileInfoHandle != NULL)
		...
		if (!firstFileStreamLoadedFlag)
					// This implies that the first link has been loaded in the window structure.
					// Allow for it for later use with hdf like formatted files.
			linkOffsetIndex = 1;
		...
		while (!doneFlag) 
			...
			if (fileInfoLoadedFlag && fileFormat != kArcViewShapeType) 
				...
				if (GetBandInterleave(fileInfoHandle) != kBIS)
					...
					DisplayAlert(
                        kErrorAlertID, kCautionAlert, kAlertStrID, 15, 0, NULL);
		...
		//NumToString (  
		//				(SInt32)(windowInfoPtr->numberImageFiles-1), gTextString);
...
//#			endif
			...
			if (tReturnCode == 1 &&
				...
				if (bandNumber == 6 &&
                        !CompareStringsNoCase ((UCharPtr)"_B6_VCID_2", &fileName[bandNameStart], 10))
					fileVectorIndex = 7;
				...
				if ((bandNumber == 6 || bandNumber == 8) && itemCount == 6)
                    ...
                    // with only 6 channels. A reflective channel must have been left out.
					continueFlag = FALSE;
	...
	for (fileIndex = 0; fileIndex < itemCount; fileIndex++)
		...
		if (errCode == noErr)
			...
			if (!firstFileFoundFlag)
				...
				CopyPToP (savedFileName, fileName);
				savedFileName[0] -= 3;
				if (CompareStringsNoCase ((UCharPtr)"LT5", &savedFileName[1], 3) &&
							CompareStringsNoCase ((UCharPtr)"LT4", &savedFileName[1], 3))
					break;
            ...
            Boolean*                      callGetHDFLineFlagPtr)
{
   ...
   *hdfDataSetPtr = fileInfoPtr->hdfDataSetSelection;
   ...
   if (gProcessorCode == kOpenImageFileProcessor &&
      ...
      }     // end "if (gProcessorCode == kOpenImageFileProcessor && (fileInfoPtr->format == kHDF4Type || ..."
...
#				endif
			...
			*newMapProjectionHandlePtr = NULL;
					...
					// The map information change may affect the setting in the coordinate view. If
					// current setting is for decimal degrees or map units but there is no map 
					// information, force units to be line-column.
			...
			*newChannelToHdfDataSetHandlePtr = NULL;
			...
			{
					// This is the first time through and the first data set is selected. Make 
					// sure that the hfaHandle is saved.  
...
#		if defined multispec_lin
			...
			SInt64 mclientdata = (SInt64)((int*)comboBoxPtr->GetClientData(hdfDataSetSelection));
				...
				FSRef*								fileAsFSRefPtr)
{
	...
	if (fileInfoHandle != NULL) 
		{
		setUpFileInfoStructureFlag = CheckForLandsatFileList (itemCount, fileAsFSRefPtr, &instrumentCode);
        ...
        SInt16 messageCode)
{
	...
	fileNamePtr = (char*)GetFileNameCPointer (fileInfoHandle);
				...
				if (gMultipleImageFileCode == 0) 
					...
					if (gNumberOfIWindows + itemCount > kMaxNumberIWindows) 
						...
						if (itemCount <= 0 || itemHit == 2)
							...
							// this file is from a file handler situation. Some iRods files take
							// a long time to setup, read and display.
					...
					if (gFromToolParameterFileFlag)
						...
						if (gStatusDialogPtr != NULL)
							...
							LoadDItemValue (gStatusDialogPtr, IDC_ShortStatusValue, (SInt32)1);
		...
		for (dataSetIndex = 1; dataSetIndex <= numberHdfDataSets; dataSetIndex++) 
            ...
            SetMenuItemText (gPopUpTemporaryMenu,
				...
				cfStringRef = CFStringCreateWithBytes (kCFAllocatorDefault,
																	...
																	kCFStringEncodingUTF8,
																	false);
				//cfStringRef = CFStringCreateFromExternalRepresentation (kCFAllocatorDefault,
