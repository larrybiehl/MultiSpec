...

{
	...
	if (numberChannels > 0 && channelStatsPtr && covariancePtr &&
																		numberPixels > 0)  
		{
...

{
	...
	if (bytesNeeded > currentBufferBytes)
		...
		if (newBufferPointer != NULL)
			...
			usedBufferBytes = *inputStringPtrPtr - (UCharPtr)gCharBufferPtr1;
	...
	if (outputPtr != NULL)
		...
		if (outputPtr[0] == 0)
			...
			if (returnCode == kReturnASCII)
				...
				unicodeOutputPtr[0] = wcslen (&unicodeOutputPtr[1]);
...

{
	...
	if ( !projectWindowInfoPtr	)
			...
			// Now determine the size of a buffer needed to read the rest of the project
			// lines into that will be long enough for any matrix data.							
	...
	stringLength = strlen( (char*)&gTextString );
...
//	Revised By:			Larry L. Biehl			Date: 01/29/1999	
	...
	if (gProjectInfoPtr != NULL)
		...
		do
			...
			continueFlag = (errCode == noErr) & 
													FileExists(projectFileStreamPtr);
			...
			if (continueFlag)
				...
				if (gProjectInfoPtr->version < 0)
					...
					char* projectFileNameCPtr = 
											(char*)GetFileNameCPointer (projectFileStreamPtr);
...

{
	...
	projectFileNamePtr = (FileStringPtr)GetFileNamePPointer (projectFileStreamPtr);
	...
	if (saveCode == 0)
		...
		char* projectASCIIFileNamePtr =
							(char*)GetFileNamePPointer (projectFileStreamPtr);
...
{			
	...
	switch (type)
		...
		case 3:		
			...
			for (channel=0; channel<numberStatisticsChannels; channel++)
				{
				chanStatsPtr->minimum = (SInt32)strtod ((char*)inputStringPtr, (Ptr*)&inputStringPtr);
				if (errno == ERANGE)											 	
					break;
		...
		case 4:		
			...
			for (channel=0; channel<numberStatisticsChannels; channel++)
				{
				chanStatsPtr->maximum = (SInt32)strtod ((char*)inputStringPtr, 
																		(Ptr*)&inputStringPtr);
				if (errno == ERANGE)											 	
					break;
		...
		if (sumSquaresStatsPtr != NULL)
			...
			if( (**inputStringPtrPtr != kCarriageReturn && 
				...
				badStatisticsFlag = TRUE;	
					...
					while (paramBlockPtr->ioActCount == 
						...
						character = paramBlockPtr->ioBuffer[
															paramBlockPtr->ioActCount-1];
...
{ 
   ...
   #if defined multispec_win || defined multispec_lin		 		
		UInt32 numberCharsToReread = paramBlockPtr->ioActCount - 
		...
		if (numberValueBytes == k68881Bytes)
			{
					...
					BlockMoveData (*inputStringPtrPtr, 
								...
								// Reading project files with sane double values will not be allowed
								// in MachO linked version of MultiSpec.
	...
	if (gProjectInfoPtr != NULL && gProjectInfoPtr->windowInfoHandle)
      ...
      bufferBytes = 5000;
		...
		if (returnCode != noErr)
				...
				// Save the inputStringPtr value. It gets reset in the sscanf call on linux
		...
		if (gProjectInfoPtr->version == 890105)
         ...
         /*
		int numberChars = sprintf ((char*)&gTextString3,
											" SProjFIO::(lines, columns, channels, bytes, sLine, sColumn, bits): %d, %d, %d, %d, %d, %d, %d, %d, %d, %d%s", 
											projectWindowInfoPtr->maxNumberLines,
											projectWindowInfoPtr->maxNumberColumns, 
											projectWindowInfoPtr->totalNumberChannels,
											projectWindowInfoPtr->numberBytes,
       ...
       ListString ((char*)&gTextString3, numberChars, gOutputTextH);
		...
		bytesNeeded = 
			(SInt32)gProjectInfoPtr->numberStorageClasses * sizeof(SInt16);
		...
		if (gStatusDialogPtr)
			...
			LoadDItemValue (gStatusDialogPtr, IDC_Status6, 
		...
		for (classIndex=0; 
			...
			if (classNamesPtr->numberStatisticsPixels > 0)
				gProjectInfoPtr->statsLoaded = TRUE;
					...
					// Initialize first field number and last field number for the	
					// class.																		
            ...
            sscanfReturnCode = sscanf ((char*)inputStringPtr,
				...
				if (fieldIdentPtr->pointType <= kPolygonType)
					...
					for (point=0;
						  ...
						  point++)
						{
						sscanfReturnCode = sscanf ((char*)inputStringPtr, "V1\t%d\t%d\r",
		...
		switch (cardCode)
			...
			case 1:
				...
				if ( !(statsLoadedCode & 0x0002) )
					...
					errCode = 32;
					returnCode = ReadChannelInformation (
												chanStatsPtr, 
												&inputStringPtr,
												numberStatisticsChannels, 
												binaryListingCode+1);
			...
			case 5:				
				...
				errCode = 36;
				returnCode = ReadCovarianceInformation (
												sumSquaresStatsPtr,
												paramBlockPtr, 
												&inputStringPtr,
												numberStatisticsChannels, 
	...
	if (classFieldCode == 1)
		...
		if (statisticsBadFlag)						
			fieldIdentPtr->numberPixels = 0;
				...
				// If the number of pixels is 0 or less, then if this is a cluster
				//	field, then return an error. If not a cluster type, then force
				// the statistics to be recomputed.
...
//							Note that pre PowerPC processors are not being handled. I.e. 80 bit
//							doubles.
//
//	Parameters in:		None
//
...

{
	...
	if (saveCode == 2)
		...
		if (FileExists(projectFileStreamPtr))
			GetCopyOfPFileName (gProjectInfoPtr, (FileStringPtr)gTextString3);
	...
	if (saveCode <= 1)
		...
		if (GetProjectFileName (saveCode))
			...
			SignedByte handleStatus = 
										MHGetState (gProjectInfoPtr->windowInfoHandle);
...
//					outLongLongTemp = 0;
		...
		for (classIndex=0; 
			...
			while (field > -1 && continueFlag)
				{
						// Continue if field is a test field.  Otherwise go			
						// to next field.															
