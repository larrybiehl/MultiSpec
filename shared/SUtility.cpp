...
//							False: User cancelled closing the project (and session if this is
//										from a application exit event.
// 
// Called By:			Menus in menus.c
//							ExitMultiSpec in multiSpec.c
//
//	Coded By:			Larry L. Biehl			Date: 01/05/1989
//	Revised By:			Larry L. Biehl			Date: 01/06/1992	
...

{
	...
	if (windowInfoH != NULL)
		...
		if (displaySpecsH != NULL && fileInfoHandle != NULL)
			...
			windowRectPtr->top = 
				(SInt32)( ( lineColumnRect->top + lineOffset )/
					displaySpecsPtr->displayedLineInterval );
													
			windowRectPtr->left = 
				(SInt32)( ( lineColumnRect->left + columnOffset )/
					displaySpecsPtr->displayedColumnInterval );
			
			windowRectPtr->bottom = 
				(SInt32)( ( lineColumnRect->bottom  + lineOffset )/
					displaySpecsPtr->displayedLineInterval );
													
			windowRectPtr->right = 
				(SInt32)( ( lineColumnRect->right + columnOffset )/
					displaySpecsPtr->displayedColumnInterval );
	...
	if (continueFlag)
		...
		if (fileInfoPtr->numberBytes == 1)
			...
			endioOut2ByteBufferPtr = &localIOOut2ByteBufferPtr[
											numberLinesToFill * fileInfoPtr->numberColumns];
			for (; localIOOut2ByteBufferPtr<endioOut2ByteBufferPtr; localIOOut2ByteBufferPtr++)
...

{
	...
	resultsFileInfoPtr = diskFileListPtr->fileInfoPtr =
				(FileInfoPtr)GetHandlePointer (
											diskFileListPtr->fileInfoH, kLock, kMoveHi);

	resultsFileStreamPtr = GetFileStreamPointer(resultsFileInfoPtr);

	resultsFileNamePtr = (FileStringPtr)GetFileNamePPointer(resultsFileInfoPtr);
	...
	if (lOutputStorageType & (kProbFormatCode+kEchoFieldsCode+kEchoClassesCode))
		...
		if (fileZeroInfoPtr != NULL)
			{                            
			GetCopyOfPFileName (fileZeroInfoPtr, resultsFileNamePtr);
	...
	if (resultsFileNamePtr[0] == 0)
		...
		if (gProjectInfoPtr != NULL)
			{
			FileStringPtr fileNamePointer = (FileStringPtr)GetFileNamePPointer (gProjectInfoPtr);
	...
	if (resultsFileNamePtr[0] == 0)
		{
		FileStringPtr fileNamePointer = (FileStringPtr)GetFileNamePPointer (gImageFileInfoPtr);
		CopyPToP (resultsFileNamePtr, fileNamePointer);
...
//				#endif	// defined _WIN32 
		...
		CMFileStream* fileStreamPtr = GetFileStreamPointer (resultsFileInfoPtr);	
...
*/			
	...
	resultsFileNamePtr = (FileStringPtr)GetFilePathPPointer (resultsFileStreamPtr);     
...
//	Revised By:			Larry L. Biehl			Date: 06/22/1998
...
//			updateRectPtr->right = (SInt16)(drawInterval * numberChannels); 
			updateRectPtr->right = (SInt16)(drawInterval * 
				...
				// Also 'drawInterval' needs to be recomputed without the 
	#endif	// defined multispec_win 
...
//			updateRectPtr->right = (SInt16)(drawInterval * numberChannels); 
	...
	textWidth = (SInt16)(numChars * characterWidth + .5);
			...
			// Check if the drawinterval between the channel centers is less		
			// than the character width.  If so try just the descriptions			
			// in the titles.																		
		...
		if (includeDescriptionsFlag && 
			...
			if (localFileInfoPtr->channelDescriptionH != NULL)
				...
				channelDescriptionPtr = (ChannelDescriptionPtr)GetHandlePointer(
										localFileInfoPtr->channelDescriptionH, kLock, kNoMoveHi);
				gTextString[0] = (UInt8)numChars;
...
//		if (drawStart + textWidth > 0)
		if (imageRight >= 0)
			...
			if (channelDescriptionPtr != NULL)
				...
				BlockMoveData ( &channelDescriptionPtr[fileChannelNumber-1], 
...
//				size.cx /= 2;
            ...
            pdc.DrawText(image_label, (int)(drawCenter-width-columnOrigin), 
												(int)updateRectPtr->top);
            

//            gCDCPointer->DrawRectangle(updateRectPtr->left, updateRectPtr->top, 
//            updateRectPtr->right - updateRectPtr->left,15);      
...
//            gCDCPointer->DrawText(image_label, (int)(drawCenter-width-columnOrigin), 
//												(int)updateRectPtr->top);
//
		...
		do
			...
			if (compareWindowInfoPtr != NULL)
				...
				if (loadListFlag)
					...
					UCharPtr fileNamePtr = (UCharPtr)GetFileNamePPointer (fileInfoPtr);
...
//	Revised By:			Larry L. Biehl			Date: 04/02/1998	
...
{    
	...
	if (interval > 0)
		...
		allNonZeroMinDigitsFlag = GetNumberNonZeroLeadingDigits (
														xMin,
														logXMin,
														&nonZeroMinDigits);
		...
		allNonZeroMaxDigitsFlag = GetNumberNonZeroLeadingDigits (
														xMax,
														logXMax,
														&nonZeroMaxDigits);	
		...
		if (logXMin > logInterval)
			...
			if (allNonZeroIntervalDigitsFlag)
				nonZeroMinDigits = MAX (nonZeroMinDigits, (UInt16)(logXMin - logInterval));
...
//									(UInt16)(logXMin - logInterval + nonZeroIntervalDigits));
		...
		if (logXMax > logInterval)
			...
			if (allNonZeroIntervalDigitsFlag)
				nonZeroMaxDigits = MAX (nonZeroMaxDigits, (UInt16)(logXMax - logInterval));
...
*/					
				...
				fDecimalDigits2 = MAX ((SInt16)(fabs (logXMax - 1)), intervalDecimalPlaces);
...
//							for the requested probability threshold value.  This
//							routine must determine which type of probability map
//							it is for.  One type has 109 levels; the other type
//							has 120 levels.  The number of levels is obtained
//							from the input file information structure handle.
//							If that handle is NULL, then the current number
//							of probability levels is used.
//
//	Parameters in:		.
//
//	Parameters out:	None
...
{	
	...
	if (fieldTypeCode & kAreaType)
		...
		pixelCount = GetNumberPixelsInArea (
										selectionAreaPtr[0],
										selectionAreaPtr[1],
										selectionAreaPtr[2],
										selectionAreaPtr[3],
...
//					outRectanglePtr->right = outRectanglePtr->left + 30;
				...
				case kTitleArea:
					outRectanglePtr->top += windowInfoPtr->coordinateHeight;
					outRectanglePtr->left += windowInfoPtr->legendWidth;
					outRectanglePtr->bottom = outRectanglePtr->top + windowInfoPtr->titleHeight;
					outRectanglePtr->right -= kSBarWidth;
					break;
...
*/		
			...
			switch (areaCode)
				...
				case kImageFrameArea:
					//frameArea = (wxTheApp->GetTopWindow())->GetClientRect(); // Use for mdi parent/child version
...
/*
		int numberChars = sprintf ((char*)&gTextString3,
														" SUtility:GetWindowClipRectangle (windowPtr, gActiveImageViewCPtr, m_fame): %d, %d %d%s", 
														windowPtr,
		...
		numberChars = sprintf ((char*)&gTextString3,
														" SUtility:GetWindowClipRectangle (framerect): %d, %d, %d, %d%s", 
														frameArea.GetLeft(),
														frameArea.GetRight(),
														frameArea.GetTop(),
														frameArea.GetBottom(),
		...
		numberChars = sprintf ((char*)&gTextString3,
														" SUtility:GetWindowClipRectangle (width, height): %d, %d%s", 
														width,
														height,
														gEndOfLine);
		ListString ((char*)&gTextString3, numberChars, gOutputTextH);
...
/*
		numberChars = sprintf ((char*)&gTextString3,
														" SUtility:GetWindowClipRectangle (coordinate bottom, legend right): %d, %d%s", 
														coordinateBarHeight,
														legendWidth,
														gEndOfLine);
		ListString ((char*)&gTextString3, numberChars, gOutputTextH);
...
//	Revised By:			Larry L. Biehl			Date: 10/15/2015	
...
{  
	...
	returnFlag = GetSelectionRectangle (
											imageWindowPtr,
...
void RemoveSuffix (
				...
				SInt16									charFormatStringCode)

{
				// Remove some standard suffixes to be ready for a new suffix.		
				
	if (charFormatStringCode == kUnicodeCharString)
		{
	...
	else // not wide character format
		{
		UCharPtr		asciiFileNamePtr = (UCharPtr)fileNamePtr;
		
		RemoveCharsNoCase ((char*)"\0.asc\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.bag\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.grb\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.LAN\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.GIS\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.IMG\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.ige\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.DAT\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.cluster\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.CLU\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.RAW\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.bil\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.bsq\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.bip\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.fst\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.cal\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.drk\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.hdf\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.nc\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.I1\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.I2\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.I3\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.I4\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.I5\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.I6\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.I7\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.I8\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.I9\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.dd\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.h5\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.jp2\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.jpg\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.png\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.tiff\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.tif\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.prj\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.lbl\0", asciiFileNamePtr);
		RemoveCharsNoCase ((char*)"\0.project\0", asciiFileNamePtr);
      RemoveCharsNoCase ((char*)"\0.STI\0", asciiFileNamePtr);
		
		}		// end "else not wide character format"
...

{
	...
	if (gImageWindowInfoPtr->projectBaseImageFlag)	
		...
		if ( (gImageFileInfoPtr->format == kTIFFType || 
					...
					gImageFileInfoPtr->startColumn == 1 && gImageFileInfoPtr->startLine == 1)
			areaDescriptionPtr->applyOffsetFlag = FALSE;
...
//
//	Software purpose:	This routine sets up the global variables that
//							are used to convert between line-column units
//							and window units.
//
//	Parameters in:				
//
...
{	
	...
	if (windowInfoHandle != NULL)
		...
		if (displaySpecsPtr != NULL && displaySpecsPtr->displayedLineStart > 0)
			{
...
//					gSideBySideChannels = gStartChannel + 1;
				...
				if (windowCode == kToClipboardWindow || windowCode == kToPrintWindow)
					...
					if (GetSelectionOffscreenRectangle (GetWindowPtr(windowInfoHandle), 
						...
						if (offScreenRectangle.right - offScreenRectangle.left > 1 ||
							...
							numberChannels = (offScreenRectangle.right - offScreenRectangle.left)/
															displaySpecsPtr->offscreenChannelWidth;
			...
			gChannelWindowInterval = (SInt32)(displaySpecsPtr->offscreenChannelWidth * 
																							magnification);
			...
			}		// end "if (displaySpecsPtr != NULL && ...->displayedLineStart > 0)" 
...
{ 
	...
	planarMapUnitsCode = GetFilePlanarMapUnitsCode (windowInfoHandle);
		...
		CComboBox* comboBoxPtr = 
					(CComboBox*)dialogPtr->GetDlgItem(IDC_AreaUnitsCombo);
													...
													
{
	...
	#if defined multispec_win || defined multispec_lin
//		double magnification = gActiveImageViewCPtr->m_displayMultiCPtr->GetMagnification()    
