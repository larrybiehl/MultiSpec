...

{
	...
	identityFlag = FALSE;
	if ( mapMatrixPtr->map[0][0] == 1 && mapMatrixPtr->map[1][1] == 1 &&
					mapMatrixPtr->map[0][1] == 0 && mapMatrixPtr->map[1][0] == 0 )
		identityFlag = TRUE;
...

{
	...
	DrawPopUpMenuItem (dialogPtr, itemNumber, 
						gPopUpResampleMenu, gResampleSelection, TRUE);
...
//
//	Software purpose:	This routine determines the mapping matrix
//							based on the input user parameters.
//
//	Parameters in:					
//
//	Parameters out:				
...

{
	...
	if (DetermineIfIdentityMatrix(mapMatrixPtr) && 
					mapMatrixPtr->map[2][0] == 0 && mapMatrixPtr->map[2][1] == 0)
...
{  
	...
	if (rectifyImageOptionsPtr->procedureCode == kReprojectToReferenceImage)
		...
		if (continueFlag)	
			continueFlag = ListString( 
						(char*)&gEndOfLine,  strlen((char*)&gEndOfLine), gOutputTextH);
	...
	fileNamePtr = (char*)GetFileNameCPointer (outFileInfoPtr);
	sprintf( (char*)&gTextString,
					...
					while (channelCount<numberOutChannels)
						...
						if (!rectifyChannelPtr[channelCount])
							...
							if (fileInfoPtr->bandInterleave != kBIS)
								...
								{
								ioIn1ByteBufferPtr = (unsigned char*)gOutputBufferPtr;
								ioIn1ByteBufferPtr = &ioIn1ByteBufferPtr[channelCount*numberBytes];
...
//											savedSwapBytesFlag;
	...
	if (continueFlag)
		...
		reformatOptionsPtr->numberChannels = 
			...
			// Get a block of memory to use for the reformatting rectification	
			// options. 																			
	...
	if (continueFlag)
		...
		if (RectifyImageDialog (gImageFileInfoPtr, 
			...
			int numberChars = sprintf ((char*)&gTextString3,
												" SRectification:RectifyImageControl (continueFlag, procedureCode): %d, %d%s", 
												continueFlag,
...
//					reformatOptionsPtr->lineEnd = gImageFileInfoPtr->numberLines;
			...
			if (continueFlag)
				...
				if (gStatusDialogPtr)
					...
					LoadDItemString (gStatusDialogPtr, IDC_ShortStatusText, &gTextString);
					...
					ShowStatusDialogWindow (kShortStatusInfoID);
							...
							// file swap bytes flag to FALSE.  The bytes for the 		
							// input and output files will stay the same for 			
							// rectification.		Changed on 11/8/2005												
								
//					savedSwapBytesFlag = gImageFileInfoPtr->swapBytesFlag;
//					gImageFileInfoPtr->swapBytesFlag = FALSE;
...
//	SignedByte							handleStatus;
	...
	SetDialogItemDrawRoutine (
							dialogPtr, 17, &headerBox, drawHeaderOptionsPopUpPtr);
...
//					dialogSelectArea.imageWindowInfoPtr = NULL;
				...
				case 25:				//	 line shift  
					if (labs(theNum) > kMaxNumberLines - gImageWindowInfoPtr->maxNumberLines)
				...
				case 26:				//	 column shift  
					if (labs(theNum) > kMaxNumberLines - gImageWindowInfoPtr->maxNumberColumns)
				...
				case 33:				//	 Channels  	
					...
					if (itemHit == 2)
						...
						ChannelsDialog (
								(short int*)&rectifyImageOptionsPtr->numberChannelsToRectify,
								rectifyChannelPtr,
								gImageLayerInfoPtr,
								fileInfoPtr,
								1,
...
//	WindowPtr							windowPtr;
	...
	*blankOutsideSelectedAreaFlagPtr = rectifyImageOptionsPtr->blankOutsideSelectedAreaFlag;
	...
	*mapOrientationAnglePtr = GetMapOrientationAngle (fileInfoPtr->mapProjectionHandle) *
																								kRadiansToDegrees;
...
//	Revised By:			Larry L. Biehl			Date: 07/13/2015	
...
/*			
			...
			// represent that to be used that is selected in the active image window
			...
			if (procedureCode == kTranslateScaleRotate)
				{
	     		if (mapProjectionInfoPtr->planarCoordinate.mapOrientationAngle == 0 &&
	     																				rotationAngle == 0)
		     		{
		     				// Take care of the area being a selected portion of the entire image.
		     				// Note that the column/line shift is relative to the new pixels size.
					...
					leftEdgeOfPixel11 = mapProjectionInfoPtr->planarCoordinate.xMapCoordinate11 -
										mapProjectionInfoPtr->planarCoordinate.horizontalPixelSize/2;
										
					topEdgeOfPixel11 = mapProjectionInfoPtr->planarCoordinate.yMapCoordinate11 +
					...
					mapProjectionInfoPtr->planarCoordinate.verticalPixelSize /= 
							...
							// scale change.  We are assuming that any translation of the pixels
							// are relative to the fixed map coordinates for the image.  
...
//					else		// ...->planarCoordinate.polynomialOrder == 0 || ...
//						{
			     				// Determine the input line/column needed for the upper left pixel in
			     				// the new image to be created taking the image rotation angle into 
			     				// account. 
						...
						mapProjectionInfoPtr->planarCoordinate.mapOrientationAngle -= rotationAngle;
...
//											inputLine,
	...
	referenceMapProjectionInfoPtr = (MapProjectionInfoPtr)GetHandlePointer(
												referenceMapProjectionHandle, kLock, kNoMoveHi);
		...
		if (columnStart <= maxNumberInputColumns && 
			...
			if (errCode == noErr)
				...
				for (index=0; index<fileInfoPtr->maxClassNumberValue; index++)
					...
					}		// end "for (index=0; index<fileInfoPtr->maxClassNumberValue; index++)"
...
//			DisableMenuItem (popUpResampleSelectionMenu, kMean);
	...
	#if defined multispec_win
		CComboBox* comboBoxPtr = (CComboBox*)(dialogPtr->GetDlgItem(IDC_ResampleMethod));
	...
	#if defined multispec_lin
		wxComboBox* comboBoxPtr = (wxComboBox*)wxWindow::FindWindowById(IDC_ResampleMethod);
