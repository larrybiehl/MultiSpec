...
//
//	Software purpose:	The purpose of this routine is to indicate that the pointers to the
// 						control point vectors may not be valid any more by setting them
//							to NULL.
//
//	Parameters in:				
//
...

{
	...
	FileInfoPtr fileInfoPtr = (FileInfoPtr)GetHandlePointer ( 
													imageFileInfoHandle, kNoLock, kNoMoveHi);
...

{
	...
	if (fileInfoHandle != NULL)
		...
		if (inputPtrFlag)
			...
			fileInfoPtr = (FileInfoPtr)
				GetHandleStatusAndPointer (fileInfoHandle, &handleStatus, kNoMoveHi);
		...
		#if defined multispec_lin
			fileInfoPtr->fileStreamCPtr = InitializeFileStream(fileInfoPtr->fileStreamCPtr);
...

{
	...
	hfaPtr = (HierarchalFileFormatPtr)
				GetHandleStatusAndPointer (hfaHandle, &handleStatus, kNoMoveHi);
...
//	Revised By:			Larry L. Biehl			Date: 01/20/2013
