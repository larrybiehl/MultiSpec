	...
	
{
	...
	if (histogramSpecsPtr->computeFlag)
		...
		channelOffset = 0; 						
		channelListPtr = (UInt16*)GetHandlePointer (
										histogramSpecsPtr->channelsHandle, kNoLock, kNoMoveHi);
		localChannelListPtr = 
							&channelListPtr[gImageWindowInfoPtr->totalNumberChannels];
		...
		for (fileInfoIndex=fileInfoIndexStart; 
			...
			if (localFileInfoPtr->bandInterleave == kBIS)
				{
//				interval = (SInt32)localFileInfoPtr->numberChannels;
//				numberSamples *= localFileInfoPtr->numberChannels;
			...
			if (localFileInfoPtr->numberBytes <= 2)
				...
				{
				DetermineMinAndMaxValuesForEachChannel (
...
//								}		// end "if(dataValue >= maxSaturatedValue)" 
						...
						for (index=0; index<numberSamples; index+=interval)
							...
							if (computeCode & 0x0002)
								...
								if (binType == kDataValueIsBinIndex)
									...
									else		// doubleDataValue >= minNonSatValue && doubleDataValue <= ...
										{
										if (binType == kBinWidthOfOne)
					...
					if (computeCode & 0x0002)	
//						medianArrayChanPtr += histogramSummaryPtr[chanIndex].numberBins;
...
/*					else if (histogramSummaryPtr[chanIndex].minNonSatValue == 
						...
						if (histogramSummaryPtr[chanIndex].binType != kBinWidthNotOne)
							...
							}		// end "if (histogramSummaryPtr[chanIndex].binType != kBinWidthNotOne)"
...
//																localFileInfoPtr->dataTypeCode);
					...
					GetNumberDecimalDigits (localFileInfoPtr->dataTypeCode,
													histogramSummaryPtr[chanIndex].minValue,
													histogramSummaryPtr[chanIndex].maxValue,
...
//					savedHistogramArrayPtr += histogramSummaryPtr[chanIndex].numberBins;
...
//		channelOffsetCount = gImageWindowInfoPtr->numberBins;
		...
		for (channel=channelIndex; 
			...
			theChar = (char)(histogramSummaryPtr[channel].maxValue + 
																			signedValueOffset);
			BlockMoveData (&theChar, &ioChanBufferPtr[8], 1);
	
					// Write minimum value for band.											
	
			theChar = (char)(histogramSummaryPtr[channel].minValue + 
																			signedValueOffset);
			...
			tempLongInt = ConvertRealAT ( 
					(double)(histogramSummaryPtr[channel].averageValue + 
																			signedValueOffset) );
			...
			tempLongInt = ConvertRealAT (
				(double)(histogramSummaryPtr[channel].medianValue + 
																			signedValueOffset) );
			...
			tempUInt16 = (UInt16)(histogramSummaryPtr[channel].maxValue + 
																			signedValueOffset);
      	tempInt = GetShortIntValue((char*)&tempUInt16);
			...
			tempUInt16 = (UInt16)(histogramSummaryPtr[channel].minValue + 
																			signedValueOffset);
      	tempInt = GetShortIntValue((char*)&tempUInt16);
...
//				compaction = (histogramSummaryPtr[channel].numberBins - 2 +
//											254)/254;
...
//				medianArrayPtr += histogramSummaryPtr[channel].numberBins;
...
//	maxImageIndexValue = gImageWindowInfoPtr->numberBins - 1;
...
//				compactionFactor = (UInt32)GetLongIntValue (&statBufferPtr[72]);
					...
					if (compactionFactor <= 1)
						{
								...
								// the data values are just loaded in order.  Force compactionFactor
								// to be one.
						...
						while (binIndex <= endIndex)
							{
							if (compactionCount >= (SInt32)nextBinForSTABinRead && countHistogramFileVectorReads < 255)
								{
								*histogramArrayPtr = totalCount;
								...
								numberBinsPerInputDataValue = (SInt32)(nextBinForSTABinRead-compactionCount);
						...
						// If the count left has not been used (which is in totalCount), add it to
						// the last bin used.
			...
			SetNumberOfMinimumDataValuesInArray (
													&histogramSummaryPtr[channel],
													savedHistogramArrayPtr,
													signedValueOffset);
			...
			SetNumberOfMaximumDataValuesInArray (
													&histogramSummaryPtr[channel],
													savedHistogramArrayPtr,
													signedValueOffset);
				
//			savedHistogramArrayPtr += histogramSummaryPtr[channel].numberBins;
			savedHistogramArrayPtr += histogramSpecsPtr->maxNumberBins;
...

{
	...
	if (textWindowFlag)
		{
//		bytesNeeded += numberLines * sizeof(LineRec);
...

{
	...
	for (loop=0; loop<numberLoops; loop++)
		...
		if (continueFlag && loop == 0)	
			continueFlag = ListHistogramTitle(histogramSpecsPtr,
	...
	if (continueFlag)
		...
		continueFlag = ListHistogramSummary (
											gImageFileInfoPtr, 
											histogramSpecsPtr,
											resultsFileStreamPtr);
...

{
	...
	DrawPopUpMenuItem (dialogPtr, itemNumber, gPopUpHistogramMethodMenu, 
																	gHistogramMethod, TRUE );
...

{
	...
	continueFlag = GetClippedMinMaxValueIndices (
												histogramSpecsPtr,
					 							percentTailsClipped,
				 								channel, 
								 				numberDataValues, 
...

{
	...
	if (windowInfoPtr != NULL)
      ...
      #if defined multispec_win | defined multispec_lin		
			handlePtr = 
				&gActiveImageViewCPtr->m_histogramCPtr->mHistogramSpecsHandle;	 
		#endif	// defined multispec_win 
...

{
	...
	*histogramArrayHPtr = UnlockAndDispose (*histogramArrayHPtr);
	...
	histogramArrayPtr = (HUInt32Ptr)CheckHandleSize (
												histogramArrayHPtr,
												&continueFlag, 
												&changedFlag, 
												numberBytes);
...
//		fileStreamPtr = GetFileStreamPointer (gImageFileInfoPtr);
		...
		continueFlag = ReadSTASupportFile (
								histogramSpecsPtr->statBufferPtr,
								supportFileStreamPtr,
								channelListPtr, 
								numberChannels, 
		...
		if (continueFlag)	
			continueFlag = DecodeSTASupportFile (
						histogramSpecsPtr,
						histogramSpecsPtr->histogramArrayPtr,
						supportFileStreamPtr,
						channelListPtr, 
...

{
	...
	continuousFlag = 
				DetermineIfContinuousChannels (channelListPtr, *numberChannelsPtr);
	...
	if (itemHit == kReadFromSupportFile)
		...
		if ( GetSTASupportFile (fileInfoPtr) )
			...
			if (CheckIfDefaultHistogramInfoExists (windowInfoPtr) ||
                ...
                methodCode = (SInt16) ((CMHistogramSpecsDlg *)dialogPtr)->GetItemCode(comboBoxPtr->GetString(0));
                if (methodCode != kStoredInMemory) {
...

{
	...
	if (dialogString[0] == 0)
		{
		imageFileNamePtr = (FileStringPtr)GetFileNamePPointer (fileInfoPtr);
		CopyPToP (dialogString, imageFileNamePtr);

		}		// end "if (dialogString[0] == 0)"

#	if defined multispec_mac
		if (CreateUnicodeStaticTextControl (dialogPtr, &dialogString[1], dialogString[0], 30, NULL) != noErr)
			LoadDItemString (dialogPtr, 30, (Str255*)&dialogString);
	#	endif	// defined multispec_mac
	
#	if defined multispec_win
		WideFileStringPtr wFileNamePtr = 
					(WideFileStringPtr)GetFileNameCPointer (fileInfoPtr, kReturnUnicode);
...
//		HideDialogItems (dialogPtr, 23, 29);
		HistogramDialogHideListItems (dialogPtr);
		...
		if (listHistogramFlag)
			{
//			ShowDialogItems (dialogPtr, 23, 29);
...
//				fileStreamPtr = GetFileStreamPointer (fileInfoPtr);
			...
			fileNamePtr = (FileStringPtr)GetFileNamePPointer (fileStreamPtr);
			...
			MultiByteToWideChar (CP_UTF8, 0, (LPCSTR)&fileNamePtr[1], -1, wideFileName, sizeNeeded);
...
//											theNum;
	...
	if (numberBytesNeeded >= longestContBlock + 50000)
      ...
      #if defined multispec_win | defined multispec_lin
//			ShowHideDialogItem(dialogPtr, IDC_ColumnLineFormat, m_listHistogramValues); 
			HideDialogItem(dialogPtr, IDC_Lines); 
//			DDX_CBIndex(dialogToPtr, IDC_ColumnLineFormat, m_columnsLinesFormat);
		#endif	// defined multispec_win
	...
	for (channelIndex=startChannelIndex; 
		...
		if (!oneTitleLinesForAllChannelsFlag || startChannelIndex == 0)
			...
			if (lineFormatHistFlag)
				...
				numberBytes = (SInt32)channelTextWidth+1 + minValueCountTextWidth+1 + maxValueCountTextWidth+1 +
																		value * (valueCountTextWidth+1) + 128;
				...
				numberBytes = (SInt32)valueCountTextWidth+1 + value * (countTextWidth+1) + 128;
			...
			gCharBufferPtr1 = (HPtr)GetHandlePointer (
													gCharBufferHandle1, kLock, kNoMoveHi);
...
//																0,
		...
		if (continueFlag)
			...
			if (lineFormatHistFlag)
				...
				continueFlag = ListHistogramValuesInLines(
											histogramSpecsPtr,
											resultsFileStreamPtr,
											fileInfoPtr,
											forLineMinValue, 
...

{
	...
	while (m < numberBins) 
		...
		if (countFlag > 0)
			...
			medianArrayIndex = 0;
			for (channelIndex=startChannelIndex; channelIndex<=lastChannelIndex;	channelIndex++)
				{
						// Set up channel pointer for median array						
...
//			channelOffsetCount += histogramSummaryPtr[channel].numberBins;
			channelOffsetCount += histogramSpecsPtr->maxNumberBins;
...

{
	...
	UInt32								columnInterval,
			...
			// Use an line interval such that no more than 3 million pixels
			// (lines x columns x number channels) will be used. Do not
			// allow less than 10,000 pixels per channel.
			
	maxNumberChannelPixels = MAX(3000000, numberChannels * 10000);
...
{ 
	...
	if (FileExists(windowInfoPtr->supportFileStreamPtr))
		{
		supportFilePathPtr = (FileStringPtr)GetFilePathPPointer (windowInfoPtr->supportFileStreamPtr);
