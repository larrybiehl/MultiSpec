...
/*	
		...
		if (errCode == noErr && ioParamPtr->ioBuffer[ioParamPtr->ioActCount-1] != 0x0d)
			...
			if (errCode == noErr && ioParamPtr->ioBuffer[ioParamPtr->ioActCount-1] != 0x0a)
...
{								
	...
	if (inputFileInfoHandle != NULL)
		...
		while (!doneFlag)
	    	...
	    	if (fileFlag)
	    		{
						// Determine whether an image file which may have group info at the end
						// of the file or a .clr file.
		
				fileNamePtr = (FileStringPtr)GetFilePathPPointer(fileStreamPtr);
				...
				if (CompareSuffixNoCase ((char*)"\0.clr\0", fileNamePtr, NULL))
					...
					fileInfoLoadedFlag = LoadImageInformation (NULL, inputFileInfoHandle, FALSE, NULL, 0);
					...
					if (fileInfoLoadedFlag)
						{
						if (inputFileInfoPtr->numberClasses == outputFileInfoPtr->numberClasses)
							doneFlag = TRUE;
							
						else		// inputFileInfoPtr->numberClasses != outputFileInfoPtr->numberClasses
...
//
//	Software purpose:	This routine finds the start of the next line
//							in the project file. If there is no next line
//							in the buffer, then a buffer is read from the file.
//							Note that for the Mac a new line is read each time.
//							For Windows, we have to check for end of line
//							characters.
//
//	Parameters in:					
//
...
//							ReadCovarianceInformation in SProjFIO.cpp
//							ReadModifiedStats in SProjFIO.cpp
//							ReadProjectFile in SProjFIO.cpp
//							ReadOffsetGainFile in SSAveWrt.cpp
//
	   ...
	   UInt32					count,
		...
		if (lastLinePtr != NULL && 
			...
			if (nextLinePtr == NULL)
				...
				if (newLinePtr != NULL)
					{
					numberCharsToMove = paramBlockPtr->ioActCount - 
	...
	if (supportFileStreamPtr != NULL)
		...
		do
			...
			if (continueFlag)
				...
				supportFileNamePtr = (FileStringPtr)GetFilePathPPointer(supportFileStreamPtr);
...
//			geographicTypeGeoKey = 4035;		// GCSE_Sphere
		...
		else if (mapProjectionInfoPtr->geodetic.datumCode == kNewZealandGeodeticDatum1949Code)
		...
		else if (mapProjectionInfoPtr->geodetic.datumCode == kGeocentricDatumAustralia1994Code)
			...
			// If GeographicTypeGeoKey is user defined, then define the Geographic Geodetic Datum
...
//			geogCitationGeoKeyFlag = TRUE;
	...
	if ((projectedCSTypeGeoKey == 0 || projectedCSTypeGeoKey == 32767) && 
		...
		else if (mapProjectionInfoPtr->geodetic.spheroidCode == kAustralianEllipsoidCode)
			geogEllipsoidGeoKey = 7003;	
		else if (mapProjectionInfoPtr->geodetic.spheroidCode == kBesselEllipsoidCode)
			geogEllipsoidGeoKey = 7004;
		else if (mapProjectionInfoPtr->geodetic.spheroidCode == kClarke1866EllipsoidCode)
			geogEllipsoidGeoKey = 7008;
		else if (mapProjectionInfoPtr->geodetic.spheroidCode == kClarke1880EllipsoidCode)
		...
		else if (mapProjectionInfoPtr->geodetic.spheroidCode == kInternational1909EllipsoidCode)
			geogEllipsoidGeoKey = 7022;
		else if (mapProjectionInfoPtr->geodetic.spheroidCode == kKrassovskyEllipsoidCode)
...
//			else		// mapProjectionInfoPtr->geodetic.semiMajorAxis <= 0
		...
		else if (mapProjectionInfoPtr->geodetic.spheroidCode == kGRS1967ModifiedEllipsoidCode)
	...
	projectionGeoKey = 0;
	if (projectedCSTypeGeoKey == 32767 && mapProjectionInfoPtr->gridCoordinate.projectionCode != 0)
		projectionGeoKey = 32767;
		
			// If Projected CS Type Geo Key not found, then check for Projected Coordinate
			//	Transformation Geo Key.
	...
	if (projectedCSTypeGeoKey == 0 || projectedCSTypeGeoKey == 32767)
		...
		switch (mapProjectionInfoPtr->gridCoordinate.projectionCode)
			...
			case kCylindricalEqualAreaCode:
				...
				break;
			case kKrovakCode:
						// This seems to be special case. I have more to sort out on this "projection".
				if (projectedCSTypeGeoKey == 0)
...
*/
//				The following was for debugging a problem with gdal in April 2012. Will take out when problem is completely resolved.
//	count = sprintf (string, 
//								"IMAGINE GeoTIFF Support.\nERDAS Desktop 2011 Version 11.0.3 11.0.0.896.\nProjection Name = State Plane.\nUnits = us_survey_feet.\nGeoTIFF Units = us_survey_feet|");
//								"IMAGINE GeoTIFF Support.\nERDAS Desktop 2011 Version 11.0.3 11.0.0.896.\nProjection Name = State Plane.\nUnits = us_survey_feet.|");
	...
	if (projectedCSTypeGeoKey <= 0 || projectedCSTypeGeoKey >= 32767)
		...
		if (projFalseOriginEastingGeoKey != -9999)
			...
			if (bufferPtr != NULL && doubleParametersBufferPtr != NULL)
				...
				tempDoubleValue = GetDoubleValue ((UCharPtr)&projFalseOriginEastingGeoKey);
				BlockMoveData (&tempDoubleValue, 
										&doubleParametersBufferPtr[doubleParameterBuffIndex], 
										8);
		...
		if (projFalseOriginNorthingGeoKey != -9999)
			...
			if (bufferPtr != NULL && doubleParametersBufferPtr != NULL)
				...
				tempDoubleValue = GetDoubleValue ((UCharPtr)&projFalseOriginNorthingGeoKey);
				BlockMoveData (&tempDoubleValue, 
										&doubleParametersBufferPtr[doubleParameterBuffIndex], 
										8);
		...
		if (projStraightVertPoleLongGeoKey != -9999)
			...
			if (bufferPtr != NULL && doubleParametersBufferPtr != NULL)
				...
				tempDoubleValue = GetDoubleValue ((UCharPtr)&projStraightVertPoleLongGeoKey);
				BlockMoveData (&tempDoubleValue, 
										&doubleParametersBufferPtr[doubleParameterBuffIndex], 
										8);
...
//							classes info from the requested trailer file. 
//
//	Parameters in:		None
//
//	Parameters out:	None
...
{  
	...
	if (supportFileSelectedFlag)
		...
		if (classNamesExistFlag)
			...
			FileStringPtr infileNamePtr = (FileStringPtr)GetFileNameCPointer (supportFileStreamPtr);
...

{
	...
	if (continueFlag)
		...
		if (ReadThematicGroups (
			...
			FileStringPtr outfileNamePtr = (FileStringPtr)GetFileNameCPointer (outputFileInfoPtr);
			FileStringPtr infileNamePtr = (FileStringPtr)GetFileNameCPointer (inputFileInfoPtr);
...

{
	...
	fileInfoPtr = (FileInfoPtr)GetHandlePointer(fileInfoHandle, kLock, kNoMoveHi);
...
{  
	...
	for (classNumber=1; classNumber<=maxClass; classNumber++)
		...
		if (backgroundFlag)
			blueVectorPtr[vectorIndex] = greenVectorPtr[vectorIndex] = redVectorPtr[vectorIndex] = 65535;
		
		else		// !backgroundFlag 
			{
			if (colorSpecPtr == NULL)
				{
				greenVectorPtr[vectorIndex] = vectorIndex * 256;
...

{
			...
			if (!fileFoundFlag)
				...
				if (errCode >= 1001 && errCode <= 1003)
					...
					if (LoadSpecifiedStringNumberLongP (
														...
														versionNumber,
														0))
						errCode = DisplayAlert (kErrorAlertID, kStopAlert, 0, 0, 0, gTextString);
	...
	if (fileFoundFlag)
		...
		char* fileNamePtr = (char*)GetFileNamePPointer (fileStreamPtr);
...
//							if (eigenvectorValueSize == 8 ||
//									(eigenvectorValueSize == k68881Bytes &&
//													eigenvalueValueSize == 8))
							{
							doublePtr = (double*)charPtr;
							doubleValue = *doublePtr;
								
							}		// end "if (eigenvectorValueSize == 8)"
...
{    
	...
	if (SetUpActiveImageInformationGlobals ( 
		...
		if (continueFlag)
			...
			if (continueFlag)
				{
				char* fileNamePtr = (char*)GetFileNameCPointer (fileStreamPtr);
			...
			if (continueFlag && DetermineIfInverseLatLongPossible (windowInfoHandle))
				{
/*				GetCopyOfPFileName (fileStreamPtr, savedImageFileName);
				...
				ConcatFilenameSuffix (filePathPtr, (StringPtr)"\0.kml\0");
							...
							// Force the uniFileName to be recreated to match the kml file name.
...
//
//	Software purpose:	The purpose of this routine is to save the text
//							in the text output window to a disk file to be 
//							named by the user.
//
...

{
	...
	if (fileStreamPtr != NULL)
		...
		if (!abortedFlag)
			...
			abortedFlag = !WriteTextOutputFile (
...
//
//	Software purpose:	This routine finds the next carriage return in
//							the input string and returns the location just
...

{
	...
	if (fileInfoPtr != NULL)		
		...
		if (planarCoordinateInfoFlag)
			...
			blwFileNamePtr = (FileStringPtr)GetFilePathPPointer(blwStreamPtr);
...

{
	...
	if (fileInfoPtr != NULL)		
		...
		imageFileNamePtr = (FileStringPtr)GetFilePathPPointer(fileStreamPtr);
		headerFileNamePtr = (FileStringPtr)GetFilePathPPointer(headerStreamPtr);
...
//										fileInfoPtr->numberPostLineBytes,
		...
		if (planarCoordinateInfoFlag)
			...
			SInt16 referenceSystemCode = mapProjectionInfoPtr->gridCoordinate.referenceSystemCode;
			...
			if (errCode == noErr && 
						(UTMFlag || referenceSystemCode == kStatePlaneNAD27RSCode ||
								referenceSystemCode == kStatePlaneNAD83RSCode ||projectionCode == kAlbersConicalEqualAreaCode))
				{
	...
	*numberGeoEntriesPtr = 0;
	...
	if (fileInfoPtr != NULL)		
		...
		if (mapProjectionInfoPtr != NULL)
			...
			if (mapProjectionInfoPtr->gridCoordinate.projectionCode > 0 || *numberGeoEntriesPtr > 0)
				{
			...
			if (fileInfoPtr->noDataValueFlag)
				...
				if (!countOnlyFlag)
					...
					if (numberGDALNoDataValueDigits > 4)
						...
						BlockMoveData ((char*)&gTextString, &tiffValue, numberGDALNoDataValueDigits);
			...
			if (!countOnlyFlag)
				...
				if (d34735Flag)
					{
					LoadGeoKeyDirectory (fileInfoPtr,
													mapProjectionInfoPtr,
													&geoTiffInfoBuffer[index],
													&geoTiffInfoBuffer[index+numberGeoKeyDirectoryBytes],
													NULL,
													&geoTiffInfoBuffer[index+numberGeoKeyDirectoryBytes+numberGeoDoubleParameterBytes],
													NULL);
					index += numberGeoKeyDirectoryBytes + numberGeoDoubleParameterBytes + numberGeoASCIIParameterBytes;
...
//									"UTM Zone 16N NAD27\|Clarke, 1866 by Default|");
				...
				if (d42113Flag)
					BlockMoveData ((char*)&gTextString, (char*)&geoTiffInfoBuffer[index], numberGDALNoDataValueDigits);
...
#		endif		
#		if defined multispec_win
					// Force the unicode file name to be recreated to match the kml file name.
...
#		endif		
		...
		if (continueFlag)
			...
			maxMapSize = MIN(fabs(rightMapEdge-leftMapEdge), fabs(topMapEdge-bottomMapEdge));
//			if (mapProjectionInfoPtr->planarCoordinate.mapUnitsCode == kMetersCode)
				range = 8 * maxMapSize;
...
//							the parameters and write the header record to
//							the file.
//
//	Parameters in:					
//
			...
			switch (outFileInfoPtr->format)
				...
				case kGAIAType:		
					...
					if (gProcessorCode != kMultispecToThematicProcessor &&
						...
						{	
							continueFlag = CopyToNewGAIAHeader (
																			inFileInfoPtr, 
																			outFileInfoPtr, 
																			headerRecordPtr, 
																			lineInterval, 
				...
				case kGeoTIFFType:
					...
					writePosOff = 0;
							...
							// file if the header was not original written by MultiSpec.  It
							// it pretty easy to use tags in different ways between applications.
...

{
	...
	#else	
		#if use_mlte_for_text_window
			IOResult = TXNGetDataEncoded (textObject, beginText, endText, &hText, kTXNTextData);
		#endif	
		#if !use_mlte_for_text_window	
	...
	textPtr = (char*)GetHandlePointer (hText, kLock, kNoMoveHi);
...
#			endif
		...
		CMFileStream* outputFileStreamPtr = GetSupportFileStreamPointer (
																			windowInfoHandle,
																			&handleStatus);
		char* outfileNamePtr = (char*)GetFileNameCPointer (outputFileStreamPtr);
...
{     
	...
	if (fileInfoPtr->bandInterleave > kBIS)
			...
			// Do not allow group information to be written to image files
			// that are in Imagine, TIFF, GeoTIFF, HDF and netCDF formats, since we do not
			// know for sure where the end of the file is based on the parameters 
	...
	if (continueFlag)
		...
		groupNamePtr = (char*)GetHandlePointer(
										fileInfoPtr->groupNameHandle, kNoLock, kNoMoveHi);
		...
		if (errCode == noErr)
			{
			errCode = MSetMarker (fileStreamPtr, fsFromStart, sizeImageFile, kErrorMessages);
	...
	if (continueFlag)
		...
		count = gTextString[0];
		errCode = MWriteData (fileStreamPtr, &count, (char*)&gTextString[1], kErrorMessages);
...
//
//	Software purpose:	The purpose of this routine is to write the thematic
//							classes or group info to the requested trailer file. 
//
//	Parameters in:		None
//
//	Parameters out:	None
...
//														  1, 
			...
			if (paletteObject != NULL)
				...
				if (thematicListType == kClassDisplay)
					{
					if (paletteOffset == USHRT_MAX ||
										// Following will catch when a background class is added
										// during the reformatting step.
								(tiffSourceCode == kFromReformat && 
...
{     	
	...
	offScreenMapH = gImageWindowInfoPtr->offScreenMapHandle; 
	   ...
	   BITMAPINFOHEADER* bitMapInfoHeadPtr = (BITMAPINFOHEADER*)GetHandlePointer(
	   												offScreenMapH, kNoLock, kNoMoveHi);
	   ...
	   rowBytes = GetNumberPixRowBytes (bitMapInfoHeadPtr->biWidth, pixelSize);
		...
		UInt32		linuxIncrement = 1;
				...
				// In linux the offscreenbuffer always has 3 bytes for multispectral image windows and
				// 1 byte for thematic image windows.
				
		if (!gImageFileInfoPtr->thematicType)
			linuxIncrement = 3;

		DisplaySpecsPtr displaySpecsPtr = (DisplaySpecsPtr) GetHandlePointer(gActiveImageViewCPtr->GetDisplaySpecsHandle(), kLock, kNoMoveHi);
		pixelSize = displaySpecsPtr->pixelSize;
		int numberColumns = ((displaySpecsPtr->columnEnd - displaySpecsPtr->columnStart) / displaySpecsPtr->columnInterval) + 1;
	...
	if (pixelSize == 32)
		...
		if (gOSXCoreGraphicsFlag)
			offScreenBufferPtr = (HUCharPtr)GetHandlePointer(
							gImageWindowInfoPtr->imageBaseAddressH, kNoLock, kNoMoveHi);
		...
		offScreenBufferPtr = 
					(HUCharPtr)GetHandlePointer(
							gImageWindowInfoPtr->imageBaseAddressH, kLock, kNoMoveHi);  
...
////				paletteOffset = 1;
		...
		rowsPerStrip = GetNumberStripsToUse (
										planarConfiguration,
										numberColumns,
										numberLines,
										numberTIFFChannels,
		...
		textStringLength += LoadInstrumentNameToTIFFAsciiTag (fileInfoPtr, tiffSourceCode, &textString[textStringLength]);
		textString[textStringLength] = 0;
		textStringLength++;
		textStringStart = imageDataOffset;
		textStringBufferLength = ((textStringLength+3)/4) * 4;		// Force buffer for string to be multiple of 4
...
//										kErrorMessages);
	...
	if (errCode == noErr)
		...
		ptr = (HPtr)GetHandlePointer (gTransformationMatrix.eigenValueHandle,
												kLock,
												kNoMoveHi);
