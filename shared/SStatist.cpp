...
#			endif	// defined multispec_win || ...
         ...
         if (gStatisticsMode == kStatNewFieldRectMode)
            hiliteValue = 0;
         MHiliteControl(gProjectWindow, gProjectInfoPtr->editNameControlH, hiliteValue);
         break;
...
#			endif	// defined multispec_win
      ...
      case kCoordinateListMode: // "Coordinate List" mode (List of Polygon Points)
         ...
         // Show the controls that are part of the "Polygon Point List"	
         // mode 																	
...
#	endif
   ...
   // Unlock some project handles if needed so as not to fragment			
   // memory.																				
...
//							StatisticsWControlEvent SStatist.cpp
//
//	Coded By:			Larry L. Biehl			Date: 09/30/1988
				...
				UCharPtr								fieldIdentifierPtr) 
{
   ...
   if (OKClicked) 
      ...
      gProjectInfoPtr->fieldIdentPtr =
         (HPFieldIdentifiersPtr) CheckHandleSize(
      ...
      if (classNamesPtr[currentStorageClass].numberOfFields == 1) {
         classNamesPtr[currentStorageClass].firstFieldNumber =
            currentStorageField;
      ...
      if (gProjectWindow != NULL &&
         ...
         // Unhilite the "Add to List" control until the next field is	
         // selected																		
...
// Value Returned:	None
// 
// Called By:			AddFieldToProject in statistics.c
//
...
#	endif
   ...
   if (gProjectInfoPtr->lastPointType != kPolygonType) {
      ...
      MHiliteControl(gProjectWindow, gProjectInfoPtr->addToControlH, 255);
		...
		LSetCell(&(gTextString[1]), (short int) gTextString[0], cell, gStatisticsListHandle);
		...
		sprintf((char*) gTextString,
			...
			selectedLineCol.v,
			selectedLineCol.h);
		// index = (SInt16) gStatisticsListHandle->InsertString (index, (char*)gTextString);
...
//							GetDefaultClassName in SStatist.cpp
//							GetUniqueClassName in SStatist.cpp
//							NewClassFieldDialog in SStatist.cpp
//
				...
				Str255*								namePtr) 
{
   ...
   if (gProjectInfoPtr->numberStatisticsFields > 0) {
      ...
      classNamesPtr = (HPClassNamesPtr) GetHandleStatusAndPointer(
         gProjectInfoPtr->classNamesHandle, &handleStatus1, kNoMoveHi);

      fieldIdentPtr = (HPFieldIdentifiersPtr) GetHandleStatusAndPointer(
         gProjectInfoPtr->fieldIdentifiersHandle, &handleStatus2, kNoMoveHi);
      ...
      for (classIndex = 0;
         ...
         while (field > -1) {
            ...
            if (currentField != field) {
               ...
               //					if (!strncmp( (char*)&fieldIdentPtr[field].name,
               //																(char*)namePtr,
               //																numChars))
               //																						return(2);
...
void ClassListStatMode(void)
{
   ...
   InvalWindowRect(gProjectWindow, &gProjectInfoPtr->listTitleBox);
	...
	#if defined multispec_win
		(gProjectWindow->GetDlgItem(IDC_ListTitle))->SetWindowText((LPCTSTR)_T("Classes"));
   ...
   #if defined multispec_lin 
		wxStaticText* list_title = (wxStaticText*)gProjectWindow->GetFrame()->FindWindow(IDC_ListTitle);
		list_title->SetLabel(wxT("Classes"));

		(gProjectWindow->GetFrame()->FindWindow(IDC_ProjectWindowMode))->SetLabel(wxT("CLASS LIST"));

		((CMStatisticsView*)gProjectWindow)->m_frame->UpdateStatsTypeCombo(kClassListMode);
	...
	#if defined multispec_win || defined multispec_mac
...
void CloseProjectWindow(void)
{
   if (gProjectWindow != NULL)
	...
	#if defined multispec_win
      CMStatisticsDoc* statisticsDocPtr = ((CMStatisticsForm*) gProjectWindow)->GetDocument();
      CMStatisticsFrame* statisticsFrameCPtr = statisticsDocPtr->GetStatisticsFrame();
      statisticsFrameCPtr->MDIDestroy();
		...
		if (gProjectWindow != NULL)
			{
			CMStatisticsDoc* statisticsDocPtr = ((CMStatisticsView*)gProjectWindow)->GetDocument();
			CMStatisticsFrame* statisticsFrameCPtr = statisticsDocPtr->GetStatisticsFrame();  
...
//							StatisticsControl in SStatist.cpp
//
//	Coded By:			Larry L. Biehl			Date: 09/27/1988
   ...
   if (continueFlag) 
      ...
      InitializeWindowInfoStructure (windowInfoHandle,
         ...
         kStatisticsWindowType);
				...
				// Initialize items specific for project window information			
				// structure.																		
...
void DeletePolyPointStatNewFieldW(
   ...
   // Unhilite the "Add to List" control if there fewer than				
...
pascal void DrawOutlineColorPopUp(
   ...
   DrawPopUpMenuItem(dialogPtr, itemNumber,
      gPopUpTemporaryMenu, gOutlineColorSelection, true);
...
#		if defined multispec_win 
			(gProjectWindow->GetDlgItem(IDC_ListTitle))->SetWindowText((LPCTSTR)_T("Fields"));
...
SInt16 GetCurrentClassField(
   ...
   if (classNumber < (SInt16) gProjectInfoPtr->numberStatisticsClasses && classNumber >= 0) {
      if (inputFieldNumber >= 0 &&
         inputFieldNumber < gProjectInfoPtr->numberStatisticsFields) {
         // Convert the field number to the						
         // (classFieldNumber) within the class.												
...
#	if defined multispec_win
		char*		returnStringPtr = (char*)gTextString;
...
// Value Returned:	None			
// 
// Called By:			UpdateStatisticsWindow in SStatist.cpp
//
   ...
   do {
      ...
      /*									
            if (useProjectClassNameFlag && namePtr != NULL)
               ...
               BlockMoveData ( &namePtr[1], (Ptr)&outputStringPtr[stringLength+1], namePtr[0]);
...
#	if defined multispec_lin
		if (controlHandle != NULL)
			...
			((CMStatisticsView*) gProjectWindow)->GetFrame()->FindWindow(windowid)->Show(false);
			//((CMStatisticsView*) gProjectWindow)->GetFrame()->Layout();
...
void InvalPopUpCovarianceToUse(void) 
{
	if (gProjectWindow != NULL &&
		...
		#if defined multispec_lin
			((CMStatisticsView*)gProjectWindow)->m_frame->UpdateStatsTypeCombo (gProjectInfoPtr->statsWindowMode);
			wxComboBox* statsTypeComboPtr = (wxComboBox*)gProjectWindow->GetFrame()->FindWindow(IDC_StatsCombo);
...
#				if defined multispec_lin
					sprintf ((char*)gTextString,
									...
									line,
									column);
					cell.v = (SInt16) gStatisticsListHandle->Insert ((char*)gTextString, cell.v);               
#				endif	// defined multispec_lin 
...
//							LoadRectStatNewFieldW in SStatist.cpp
//
...
#	endif
   ...
   LSetCell(&(gTextString[1]), (SInt16) gTextString[0], cell, gStatisticsListHandle);
...
Boolean LOOCOptionsDialog(
   ...
   ShowDialogWindow(dialogPtr, kSetLOOCParameterSpecificationID, kSetUpDFilterTable);
...
void LOOCOptionsDialogOK(
   ...
   double userMixingParameter) {
   if (statsWindowMode == kClassListMode) {
      SetProjectMixingParameter(mixingParameterCode,
         userMixingParameter);
   ...
   do {
      ...
      if (itemHit > 2) {
         ...
         switch (itemHit) {
            ...
            case 10: // Display spectral classes/information groups 		
               // Popup menu. 												
...
// Value Returned:	None
//
// Called By:			NewClassFieldDialog in SStatist.cpp
//							CMNewClassFieldDlg::OnInitDialog in WNewClas.cpp
//
				...
				SInt64*								numberSelectionPixelsPtr)
{
   *selectedItemPtr = 0;
   ...
   if (fieldNamePtr[0] != 0)
      LoadDItemString(dialogPtr, IDC_FieldName, (char*)fieldNamePtr);

   else // fieldNamePtr[0] == 0 
		{
      GetDefaultFieldName((char*) &gTextString3);

      LoadDItemString(dialogPtr, IDC_FieldName, &gTextString3);
...
#	endif	// defined multispec_lin
   ...
   if (gProjectInfoPtr->selectionType == kRectangleType)
      //	if ( GetControlValue(gProjectWindow, gProjectInfoPtr->polygonTypeControlH) == 0)
...
#	endif
   ...
   fieldNumber =
      GetCurrentField(gProjectInfoPtr->currentClass, classFieldNumber);
...
#				if defined multispec_mac 
					...
					NumToString(
						...
						gTextString);
					LSetCell(&(gTextString[1]), (short int) gTextString[0],
						cell, gStatisticsListHandle);
#				endif	// defined multispec_mac 
...
//							AddPolyPointStatNewFieldW in SStatist.cpp
//							ClassListStatMode in SStatist.cpp
//							FieldListStatMode in SStatist.cpp
//							LoadPolyStatNewFieldW in SStatist.cpp
//							LoadRectangleInStatList in SStatist.cpp
//							NewFieldStatMode in SStatist.cpp
//							PolygonListStatMode in SStatist.cpp
...
//							CreateStatisticsWControls in SStatist.cpp
//
...
//	Revised By:			Larry L. Biehl			Date: 02/27/2002
...
void SetProjectWTitle(void) {
   ...
   //	GetCopyOfPFileName (gProjectInfoPtr, (UCharPtr)gTextString);
...
#	if defined multispec_lin
		...
		((CMStatisticsView*) gProjectWindow)->GetFrame()->FindWindow(windowid)->SetLabel(titlestring);
#	endif	// defined multispec_lin
   ...
   ControlHandle controlHandle)
{
		...
		SInt64 windowid64 = (SInt64)((int*)(controlHandle));
		SInt16 windowid = (SInt16)windowid64;
		((CMStatisticsView*) gProjectWindow)->GetFrame()->FindWindow(windowid)->Show(true);
	#endif	// defined multispec_lin
...
void StatisticsControl(void) {
   ...
   if (GetProjectImageFileInfo(kDoNotPrompt, kSetupGlobalInfoPointersIfCan))
      ...
      // If new project was initialized at the beginning, then				
      // dispose of the project structure.										
...
#	endif	// defined multispec_win 
   ...
   if (maskInfoPtr->maskHandle != NULL) 
      ...
      fileNamePPointer = GetMaskFileNamePPointer(
									maskInfoPtr,
									&maskFileStreamHandleStatus);
...
#	if defined multispec_win
		int count = ((CComboBox*) dialogPtr->GetDlgItem (maskPopupItemNumber))->GetCount();
		if (count < 3)
			dialogPtr->SetComboItemText (maskPopupItemNumber,
												count+1,
												&fileNamePPointer[1],
   ...
   Boolean useCommonCovarianceInLOOCFlag)
{
   ...
   {
      localNumberChannels =
         MIN(localNumberChannels, kMaxNumberStatChannels);
      ...
      switch (refCon) 
         ...
         case kEditNameControl:
            switch (gProjectInfoPtr->statsWindowMode) 
					{
               case 1:
                  returnFlag = EditSelectionDialog(
                     kRectangleType,
                     TRUE);
                  break;
         ...
         case kStatHistogramControl:
					...
					if (itemHit >= 1) {
						...
						if (gMemoryTypeNeeded == 0) {
							...
							UnloadSeg(ForceStatHistogramCodeResourceLoad);
         ...
         default: // Mouse down in vertical scroll bar
				#if defined multispec_mac 
					doubleClick = LClick (location, gEventRecord.modifiers,
													gStatisticsListHandle);
				#endif	// defined multispec_mac 
            break;
...
void StatisticsWMouseDn(
   ...
   if (PtInRect(mousePt, &rect)) {
      doubleClick = LClick(mousePt, gEventRecord.modifiers,
         gStatisticsListHandle);
      if (doubleClick) {
      ...
      else // !doubleClick 
      {
         ...
         if (LGetSelect(true, &cell, gStatisticsListHandle)) {
            ...
            } // end "else if (gProjectInfoPtr->statsWindowMode == kCoordinateListMode)" 
...
void UpdateStatisticsWindow(void) 
{
   ...
   GetCopyOfPFileName (gProjectInfoPtr, (FileStringPtr)gTextString);
...
#	if defined multispec_win 
		...
		GetCopyOfPFileName (gProjectInfoPtr, (FileStringPtr)gWideTextString, kUnicodeCharString);
