...
/*		if (gActiveImageViewCPtr != NULL)
			...
			return ( imageWindowCPtr->GetFileInfoHandle() );
...
//									All rights reserved.
//
//	Function name:		Handle GetActiveLegendListHandle
//
...
ListHandle GetActiveLegendListHandle (void)
...
//							stored in the structure pointed to by the input window information
//							pointer.
//
...
Handle GetWindowInfoHandle(wxView* windowPtr)
	...
	if (windowPtr != NULL) 
		{
		CMImageWindow* imageWindowCPtr = ((CMImageView*)windowPtr)->GetImageWindowCPtr();
		if (imageWindowCPtr != NULL)
			return ( imageWindowCPtr->GetWindowInfoHandle());
...
{ 
	...
	if (windowInfoPtr != NULL) 
		{   
			...
			if (windowPtr != NULL)				offscreenImageMapExistsFlag = 
											windowPtr->CheckIfOffscreenImageExists();
		#endif	// defined multispec_lin
...
{	
		...
		if (handleStatus >= 0)
			...
			WindowInfoPtr windowInfoPtr = (WindowInfoPtr)GetHandlePointer(
										gActiveImageWindowInfoH, kNoLock, kNoMoveHi);
...
{	
		...
		if (windowInfoPtr != NULL)
			...
			CMFileStream* supportFileStreamPtr = (CMFileStream*)GetHandlePointer(
													supportFileStreamHandle, kNoLock, kNoMoveHi);
