...

{
	...
	if (cemParametersPtr->correlationMatrixCode & kTrainingType)
		...
		if (computeNewSumSquaresMatrixFlag)
			...
			InitializeAreaDescription (&gAreaDescription, 
					...
					// The sums of squares for all available channels will be computed so that
					// it can be saved in the image second order statistics structure.
	...
	if (computeNewSumSquaresMatrixFlag)
		...
		continueFlag = ListSpecifiedStringNumber ( 
				...
				// information then save this information in the second order image statistics
	...
	if (continueFlag)			 
		continueFlag = SetupClsfierMemory (
								&classifyChanMeanPtr, 
								&correlationPtr,
								NULL,
								NULL,
...
//	numberBytesNeeded += (numberLines + numberTextLines + 5) * sizeof(LineRec);
	...
	if (continueFlag)
		...
		if (gClassifySpecsPtr->imageAreaFlag && (returnCode <= 1) )
			...
			if (resultsFileStreamPtr == NULL)
				...
				{
				CharPtr fileNamePtr = (char*)GetFileNameCPointer (resultsFileStreamPtr);
				continueFlag = ListSpecifiedStringNumber (
										kClassifyStrID, 
										IDS_Classify61, 
										resultsFileStreamPtr, 
		...
		if ( LoadClassifySpecs (fileInfoPtr) ) 
			...
			if ( ClassifyDialog (fileInfoPtr) )
				...
				if (continueFlag)
					...
					continueFlag = ListClassifyInputParameters (
																	fileInfoPtr,
																	resultsFileStreamPtr);
...
//							case kMahalanobisMode:		// Mahalanobis Classifer
//								MahalanobisClsfierControl (fileInfoPtr);
//								break;
...
//									break;
						...
						UnlockImageOverlayInfoHandle (gClassifySpecsPtr->imageOverlayIndex, 0);
	...
	for (line=areaDescriptionPtr->lineStart; line<=lineEnd; line+=lineInterval)
		...
		if (errCode != kSkipLine)
			...
			switch (gClassifySpecsPtr->mode)
				...
				case kMaxLikeMode:
				case kMahalanobisMode:
					returnCode = MaximumLikelihoodClassifier (
										areaDescriptionPtr, 
										fileInfoPtr,
										clsfyVariablePtr,
										outputBufferPtr,
...
//				case kMahalanobisMode:
//					returnCode = MahalanobisClassifier (
//										areaDescriptionPtr, 
//										fileInfoPtr,
//										clsfyVariablePtr,
//										outputBufferPtr,
...
//					
				...
				case kEuclideanMode:
					returnCode = EuclideanClassifier (
										areaDescriptionPtr, 
										fileInfoPtr,
										clsfyVariablePtr,
										outputBufferPtr,
				...
				case kCorrelationMode:
					returnCode = CorrelationClassifier (
										areaDescriptionPtr, 
										fileInfoPtr,
										clsfyVariablePtr,
										outputBufferPtr,
				...
				case kParallelPipedMode:
					returnCode = ParallelPipedClassifier (
										areaDescriptionPtr, 
										fileInfoPtr,
										clsfyVariablePtr,
										outputBufferPtr,
			...
			if (fileIOInstructionsPtr->maskBufferPtr == NULL)
				returnCode = WriteClassificationResults (
											outputBufferPtr,
											areaDescriptionPtr, 
											resultsFileStreamPtr, 
	...
	if (!gOSXCoreGraphicsFlag)
		{
		Handle activeImageWindowInfoHandle = gProjectInfoPtr->overlayImageWindowInfoHandle;
	...
	if (classNumber > 0 && returnCode == noErr )
		returnCode = ListTrainTestSummary (
									classPointer, 
									resultsFileStreamPtr, 
									areaDescriptionPtr, 
									countVectorPtr,
...
//	ClearAreaDescriptionOffsetVariables (areaDescriptionPtr);
			...
			if (fieldIdentPtr->fieldType == fieldType)
				...
				if (fieldType == kTestingType)
					sprintf ((char*)&gTextString, 
								...
								gEndOfLine );
				continueFlag = OutputString (classifyFileStreamPtr, 
									(char*)&gTextString, 0, gOutputCode, continueFlag);
				
				if (continueFlag)
					{
							// if the field is a cluster type of field, do not 		
							// classify it since there are no coordinates to define 	
							// it.																	
					...
					if (fieldIdentPtr->pointType < kClusterType || 
						...
						GetFieldBoundary (
										gProjectInfoPtr, areaDescriptionPtr, fieldNumber);
	...
	if (returnCode != 1)
		returnCode = !ListPerformanceTables (
									clsfyVariablePtr,
									fieldType, 
									classifyFileStreamPtr, 
									&gOutputForce1Code,
			...
			// Allow for text window or ascii file not being able to write to		
			// now.																					
			...
			if (!correlationCovarianceFlag)
				...
				if (!gClassifySpecsPtr->featureTransformationFlag)
					...
					for (feat1=0; feat1<numberChannels; feat1++)
						...
						dValue += dValue;
						xTranspose_x += 
									*ioBufferReal8Ptr * (dValue + *covariancePtr * *ioBufferReal8Ptr);
						covariancePtr++;
		...
		if (!polygonField || PtInRgn (point, rgnHandle) )
		   ...
		   if (createProbabilities)
		   	...
		   	double*		doubleDataPtr = clsfyVariablePtr->workVectorPtr;
				...
				TransformSymmetricMatrix (
									clsfyVariablePtr->workVectorPtr, 
		   	...
		   	dValue = clsfyVariablePtr->classConstant2Ptr[maxClass] - 
		   																discriminantMax;
		   	
		   			// Get the threshold table index.									
					
				classIndex = GetThresholdClass (dValue, 
															thresholdTablePtr);
...

{
	...
	gInverseMatrixMemory.indexRowPtr = CheckAndDisposePtr ( gInverseMatrixMemory.indexRowPtr );
	gInverseMatrixMemory.indexColPtr = CheckAndDisposePtr ( gInverseMatrixMemory.indexColPtr );
...

{
	...
	if (continueFlag && gClassifySpecsPtr->mode != kCorrelationMode)
		...
		gClassifySpecsPtr->thresholdProbabilityPtr = 
						(SInt16*)CheckHandleSize (
...
{ 
	...
	GetTransformedCommonCovariance (
											numberOutputChannels,
											classChannelStatsPtr,
											covariancePtr,
											channelListPtr,
	...
	*determinantOKFlagPtr = InvertSymmetricMatrix ( 
										inverseCovPtr, 
										numberFeatures, 
										gInverseMatrixMemory.pivotPtr, 
										gInverseMatrixMemory.indexRowPtr, 
...
{  
	...
	else		// gClassifySpecsPtr->mode == kMaxLikeMode, kMahalanobisMode, kFisherMode, kEchoMode
...
*/		
	...
	if (continueFlag)
		continueFlag = ListThresholdValue (
							gClassifySpecsPtr->probabilityThreshold, 
	...
	sprintf(	(char*)&gTextString, 
					...
					gEndOfLine);
	continueFlag = OutputString (resultsFileStreamPtr, 
			(char*)&gTextString, 0, gOutputForce1Code, continueFlag);
...
{                   
	...
	if (listFlag)
		...
		if (continueFlag && gClassifySpecsPtr->createThresholdTableFlag)
			{
			continueFlag = ListAverageDiscriminantValue (
														resultsFileStreamPtr,
														lOutputCode,
														numberPixels,
														probabilityTotal);
...
//	Parameters out:	None
//
// Value Returned:	True if the default values for the structure were loaded okay.  False
//							indicates that memory was not available for a vector that was needed.
// 
// Called By:			ClassifyControl
//
...
//	Handle								baseImageWindowHandle;
	...
	if (gProjectInfoPtr->classifySpecsH == NULL)
		...
		if (gProjectInfoPtr->classifySpecsH != NULL)
			...
			UInt32 numberAreas = GetNumberOfAreas (
											NULL, 
											gClassifySpecsPtr->numberClasses, 
											kTrainingType, 
...
//		gClassifySpecsPtr->numberChannels = MIN(gClassifySpecsPtr->numberChannels,
//															gProjectInfoPtr->numberStatisticsClasses);
				
//		gClassifySpecsPtr->numberFeatures = MIN(gClassifySpecsPtr->numberFeatures,
//															gProjectInfoPtr->numberStatisticsClasses);
	...
	if (continueFlag)
		...
		if (gProjectInfoPtr->statisticsCode != kMeanCovariance && 
				(gClassifySpecsPtr->mode == 1 || gClassifySpecsPtr->mode == kEchoMode
	...
	if (continueFlag)
		...
		if (gClassifySpecsPtr->classSet == kAllMenuItem)								
			LoadTrainClassVector (&gClassifySpecsPtr->numberClasses,
						&gClassifySpecsPtr->classSet, gClassifySpecsPtr->classPtr);
		
		if (gClassifySpecsPtr->classAreaSet == kAllMenuItem)	
			LoadClassAreaVector (
									&gClassifySpecsPtr->numberClassAreas, classAreaPtr);
...

{
	...
	if (continueFlag)
		...
		if (tableCode == 1)
			...
			tableDegreesOfFreedomPtr = 
									&gClassifySpecsPtr->cellThresholdDegreesOfFreedom;
		...
		if (*tableDegreesOfFreedomPtr != numberDegreesOfFreedom)
			continueFlag = CreateChiSquareThresholdTable ( 
...

{
	...
	if (continueFlag)
		...
		for (classIndex=0; 
			...
			if (determinantOKFlag)
				...
				clsfyCovStart = classIndex * numberClsfyCovEntries;
				SquareToTriangularMatrix (inverseCovPtr,
							&classifyCovPtr[ clsfyCovStart ], numberFeatureChannels );
	...
	gInverseMatrixMemory.indexRowPtr = CheckAndDisposePtr (gInverseMatrixMemory.indexRowPtr);
	gInverseMatrixMemory.indexColPtr = CheckAndDisposePtr (gInverseMatrixMemory.indexColPtr);
	gInverseMatrixMemory.inversePtr = CheckAndDisposePtr (gInverseMatrixMemory.inversePtr);
	gInverseMatrixMemory.ipvotPtr = CheckAndDisposePtr (gInverseMatrixMemory.ipvotPtr);
...

{
	...
	if (continueFlag && classConstantPtr != NULL)
		...
		bytesNeeded = factor * gClassifySpecsPtr->numberClasses *
																					sizeof(double);
	...
	thresholdCode = gClassifySpecsPtr->probabilityThresholdCode;
	 ...
	 if (gClassifySpecsPtr->thresholdFlag && thresholdCode > 0) 
		...
		for (sample=0; sample<count; sample++)
			...
			// Write classification line to ERDAS (binary) disk file if 		
			// requested.																		
