...
//					reformatOptionsPtr->outFileInfoHandle, kNoLock, kNoMoveHi);
	...
	fileNamePtr = (char*)GetFileNamePPointer (gImageFileInfoPtr);
...
*/														
			...
			if (recodeThematicImagePtr->thresholdFileInfoHandle != NULL)
				{
				fileNamePtr = (char*)GetFileNameCPointer (gImageFileInfo2Ptr);
				continueFlag = ListSpecifiedStringNumber (
														kReformatStrID, 
														IDS_Reform27,
														NULL, 
...
//	UCharPtr fileNamePtr = (UCharPtr)GetFileNamePPointer (gImageFileInfoPtr);
//	ParamText ( (ConstStr255Param)fileNamePtr,"\p", "\p", "\p" );
	...
	ShowDialogWindow (
					dialogPtr, kRecodeThematicSpecificationID, kSetUpDFilterTable);
...
{  
	...
	*thresholdImageSelectionPtr = recodeThematicImagePtr->thresholdImageSelection;
	...
	char* fileNamePtr = (char*)GetFileNamePPointer (gImageFileInfoPtr);
	...
	RecodeThematicImageDialogSetThresholdItems (
														gPopUpSelectThresholdImageMenu,
														dialogPtr,
	...
	if (itemHit == 2)
		...
		*thresholdFileInfoHandlePtr = GetMaskFile (*thresholdFileInfoHandlePtr,
		...
		if (*thresholdFileInfoHandlePtr != NULL)
			{
			fileNamePPointer = (FileStringPtr)GetFileNamePPointer (*thresholdFileInfoHandlePtr,
																	&handleStatus);
...
#		if defined multispec_lin
         wxComboBox* comboBoxPtr = (wxComboBox*) dialogPtr->FindWindow(thresholdPopupItemNumber);
         if (comboBoxPtr->GetCount() < menuItemNumber)
...

{
	...
	fileNamePPointer = (FileStringPtr)GetFileNamePPointer (gImageFileInfoPtr);
