...

{
   ...
   if (continueFlag) 
   	{
	   continueFlag = setup_basic_echo_information(
...
}		// end of routine "EchoLikeClsfierControl"
   ...
   ioBuffer1Ptr[sample] = kNullTerminator;
	...
	if (!gOSXCoreGraphicsFlag)
		...
		WindowInfoPtr imageWindowInfoPtr = (WindowInfoPtr)GetHandlePointer (
						gProjectInfoPtr->overlayImageWindowInfoHandle, kNoLock, kNoMoveHi);	
...
{               
   ...
   if (continueFlag)
   	{
	   indexRowPtr = (short int *) MNewPointer(bytes);
   ...
   if (continueFlag)
   	{
	   indexColPtr = (short int *) MNewPointer(bytes);
   ...
   if (continueFlag)
   	{
	   ipivotPtr = (short int *) MNewPointer(bytes);
