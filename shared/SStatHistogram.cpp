...
//	columnOffsetCount = fileInfoPtr->numberBins;
	...
	for (columnIndex=1; columnIndex<numberColumns; columnIndex++)
		...
		histogramRangePtr[0].minValue = 
										MIN (histogramRangePtr[0].minValue, minValue);
		histogramRangePtr[0].maxValue = 
										MAX (histogramRangePtr[0].maxValue, maxValue);
...

{
	...
	switch (statsWindowMode)
		{
		case kClassListMode:
			returnCode = HistogramProjectStats (
		...
		case kFieldListMode:
			indexStart = 0;
			returnCode = HistogramClassStats (
												fileIOInstructionsPtr,
		...
		case kCoordinateListMode:
			returnCode = HistogramFieldStats (
												fileIOInstructionsPtr,
...
//							value = (SInt32)(doubleValue + SIGN2(.5, doubleValue));
						...
						if (gStatHistogramSpecsPtr->histogramOutputCode != 0)
							...
							binIndex = GetBinIndexForStatDataValue (value,
																			&histogramRangePtr[index]);						
	...
	if (numberClasses > 0)
		...
		for (channelSet=0; channelSet<numberChannelSets; channelSet++)
			...
			if (gStatHistogramSpecsPtr->histogramOutputCode == kListData &&
				...
				if (gStatHistogramSpecsPtr->totalNumberValues > 0)
					{																			
					if (!ListStatHistogramValues(fileInfoPtr,
									classPtr,
									gStatHistogramSpecsPtr->featureTransformationFlag, 
									gStatHistogramSpecsPtr->histogramRangePtr,
									gStatHistogramSpecsPtr->histogramDataPtr, 
			...
			if (LoadStatHistogramSpecs (fileInfoPtr))
				...
				if (continueFlag && 
									gStatHistogramSpecsPtr->histogramOutputCode == kPlotData)
					{
						...
						#ifdef multispec_lin
							wxDocument*  graph_doc = ((CMultiSpecApp*)wxTheApp)->ActivateGraphView();
				...
				if (continueFlag)
					continueFlag = !SetupStatHistogramMemory (
																		fileInfoPtr, statsWindowMode);
...
*/												
				...
				if (gStatHistogramSpecsPtr->featureTransformationFlag)	
					stringID = IDS_Project84;	// Finding feature transformation data value limits
				...
				if (continueFlag && 
					...
					if (continueFlag)
						...
						GetHandlePointer (gGraphRecordPtr->vectorLengthsHandle,
													kLock,
													kNoMoveHi );
						
								// Lock handle to the graph offset vector.
						
						GetHandlePointer ( gGraphRecordPtr->xVectorDataHandle, 
													kLock, 
													kNoMoveHi);
...
*/															
					...
					LoadDItemStringNumber( kProjectStrID,
													IDS_Project82,	// Computing training area histograms
													gStatusDialogPtr,
													IDC_Status11,
													(Str255*)gTextString);
				...
				if (gStatHistogramSpecsPtr->histogramOutputCode == kPlotData)
					...
					if (continueFlag)
						...
						if (gGraphRecordPtr->xVector.numberPoints < 
							...
							MSetHandleSize (&gGraphRecordPtr->xVector.baseHandle, bytesNeeded);
							MSetHandleSize (&gGraphRecordPtr->yVector.baseHandle, bytesNeeded);
...

{
	...
	UInt32								channelIndex,
			...
			// For data with 4 bytes or more, allow for an average of 10 pixels per bin, but use 
			// a minimum of 32 bins.
...
//		histogramRangePtr[index].numberDecimalDigits =
//				GetNumberDecimalDigitsForChannel (&histogramRangePtr[index],
//																dataTypeCode);
...

{
	...
	if (fileInfoPtr->numberBytes <= 2 && !featureTransformationFlag)
		...
		valueFieldSize2 = FormatHistogramSummaryString (
											string,
											gStatHistogramSpecsPtr->minValue,
											ePrecision,
											fPrecision,
	...
	for (listNumber=0; listNumber<numberColumnarLists; listNumber++)
		...
		if (continueFlag)
			...
			if (outputGroupCode == kClassFieldGrouping && 
				...
				sprintf( &gCharBufferPtr1[charIndex], "\t%*s", 
															numberFieldSize, gTextString2);
			...
			if (outputFormatCode != kColumnList ||
				...
				if (!blankValuesFlag)
					for (columnIndex=columnStart; 
						...
						charPtr = strcpy (
									&gCharBufferPtr1[charIndex], (char*)&gTextString);
						charIndex += lenText + 1;
...
//						if (m < histogramRangePtr[columnIndex].minValue || 
//											m > histogramRangePtr[columnIndex].maxValue)
...
*/													
	...
	if (gGraphRecordPtr->xVector.numberPoints >= gGraphRecordPtr->xVector.size)
		continueFlag = CheckSizeOfStatHistogramGraphVectors (
														gGraphRecordPtr,
														histogramRangePtr,
														histogramDataPtr,
														numberFeatures,
...

{
	...
	if (returnFlag)
		...
		classPtr = (SInt16*)CheckHandleSize ( 
											&gStatHistogramSpecsPtr->classHandle,
											&returnFlag, 
											&changedFlag, 
											bytesNeeded);
		if (changedFlag)
	...
	if (returnFlag)
		...
		fieldPtr = (SInt16*)CheckHandleSize ( 
											&gStatHistogramSpecsPtr->fieldHandle,
											&returnFlag, 
											&changedFlag, 
											bytesNeeded);
		if (changedFlag)
	...
	if (gStatHistogramSpecsPtr->histogramOutputCode == kPlotData)
		...
		if (continueFlag)
			...
			if (bytesNeeded+40000 > longestContBlock)
				{
						// Memory may not be available. Setting xVector.size to zero will force
						// the data to be read first for each vector and then then memory
						// for the xVector and yVector will be obtained.
...
*/	
	...
	ShowDialogWindow (
				dialogPtr, kStatHistogramSpecificationID, kSetUpDFilterTable);
		...
		if(itemHit > 2)
			...
			switch (itemHit)
				...
				case 9:		//	 lineInterval 
					...
					lineInterval = CheckIntervalValue (
													theNum, 
													lineInterval, 
													1,  
				...
				case 14:		// radio button for "list histogram"
					...
					StatHistogramDialogListParameters (dialogPtr, histogramClassFlag, columnMatrixCode);
					break;
	...
	
{
	...
	*overlayDFAllowedFlagPtr = TRUE;
	...
	if (*histogramOutputCodePtr == kPlotData)
		...
		*overlayDensityFunctionFlagPtr = gStatHistogramSpecsPtr->overlayDensityFunctionFlag;
