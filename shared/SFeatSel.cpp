//	 									MultiSpec
//
//					Laboratory for App-2017lications of Remote Sensing
//									Purdue University
...
//									All rights reserved.
//
//	File:						SFeatSel.cpp (separability.c)
//
...
//								short int 			ComputeSeparabilityForAllChannelCombinations
//								short int 			ComputeSeparabilityForStepChannelCombinations
//								short int 			ComputeSeparabilityForAllClassCombinations
//								double 				Divergence
//								pascal void			DrawChannelCombinationsPopUp
//								double 				EFBhattacharyya
//								short int 			GetCombinationLimit
...
//								short int 			UpdateSeparabilityLists
//
...
double EFBhattacharyya ( 	
				...
				short int							numberFeatures)	// number of features (channels) 
	...
	for (	channelCombinationSet=0; 
		...
		if (gSeparabilitySpecsPtr->distanceMeasure != kNonCovarianceBhattacharyya )
			...
			SInt16* classPtr = (SInt16*)GetHandlePointer(
							gSeparabilitySpecsPtr->classHandle, kNoLock, kNoMoveHi);	
		...
		if (continueFlag)
			...
			SInt16* featurePtr = (SInt16*)GetHandlePointer(
							gSeparabilitySpecsPtr->featureHandle, kNoLock, kNoMoveHi);	
	...
	
{
	...
	returnFlag = LoadSpecifiedStringNumberLongP (
								kFeatureSelectionStrID, 
								index, 
								(char*)&gTextString, 
								(char*)&gTextString2,
...

{
	...
	currentChannelSetsPtr = (short int*)MNewPointer ( 
						(SInt32)numberSeparabilityChannels * sizeof(short int) );
                             			...
                             			
{
	...
	DrawPopUpMenuItem (dialogPtr, itemNumber, 
				gPopUpAllPossibleSubsetMenu, gChannelCombinationSelection, TRUE);
...

{
	...
	if (gCharBufferPtr1 != NULL)
		...
		SInt16* channelsPtr = (SInt16*)GetHandlePointer(
						gSeparabilitySpecsPtr->channelsHandle, kNoLock, kNoMoveHi);	
...

{
	...
	if (continueFlag)
//		SInt16* classPairWeightsListPtr = (SInt16*)GetHandlePointer (
...
/*										
	#ifdef powerc
		RoutineDescriptor					drawSeparabilityDistancePopUpRD = 
														BUILD_ROUTINE_DESCRIPTOR(
																uppUserItemProcInfo, 
																DrawSeparabilityDistancePopUp);
		RoutineDescriptor					drawChannelsPopUp2RD = 
														BUILD_ROUTINE_DESCRIPTOR(
																uppUserItemProcInfo, 
																DrawChannelsPopUp2);
		RoutineDescriptor					drawChannelCombinationsPopUpRD = 
														BUILD_ROUTINE_DESCRIPTOR(
																uppUserItemProcInfo, 
																DrawChannelCombinationsPopUp);
		RoutineDescriptor					drawInterClassWeightsPopUpRD = 
														BUILD_ROUTINE_DESCRIPTOR(
																uppUserItemProcInfo, 
																DrawInterClassWeightsPopUp);
	#endif
*/	
...
/*										
	#ifdef powerc
		drawSeparabilityDistancePopUpPtr = &drawSeparabilityDistancePopUpRD;
		drawChannelsPopUp2Ptr = &drawChannelsPopUp2RD;
		drawChannelCombinationsPopUpPtr = &drawChannelCombinationsPopUpRD;
		drawInterClassWeightsPopUpPtr = &drawInterClassWeightsPopUpRD;
	#else
		drawSeparabilityDistancePopUpPtr = DrawSeparabilityDistancePopUp;
		drawChannelsPopUp2Ptr = DrawChannelsPopUp2;
		drawChannelCombinationsPopUpPtr = DrawChannelCombinationsPopUp;
		drawInterClassWeightsPopUpPtr = DrawInterClassWeightsPopUp;
	#endif	
*/	
...
//				gProjectInfoPtr->classPairWeightsListHandle, kNoLock, kNoMoveHi);
...
//		LoadDItemString (dialogPtr, 7, (Str255*)"\pFeatures:");
				...
				case 25:		// Do not sort distance measures. 
					...
					for ( itemNumber=13; itemNumber<=15; itemNumber++)
						...
						if (itemHit != itemNumber)
							{
							GetDialogItem (dialogPtr, itemNumber, &theType, &theHandle, 
																							&theBox);
...
//
//	Coded By:			Larry L. Biehl			Date: 11/03/1989
//	Revised By:			Larry L. Biehl			Date: 06/26/1990	
	...
	if (continueFlag)
		...
		if (continueFlag)
			...
			bytesNeeded = combinationSlotsNeeded * 
											numberSeparabilityChannels * sizeof(SInt16);
			gSeparabilitySpecsPtr->channelSetsPtr = 
													(short int*)MNewPointer (bytesNeeded);
