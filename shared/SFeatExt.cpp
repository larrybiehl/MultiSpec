...
//									All rights reserved.
//
//	File:						SFeatExt.cpp (featureExtraction.c)
//
//	Authors:					Chulhee Lee
//								Larry L. Biehl
//
...
{	
	...
	DrawPopUpMenuItem (dialogPtr, itemNumber, 
													gPopUpFEAlgorithmMenu, gFEAlgorithmCode, TRUE);
...
{	
	...
	DrawPopUpMenuItem (dialogPtr, itemNumber, 
				gPopUpPreprocessMenu, gPreprocessSelection, TRUE);
...

{
	...
	dialogPtr = LoadRequestedDialog ( 
											kProjectionPursuitDialogID, kCopyScrap, 1, 2);
...
/*	numericalOptimizationFlag = 
	...
	algorithmCode = projectionPursuitSpecsPtr->algorithm;	
	ProjectionPursuitDialogAlgorithm ( 
								dialogPtr, algorithmCode, numericalOptimizationFlag);
			
			// Initial grouping options.
	
	initialGroupingCode = projectionPursuitSpecsPtr->initialGroupingCode;
	if (projectionPursuitSpecsPtr->finalNumberFeatures == 0)
		{	
		SetDLogControlHilite (dialogPtr, 13, 255);
		initialGroupingCode = kUniformInitGrouping;
	...
	initialNumberFeatures = projectionPursuitSpecsPtr->initialNumberFeatures;
	LoadDItemValue (dialogPtr, 14, initialNumberFeatures);	
	
	LoadDItemValue (dialogPtr, 16, projectionPursuitSpecsPtr->finalNumberFeatures);
		
			// Band grouping methods.						
	
	firstStageMethod = projectionPursuitSpecsPtr->firstStageMethod;
	ProjectionPursuitDialogFSOptions (dialogPtr, algorithmCode, firstStageMethod);
	
			// Thresholds.				
		
	optimizeThreshold = projectionPursuitSpecsPtr->optimizationThreshold;
	LoadDItemRealValue (dialogPtr, 9, optimizeThreshold, 2);	
		
	topDownThreshold = projectionPursuitSpecsPtr->tdThreshold;
	LoadDItemRealValue (dialogPtr, 24, topDownThreshold, 2);			
		
	bottomUpThreshold = projectionPursuitSpecsPtr->buThreshold;
	LoadDItemRealValue (dialogPtr, 27, bottomUpThreshold, 2);
	
			// Maximum number features.				
		
	maximumNumberFeatures = projectionPursuitSpecsPtr->maximumNumberFeatures; 
