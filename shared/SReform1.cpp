...
*/													
		...
		if (thematicListType > kClassDisplay)
			symbolToOutputBinPtr = CreateClassToGroupTable(
														fileInfoPtr,
														outFileInfoPtr,
														symbolToOutputBinPtr,
														newPaletteIndexPtr);
...
//				&&  reformatOptionsPtr->functionCode == kNoFunction)
	...
	if (reformatOptionsPtr->transformDataCode == kCreatePCImage || 
		...
		{
		if (reformatOptionsPtr->transformDataCode == kCreatePCImage)
			tempBufferPtr = (HDoublePtr)MNewPointer (numberOutChannels * sizeof(double));
			
		else		// reformatOptionsPtr->transformDataCode == kFunctionOfChannels
			tempBufferPtr = (HDoublePtr)MNewPointer (numberColumns * sizeof(UInt32));
	...
	if (continueFlag)
		...
		savedOutBufferPtr = ioOut1ByteBufferPtr;
				...
				// sequential data when multiple channels do not need to available at a time
				// to speed the disk access up.
		...
		gNextTime = TickCount();					
		for (outsideLoopChannel=0; outsideLoopChannel<numberOutsideLoops; outsideLoopChannel++)
			{
			if (numberOutsideLoops > 1)
...
//										0, 		// Not used. NewPaletteIndexPtr takes it
													// into account
...
//											savedSwapBytesFlag;
	...
	if (continueFlag)
		...
		reformatOptionsPtr = (ReformatOptionsPtr)GetHandlePointer(
													reformatOptionsH, kLock, kNoMoveHi);
...
{                
	...
	LoadDItemString (dialogPtr, 13, (char*)inputBandInterleaveString);
	
	LoadDItemString (dialogPtr, 17, (char*)inputDataValueTypeString);
...
//	if (!fileInfoPtr->thematicType)
	...
	SetDialogItemDrawRoutine (
								dialogPtr, 44, &headerBox, drawHeaderOptionsPopUpPtr);
	...
	if (gMultiSpecWorkflowInfo.workFlowCode != 1 && 
		...
		{
		ShowDialogWindow (
						dialogPtr, kChangeImageFormatDialogID, kSetUpDFilterTable);
					...
					if (gDataValueTypeSelection > 0)
						...
						if (reformatOptionsPtr->transformDataCode == kNoTransform)
							noTransformDataValueTypeSelection = gDataValueTypeSelection;
								...
								// Indicate that the user did make a selection in the box even if
...
#	if defined multispec_mac
		if (CreateUnicodeStaticTextControl (dialogPtr, &fileNamePtr[1], fileNamePtr[0], IDC_InputFileName, NULL) != noErr)
...
#	endif
	...
	LoadDItemValue (dialogPtr, IDC_ChangeFormatInLines, (SInt32)gImageWindowInfoPtr->maxNumberLines);
	
			// item = 9: Number of columns in input image.								
			
	LoadDItemValue (dialogPtr, IDC_ChangeFormatInColumns, (SInt32)gImageWindowInfoPtr->maxNumberColumns);
	...
	*rightToLeftFlagPtr = FALSE;
 	...
 	*channelDescriptionAllowedFlagPtr = 
 			(!fileInfoPtr->thematicType && !*channelThematicDisplayFlagPtr &&
	...
	*GAIAFormatAllowedFlagPtr = FALSE;
	...
	if (mapInfoExistsFlag)
	...
	#if defined multispec_win  
		comboBoxPtr->InsertString (kTIFFGeoTIFFMenuItem, (LPCTSTR)A2T((LPCSTR)&tiffMenuNameStringPtr[1]));
      ...
      comboBoxPtr->Insert(tiffmenustring, kTIFFGeoTIFFMenuItem, (void *)gTIFFGeoTIFFMenuItem);
		comboBoxPtr->SetClientData(kTIFFGeoTIFFMenuItem, (void*)kTIFFGeoTIFFMenuItem);
      
		//if (((CMChangeFormatDlg*)dialogPtr)->bSizer124 == NULL)
		//	wxMessageBox("bSizer124 is NULL at end of ChangeImageFormatDialogInitialize", "Confirm", wxYES_NO);
   #endif
	...
	if (*headerOptionsSelectionPtr == kTIFFGeoTIFFMenuItem)
		ChangeImageFormatDialogUpdateTIFFHeader (
													dialogPtr,
													fileInfoPtr,
...
//			reformatOptionsPtr->dataValueTypeSelection != dataValueTypeSelection)
	...
	outFileInfoPtr->numberBits = 
						dataValueTypeToBitsCode[dataValueTypeSelection];
	outFileInfoPtr->numberBytes = 
						dataValueTypeToBytesCode[dataValueTypeSelection];
	outFileInfoPtr->dataTypeCode = 
						dataValueTypeToDataTypeCode[dataValueTypeSelection];
...
//		DisableMenuItem (popUpHeaderOptionsMenu, kMatlabMenuItem);
	...
	if (dataValueTypeSelection >= k4ByteSignedIntegerMenuItem)
		{
				// If 4-byte or 8-byte output is requested, turn off
				// some options.
...
//		comboBoxPtr->SetItemData(0, kNoneMenuItem);
		...
		if (menuItemList[kTIFFGeoTIFFMenuItem])
			{
			comboBoxPtr->InsertString (-1, (LPCTSTR)A2T((LPCSTR)&tiffMenuNameStringPtr[1]));
...
#	if defined multispec_lin
		...
		wxComboBox* comboBoxPtr = (wxComboBox*)(dialogPtr->FindWindow(IDC_ChangeHeader));
...

{
   ...
   #if defined multispec_lin
		wxComboBox* comboBoxPtr = (wxComboBox*) dialogPtr->FindWindow(IDC_OutputFileSelection);
   #endif
	...
	switch (headerOptionsSelection)
		...
		case kArcViewMenuItem:
			...
			if (*dataValueTypeSelectionPtr > k4ByteRealMenuItem)
				...
				*dataValueTypeSelectionPtr = 
										FileSpecificationDialogSetDataType (
															kRealType,
															numberBits,
															fileInfoPtr->signedDataFlag);
...
//			lastScaleFactor = 10;
		...
		minimumSelectedNumberBits = DeterminePCMinimumNumberBits (lastScaleFactor, 
								eigenSource, reformatOptionsPtr->pcChannelPtr[0] );
	...
	if (!GetDefaultBandRatio (gImageWindowInfoPtr, gImageFileInfoPtr, reformatOptionsPtr))
		{
	...
	lastAdjustSelectedChannelsFactor = reformatOptionsPtr->transformAdjustSelectedChannelsFactor;
...
//					if (theNum <= 0 || theNum > maxAdjustFactor)
//						NumberErrorAlert ((SInt32)lastAdjustFactor, dialogPtr, itemHit );
							
//					else		// theNum >0  && theNum <= maxAdjustFactor 
...
//					if (theNum <= 0 || theNum > maxAdjustDivisor)
//						NumberErrorAlert ((SInt32)lastAdjustDivisor, dialogPtr, itemHit );
//					else		// theNum >0  && theNum <= maxAdjustDivisor 
//						lastAdjustDivisor = (SInt16)theNum;
					break;
			...
			if	(itemHit == 1)      // User selected OK for information 
				...
				*recommendedNumberOfBitsPtr = minimumSelectedNumberBits;
				...
				if (reformatOptionsPtr->transformDataCode == kAdjustChannelsByChannel)
					...
					reformatOptionsPtr->transformAdjustSelectedChannel = lastAdjustSelectedChannel;
...

{
	...
	if (callConvertDataValueToBinValueFlag)
		...
		for (i=0; i<numberSamples; i++)
			{
			*ioOut1BufferPtr = 
						(unsigned char)symbolToOutputBinPtr[*ioIn2BufferPtr];
		...
		if (numberBytes == 1)
			...
			for (i=0; i<numberSamples; i++)
				{
				*ioOut1BufferPtr = 
							(unsigned char)symbolToOutputBinPtr[*ioOut1BufferPtr];
				ioOut1BufferPtr++;
...

{
	...
	if (outDescriptionHandle != NULL)
		...
		outDescriptionPtr = (HUCharPtr)GetHandlePointer( 
																outDescriptionHandle, 
																kLock, 
																kNoMoveHi);
...

{
	...
	if (continueFlag)
		{
		outClassNamePtr = (Str31*)GetHandlePointer (outDescriptionHandle, 
																		kLock, 
																		kNoMoveHi);
...
*/	
	if (continueFlag)
		...
		dataDisplayPtr = (HUCharPtr)GetHandlePointer (
											gToDisplayLevels.vectorHandle, kNoLock, kNoMoveHi);
										
		numberBins = gToDisplayLevels.bytesOffset;
		numberBins = MIN (inFileInfoPtr->numberBins, numberBins);
	
				// Allow for offset of the palette of one. As of 2011, do not need to 
				// allow for palette offset of 1.
				
		for (index=0; index<numberBins; index++)
////			symbolToOutputBinPtr[index] = dataDisplayPtr[index] - 1;
...

{
	...
	DrawPopUpMenuItem (dialogPtr, itemNumber, 
						gPopUpChanDescriptionMenu, gChanDescriptionSelection, TRUE);
...

{
	...
	DrawPopUpMenuItem (dialogPtr, itemNumber, 
										gPopUpFunctionMenu, gFunctionSelection, TRUE);
...

{
	...
	DrawPopUpMenuItem (dialogPtr, itemNumber, 
										gPopUpAllSubsetMenu, gPCChannelSelection, TRUE);
			...
			for (channel=1; channel<numberChannels; channel++)
				...
				for (j=0; j<numberSamples; j++)
					...
					if (!noDataValueFlag || *ioDoubleBufferPtr != noDataValue)
						*ioOutDoubleBufferPtr = MIN (*ioOutDoubleBufferPtr, *ioDoubleBufferPtr);
...
*/	
			...
			for (channel=1; channel<numberChannels; channel++)
				...
				for (j=0; j<numberSamples; j++)
					{	
					if (!noDataValueFlag || *ioDoubleBufferPtr != noDataValue)
						*ioOutDoubleBufferPtr = MAX (*ioOutDoubleBufferPtr, *ioDoubleBufferPtr);
		...
		case kFunctionKthSmallestElement:
					...
					// Determine the requested kth smallest value in the list of channel values.
		...
		case 98:		// special case
					...
					// This case is to find the channels with the latest lowest value within
	...
	if (saveFunctionCode != kFunctionMedian && 
				saveFunctionCode != kFunctionKthSmallestElement && 
						inputBISFlag)
		{
				// The output values are in the first channel locations by every numberChannels
				// since the input data is in BIS mode. Pack the data so that the columns
	...
	if (gAlgebraicTransformationFunction.denominatorString[0] > 0)
		{
				...
				// number of channels for the input image agree with that in the saved file.
				
		if (fileInfoPtr->instrumentCode == gAlgebraicTransformationFunction.instrumentCode &&
				windowInfoPtr->totalNumberChannels == gAlgebraicTransformationFunction.numberChannels)
			{
			...
			reformatOptionsPtr->transformFactor = gAlgebraicTransformationFunction.transformFactor;
			reformatOptionsPtr->transformOffset = gAlgebraicTransformationFunction.transformOffset;
...
/*	if (continueFlag && reformatOptionsHPtr != NULL)
		{		
		gNonProjProcessorSpecs.reformatOptionsSpecsH = MNewHandle (sizeof(ReformatOptions));
...

{
		...
		CComboBox* comboBoxPtr = (CComboBox*)(dialogPtr->GetDlgItem(comboBoxHeaderItem));
      ...
      UInt16			comboBoxHeaderItem;
		...
		if (popUpHeaderOptionsMenu != NULL)
		 ...
		 wxComboBox* comboBoxPtr = (wxComboBox*) (dialogPtr->FindWindow(comboBoxHeaderItem));
...

{
	...
	if (keepSameFormatAsInputFlag)
		...
		if (outFileInfoPtr->bandInterleave < kBIL || outFileInfoPtr->bandInterleave > kBIS)
			outFileInfoPtr->bandInterleave	= kBSQ;
...
//		reformatOptionsPtr->transformFactor = 1.;
//		reformatOptionsPtr->transformOffset = 0.;
//		GetDefaultBandRatio (gImageWindowInfoPtr, gImageFileInfoPtr, reformatOptionsPtr);
		...
		if (gImageFileInfoPtr != NULL && gImageWindowInfoPtr != NULL)
			{
			reformatOptionsPtr->lastInputFileDataTypeCode = gImageFileInfoPtr->dataTypeCode;
			reformatOptionsPtr->lastInputFileSignedDataFlag = gImageFileInfoPtr->signedDataFlag;
			reformatOptionsPtr->lastInputWindowNumberBytes = gImageWindowInfoPtr->numberBytes;
	...
	if (gNonProjProcessorSpecs.reformatOptionsSpecsH != NULL)
		...
		if (!releaseReformatMemoryFlag &&
			...
			releaseReformatMemoryFlag = TRUE; 
				...
				// by the last reformat task, then clear the reformat options structure and
				// start again.
				
//		if (!releaseReformatMemoryFlag && 
//					(reformatOptionsPtr->lineEnd > (SInt32)windowInfoPtr->maxNumberLines ||
//							reformatOptionsPtr->columnEnd > (SInt32)windowInfoPtr->maxNumberColumns))
//			releaseReformatMemoryFlag = TRUE; 
...
{				
	...
	gAlgebraicTransformationFunction.transformFactor = reformatOptionsPtr->transformFactor;
	gAlgebraicTransformationFunction.transformOffset = reformatOptionsPtr->transformOffset;
