...
//
//	Brief description:	This file contains routines to handle displaying multispectral image
//                      windows. Some routines which write into the offscreen buffers are in
//								other files.
//
	...
	if (continueFlag)
		...
		if (continueFlag)
			...
			if (gCallProcessorDialogFlag)
                ...
                // need to the cursor to a wait status.
				MSetCursor(kWait);
...
#					endif		// defined multispec_lin
					...
					displaySpecsPtr->displayedColumnInterval = displaySpecsPtr->columnInterval;

					displaySpecsPtr->displayedNumberChannels = displaySpecsPtr->numberChannels;

							// Indicate that class names for the legend for 1-channel thematic
							// displays will need to be re-created.
...
//	Revised By:			Larry L. Biehl			Date: 06/08/2015	
				...
				
{
	...
	sprintf ((char*)gTextString, "Set Display Specifications for '%s'", (char*)&fileNamePtr[1]);
	...
	//	{
		...
		osErr = CreateStaticTextControl (GetDialogWindow(dialogPtr), &theBox, CFSTR("This is a test"), NULL, &controlRef);
...
void DisplayMultispectralDialogInitialize(
        ...
        Boolean* includeVectorOverlaysFlagPtr)
 {
    ...
    *displayTypeMenuSelectionPtr = displaySpecsPtr->displaySet;
    if (*localDisplayTypePtr == k1_ChannelGrayLevelDisplayType && *displayTypeMenuSelectionPtr != 2)
        *displayTypeMenuSelectionPtr = 2;

    // Enable/disable appropriate items in the display type pop up menu.	

    UpdatePopUpDisplayTypeMenu(windowInfoPtr->totalNumberChannels, *maxSystemPixelSizePtr);
				...
				Boolean								includeVectorOverlaysFlag)
{
	...
	// green channel to display for 3-channel			
	// display.																	
	...
	// Get requests for invertion of date values for each display
	// color.
    ...
    switch (newDisplayTypeMenuSelection) {
        ...
        case 7: // Side by Side channels 
            *localDisplayTypePtr = newDisplayTypeMenuSelection;
            ...
            // Indicate that only one color plane will be 	
            // used.														
        ...
        case 6: // 3-channel display 
            *localDisplayTypePtr = k3_ChannelDisplayType;
            ...
            // Indicate that red, green, blues color planes 
            // will be used.											
...
void DisplayMultispectralDialogUpdateDisplayLevels(
        ...
        SInt16* numberDisplayLevelsPtr)
 {
    ...
    {
#if defined multispec_win || defined multispec_lin
        // For windows, take into account that there are only 2 bits of color
        // choices 8 and 24 not 8, 16 and 24.  Bits of Color of 2 actually is
        // is an index of 3 into the color levels max vector.
            ...
            char                          theChar)
{
   ...
   if (window != NULL)
      ...
      //          window->CheckIfOffscreenImageExists())
      //#endif
      //#ifndef multispec_lin
      if ((windowType == kImageWindowType || windowType == kThematicWindowType) &&
               ...
               if (shiftKeyFlag)
                  {
                     ...
                     if (theChar == 0X26)
                        ...
                        if (displaySpecsPtr->channelNumber < (SInt16) totalNumberChannels)
                           {
                           displaySpecsPtr->channelNumber = (SInt16) totalNumberChannels;
                           callDisplayProcessorFlag = TRUE;
...
PascalVoid DrawBitsColorPopUp(
        ...
        SInt16 itemNumber)
 {
    ...
    DrawPopUpMenuItem(dialogPtr, itemNumber,
            gPopUpBitsOfColorMenu, gBitsOfColorSelection, TRUE);
...
PascalVoid DrawDisplayTypePopUp(
        ...
        SInt16 itemNumber)
 {
    ...
    DrawPopUpMenuItem(dialogPtr, itemNumber,
            gPopUpDisplayTypeMenu, gDisplayTypeMenuSelection, TRUE);
...

{
        ...
        DrawPopUpMenuItem (dialogPtr, itemNumber, 
										gPopUpEnhancementMenu, gEnhancementSelection, TRUE);
...
PascalVoid DrawMinMaxPopUp(
        ...
        SInt16 itemNumber)
 {
    ...
    DrawPopUpMenuItem(dialogPtr, itemNumber,
            gPopUpMinMaxMenu, gEnhanceMinMaxMenuSelection, TRUE);
...
PascalVoid DrawStretchPopUp(
        ...
        SInt16 itemNumber)
 {
    ...
    DrawPopUpMenuItem(dialogPtr, itemNumber,
            gPopUpStretchMenu, gEnhanceStretchSelection, TRUE);
...
PascalVoid DrawZeroAsPopUp(
        ...
        SInt16 itemNumber)
 {
    ...
    DrawPopUpMenuItem(dialogPtr, itemNumber,
            gPopUpTreatZeroAsMenu, gEnhanceZeroAsSelection, TRUE);
        ...
        UInt16* channelsPtr)
 {
    ...
    if (continueFlag) {
        ...
        for (channel = 0; channel < numberChannels; channel++) {
            ...
            // Get the starting point for the look up table for the next	
            // channel.																		
...
Boolean FillDataToDisplayLevels(
        ...
        UInt16* channelsPtr)
 {
    ...
    for (channel = 0; channel < numberChannels; channel++) {
        ...
        if (displaySpecsPtr->displayType == k1_ChannelThematicDisplayType) {
            // Set floatDisplayLevel to represent display class for the minBinIndex.
            // It will be updated below if it needs to be for cases where minimum
            // data value in the image is not less than the requested minimum 
            // thematic value.
        ...
        else // displaySpecsPtr->displayType != k1_ChannelThematicDisplayType
        {
            ...
            if (displaySpecsPtr->pixelSize <= 8) {
                if (minBinIndex == maxBinIndex) {
                    // For this case force the data value in the middle of the minimum
                    // value and the maximum value to be in the middle color range.
                    // Note that for this case the for loop after this will skipped.
        ...
        if (displaySpecsPtr->displayType == k1_ChannelThematicDisplayType) {
            ...
            /*			if (histogramSummaryPtr[channelNumberIndex].binType == kDataValueIsBinIndex)
                                            {
                                            indexForMinDataValue = (UInt16)displaySpecsPtr->thematicTypeMinMaxValues[0];
                                            indexForMaxDataValue = (UInt16)displaySpecsPtr->thematicTypeMinMaxValues[1];
            ...
            for (displayLevel = 0; displayLevel < numberLevels; displayLevel++) {
                ...
                /*			
                                        ...
                                        numberChars = sprintf ((char*)&gTextString3, 
                                                                                                        ...
                                                                                                        newTotal,
                                                                                                        gEndOfLine);
                                        if (!ListString( (char*)&gTextString3,
                                                                                        numberChars,
                                                                                        gOutputTextH) )
                                                                                                                                                                                        return (FALSE);
                 */
...
//									All rights reserved.
//
//	Function name:		void GetDisplayIOBuffer
//
//	Software purpose:	The purpose of this routine is to display one
//							channel of an image in color in a window on the
//							screen.
//
//	Parameters in:					
//
 ...
 *BILSpecialFlagPtr = TRUE;
        if (channelListPtr != NULL)
                {
                                // Load these variables since it has not been loaded yet.
				
                fileIOInstructionsPtr->windowInfoPtr = gImageWindowInfoPtr;	
                fileIOInstructionsPtr->layerInfoPtr = gImageLayerInfoPtr;	
                fileIOInstructionsPtr->fileInfoPtr = gImageFileInfoPtr;	
 ...
 *BILSpecialFlagPtr = GetBILSpecial(
        ...
        gUseThreadedIOFlag = gHasThreadManager && *BILSpecialFlagPtr;
	
                        // Get the number of samples to be read from disk to memory for one line.
                        // Assume all columns for one line of data.					
                        // Note that multiple image files are not allowed for BIS formatted	
                        // image files.					

        numberSamples = fileInfoPtr1->numberColumns;
	
        if (numberListChannels >= 2)
                numberSamples = MAX(numberSamples, fileInfoPtr2->numberColumns);
						
        if (numberListChannels == 3)
                numberSamples = MAX(numberSamples, fileInfoPtr3->numberColumns);
	
        if (fileInfoPtr1->bandInterleave == kBIS)
                numberSamples *= fileInfoPtr1->numberChannels;

                        // Get handle to block of memory to use as file IO buffer for 
                        // 'numberListChannels' channels of data.  If the input band interleave 
                        // format is BIS, then request memory for all channels in the file.	
                        // Note that multiple image files are not allowed for BIS formatted	
                        // image files.																	
															
        numberBytes = numberSamples * fileInfoPtr1->numberBytes;
        if (fileInfoPtr1->bandInterleave == kBIS)
                numberBytes = numberSamples * fileInfoPtr1->numberBytes;
		
        else		// fileInfoPtr1->bandInterleave != kBIS
                {				
                if (*BILSpecialFlagPtr)
                        numberBytes = numberSamples * fileInfoPtr1->numberBytes *
                                        (channelListPtr[numberListChannels-1] - channelListPtr[0] + 1);
			
                else		// !*BILSpecialFlagPtr
                        {
                        numberBytes = numberSamples * fileInfoPtr1->numberBytes;
					
                        if (numberListChannels >= 2)
                                numberBytes += numberSamples * fileInfoPtr2->numberBytes;
					
                        if (numberListChannels == 3)
                                numberBytes += numberSamples * fileInfoPtr3->numberBytes;
				
                        }		// end "else !*BILSpecialFlagPtr"
		
                }		// end "else fileInfoPtr1->bandInterleave != kBIS"
			
                        // Force number of bytes to be a multiple of 8 bytes.
			
        numberBytes = 8 * ((numberBytes + 7)/8);
        ...
        numberTileBytes = GetSetTiledIOBufferBytes (gImageLayerInfoPtr,
                        ...
                        // Get the number of buffers to be use. 2 will be needed if
                        // threaded IO is to be used.
	
        numberBuffers = 1;		
        if (gUseThreadedIOFlag)
                numberBuffers = 2; 
		
        ioBufferPtr = MNewPointer (numberBuffers * (numberBytes + numberTileBytes));
	
 *tiledBufferPtrPtr = NULL;
        if (ioBufferPtr != NULL && numberTileBytes > 0)
 *tiledBufferPtrPtr = (HUCharPtr)&ioBufferPtr[numberBuffers*numberBytes];
	
                        // Return the number of samples to be used.
	
 *numberSamplesPtr = (UInt32)displaySpecsPtr->columnEnd - 
                                                                                                                                displaySpecsPtr->columnStart + 1;
	
                        // Return the number of bytes per buffer.
			
 *numberBytesPtr = numberBytes;
	
        return (ioBufferPtr);
																						
}		// end "GetDisplayIOBuffer"  
 */
        ...
        SInt16* classForMaxDataValuePtr)
 {
    ...
    *classForMaxDataValuePtr = 0;
    ...
    if (displaySpecsPtr->displayType == k1_ChannelThematicDisplayType) {
        ...
        if (displaySpecsPtr->minMaxCode == kEntireDataRange ||
                displaySpecsPtr->minMaxCode == kPerChannelMinMax) {
            minThematicValue = GetDataValueForBinIndex(
                    minBinIndex,
                    histogramSummaryPtr,
                    signedValueOffset);

            maxThematicValue = GetDataValueForBinIndex(
                    maxBinIndex,
                    histogramSummaryPtr,
                    signedValueOffset);
...
SInt16 GetThematicClassForDataValue(
        ...
        SInt16 numberThematicClasses)
 {
    ...
    }// end "else if (numberThematicClasses == 2)"
    ...
    if (redoFlag) {
        ...
        if (gToDisplayLevels.vectorHandle != NULL) {
            ...
            if (continueFlag) {
                ...
                if (displaySpecsPtr->enhancementCode == kLinearStretch)
                    returnFlag = FillDataToDisplayLevels(
                        histogramSpecsPtr,
                        dataDisplayPtr,
                        displaySpecsPtr,
                        numberValues,
                    ...
                    returnFlag = GaussianToDisplayLevels(
                        histogramSpecsPtr,
                        dataDisplayPtr,
                        displaySpecsPtr,
                        numberValues,
                ...
                gToDisplayLevels.gaussianStretch =
                        displaySpecsPtr->gaussianStretch;
...
DisplaySpecsPtr LoadMultispectralDisplaySpecs (void)
{
   ...
   channelsPtr = (UInt16*) CheckHandleSize(
            &displaySpecsPtr->channelsHandle,
            &continueFlag,
            &pointerChanged,
            numberBytes);
   ...
   if (displaySpecsPtr->firstTime)
      ...
      if (gImageWindowInfoPtr->totalNumberChannels == 1)
         ...
         else if (gImageWindowInfoPtr->totalNumberChannels == 278)
            {
      ...
      */
      ...
      if (gDisplaySpecsDefault.structureLoadedFlag)
         ...
         if (gImageWindowInfoPtr->dataTypeCode == gDisplaySpecsDefault.dataTypeCode &&
                 ...
                 gImageFileInfoPtr->signedDataFlag == gDisplaySpecsDefault.signedDataFlag &&
            ...
            if (gDisplaySpecsDefault.lastDisplayType == k1_ChannelThematicDisplayType)
               ...
               displaySpecsPtr->numberLevels = gDisplaySpecsDefault.numberThematicLevels;
               displaySpecsPtr->channelNumber = gDisplaySpecsDefault.channelNumber;
               displaySpecsPtr->enhancementCode = gDisplaySpecsDefault.enhancementCode;
               displaySpecsPtr->minMaxCode = gDisplaySpecsDefault.minMaxCode;
               displaySpecsPtr->percentTailsClipped = gDisplaySpecsDefault.percentTailsClipped;
               displaySpecsPtr->displayType = gDisplaySpecsDefault.lastDisplayType;
               displaySpecsPtr->thematicValueFactor = gDisplaySpecsDefault.thematicValueFactor;
               displaySpecsPtr->invertValuesFlag[2] = gDisplaySpecsDefault.invertValuesFlag[2];
               displaySpecsPtr->thematicClassPaletteType =
                      gDisplaySpecsDefault.oneChannelThematicClassPaletteType;
               ...
               displaySpecsPtr->displaySet = gDisplaySpecsDefault.displaySet;
               displaySpecsPtr->invertValuesFlag[2] = gDisplaySpecsDefault.invertValuesFlag[2];
               displaySpecsPtr->backgroundValueCode = gDisplaySpecsDefault.backgroundValueCode;
               displaySpecsPtr->backgroundDataValue = gDisplaySpecsDefault.backgroundDataValue;
               ...
               {
               displaySpecsPtr->numberLevels = gDisplaySpecsDefault.numberLevels;
               displaySpecsPtr->redChannelNumber = gDisplaySpecsDefault.redChannelNumber;
               displaySpecsPtr->invertValuesFlag[2] = gDisplaySpecsDefault.invertValuesFlag[2];
               displaySpecsPtr->greenChannelNumber = gDisplaySpecsDefault.greenChannelNumber;
               displaySpecsPtr->invertValuesFlag[1] = gDisplaySpecsDefault.invertValuesFlag[1];
               displaySpecsPtr->blueChannelNumber = gDisplaySpecsDefault.blueChannelNumber;
               displaySpecsPtr->invertValuesFlag[0] = gDisplaySpecsDefault.invertValuesFlag[0];
               displaySpecsPtr->enhancementCode = gDisplaySpecsDefault.enhancementCode;
               displaySpecsPtr->minMaxCode = gDisplaySpecsDefault.minMaxCode;
               displaySpecsPtr->percentTailsClipped = gDisplaySpecsDefault.percentTailsClipped;
               displaySpecsPtr->displayType = gDisplaySpecsDefault.lastDisplayType;
...
Boolean MinMaxEnhancementDialog(
        ...
        double* minMaxValuesPtr)
 {
    ...
    SetDLogControl(dialogPtr, IDC_PerChannel, (localMinMaxCode == kPerChannelMinMax));
    ShowHideDialogItem(dialogPtr, IDC_PercentClipped, (localMinMaxCode == kPerChannelMinMax));

    SetDLogControl(dialogPtr, IDC_EntireRange, (localMinMaxCode == kEntireDataRange));

    SetDLogControl(dialogPtr, IDC_UserSpecified, (localMinMaxCode == kUserSpecified));

    SetDLogControl(dialogPtr, IDC_ThematicDefault, (localMinMaxCode == kThematicDefault));

    //	Percent to use for clipping. 													

    LoadDItemValue(dialogPtr, IDC_PercentClipped, (SInt32) * percentTailsClippedPtr);
        ...
        if (itemHit > 2) {
            ...
            switch (itemHit) {
                ...
                case IDC_ThematicDefault:
                    ...
                    SetDLogControl(dialogPtr, IDC_PerChannel, (itemHit == IDC_PerChannel));
                    SetDLogControl(dialogPtr, IDC_EntireRange, (itemHit == IDC_EntireRange));
                    SetDLogControl(dialogPtr, IDC_UserSpecified, (itemHit == IDC_UserSpecified));
                    SetDLogControl(dialogPtr, IDC_ThematicDefault, (itemHit == IDC_ThematicDefault));
                ...
                case IDC_PercentClipped: //	 Percent to use to clip at  
                    ...
                    if (theNum > 100)
                        NumberDefault(
                            theHandle, (SInt32) * percentTailsClippedPtr, &gTextString);
            ...
            if (checkValueFlag) {
                ...
                if (valueErrorFlag) {
                    if (histogramSummaryPtr != NULL) {
                        numberEDecimalDigits = histogramSummaryPtr[summaryIndex].numberEDecimalDigits;
                        numberFDecimalDigits = histogramSummaryPtr[summaryIndex].numberFDecimalDigits;

                    }// end "if (histogramSummaryPtr != NULL)"

                    else // histogramSummaryPtr == NULL
                    {
                        numberEDecimalDigits = gImageFileInfoPtr->maxNumberEDecimalDigits;
...
void MinMaxEnhancementDialogInitialize(
        ...
        Boolean* histogramAvailableFlagPtr)
 {
    ...
    *localMinMaxCodePtr = minMaxSelection;
    ...
    // Display appropriate dialog items depending on the type of			
    // display.	
    ...
    switch (displayType) {
        ...
        case 4: // 2 channel display 
            ...
            if (rgbColors & 2) {
                if (*histogramSummaryPtrPtr != NULL) {
                    greenMinValue = (*histogramSummaryPtrPtr)[
                            greenChannel - 1].minValue;

                    greenMaxValue = (*histogramSummaryPtrPtr)[
                            greenChannel - 1].maxValue;
            ...
            if (rgbColors & 1) {
                if (*histogramSummaryPtrPtr != NULL) {
                    blueMinValue = (*histogramSummaryPtrPtr)[
                            blueChannel - 1].minValue;

                    blueMaxValue = (*histogramSummaryPtrPtr)[
                            blueChannel - 1].maxValue;
...
//	Revised By:			Larry L. Biehl			Date: 06/25/2012
...
#if defined multispec_lin
	wxChoice* comboBoxPtr = (wxChoice*) (dialogPtr->FindWindow(ID3C_MinMaxValues));
...
#if defined multispec_mac				
    if (gAppearanceManagerFlag) {
        ShowHideDialogItem(dialogPtr, 28, (displayType != kSideSideChannelDisplayType));
...
void UpdateEnhancementMinMaxes(
        ...
        Boolean updateUserSpecifiedFlag)
 {
    ...
    SInt32 //							minBlueChannelIndex,
    //							minGreenChannelIndex,
    //							minRedChannelIndex,
    //							maxBlueChannelIndex,
    //							maxGreenChannelIndex,
    //							maxRedChannelIndex,
    signedValueOffset;
    ...
    if (histogramSummaryPtr != NULL) {
        ...
        if (localMinMaxCode == kPerChannelMinMax) {
            ...
            if (channelsPtr[0] >= 0) {
                //				minRedChannelIndex = 0;
                //				maxRedChannelIndex = histogramSummaryPtr[channelsPtr[0]-1].numberBins - 1;
            ...
            if (channelsPtr[1] >= 0) {
                //				minGreenChannelIndex = 0;
                //				maxGreenChannelIndex = histogramSummaryPtr[channelsPtr[1]-1].numberBins - 1;
            ...
            if (channelsPtr[2] >= 0) {
                //				minBlueChannelIndex = 0;
                //				maxBlueChannelIndex = histogramSummaryPtr[channelsPtr[2]-1].numberBins - 1;
        ...
        if (loadRedChannelLimitsFlag) {
            //			redMinDataValue = GetDataValueForBinIndex (minRedChannelIndex,
            //															&histogramSummaryPtr[channelsPtr[0]-1],
            //															signedValueOffset);

            //			redMaxDataValue = GetDataValueForBinIndex (maxRedChannelIndex,
            //															&histogramSummaryPtr[channelsPtr[0]-1],
            //															signedValueOffset);

            redNumberEDecimalDigits = histogramSummaryPtr[channelsPtr[0] - 1].numberEDecimalDigits;
            redNumberFDecimalDigits = histogramSummaryPtr[channelsPtr[0] - 1].numberFDecimalDigits;
        ...
        if (loadGreenChannelLimitsFlag) {
            //			greenMinDataValue = GetDataValueForBinIndex (minGreenChannelIndex,
            //															&histogramSummaryPtr[channelsPtr[1]-1],
            //															signedValueOffset);

            //			greenMaxDataValue = GetDataValueForBinIndex (maxGreenChannelIndex,
            //															&histogramSummaryPtr[channelsPtr[1]-1],
            //															signedValueOffset);

            greenNumberEDecimalDigits = histogramSummaryPtr[channelsPtr[1] - 1].numberEDecimalDigits;
            greenNumberFDecimalDigits = histogramSummaryPtr[channelsPtr[1] - 1].numberFDecimalDigits;
        ...
        if (loadBlueChannelLimitsFlag) {
            //			blueMinDataValue = GetDataValueForBinIndex (minBlueChannelIndex,
            //															&histogramSummaryPtr[channelsPtr[2]-1],
            //															signedValueOffset);

            //			blueMaxDataValue = GetDataValueForBinIndex (maxBlueChannelIndex,
            //															&histogramSummaryPtr[channelsPtr[2]-1],
            //															signedValueOffset);

            blueNumberEDecimalDigits = histogramSummaryPtr[channelsPtr[2] - 1].numberEDecimalDigits;
            blueNumberFDecimalDigits = histogramSummaryPtr[channelsPtr[2] - 1].numberFDecimalDigits;
