...
/* Template for debugging
		int numberChars2 = sprintf ((char*)&gTextString3,
												" SMapTran::SetCoordinateViewLocationParameters (linWidth, width): %d, %d%s",
												linWidth,
												width,
												gEndOfLine);
	...
	if (mapProjectionInfoPtr != NULL)
		...
		if (mapProjectionInfoPtr->planarCoordinate.polynomialOrder <= 0)
			...
			if (planarCoordinateSystemInfoPtr->mapOrientationAngle != 0)
				...
				xOffset = (mapPointPtr->h - xOrientationOrigin - sinOrAngle*yOffset)/cosOrAngle;
...
//	#ifdef multispec_lin
				// Magnification is not need for linux. It is already take into account.
//		magnification = 1;
...
//		#endif
//		#if defined multispec_lin
//			magnification = 1; // In Linux, draw on bitmap where magnification doesnt matter
//		#endif
...
//			boundingMapRectPtr->right = MAX(mapRect.left, mapRect.right);
			...
			if (mapProjectionInfoPtr->planarCoordinate.mapOrientationAngle != 0)
				GetBoundingOrientationAngleRect ( // boundingMapRectPtr,
															&mapRect,
...
//		mapProjectionInfoPtr->planarCoordinate.areaUnitsCode = areaUnitsCode;
		...
		if (adjustUpperLeftMapPointFlag)
			...
			mapProjectionInfoPtr->planarCoordinate.xMapCoordinate11 = upperLeftLatLongPoint.h;
			mapProjectionInfoPtr->planarCoordinate.yMapCoordinate11 = upperLeftLatLongPoint.v;
			
			mapProjectionInfoPtr->planarCoordinate.xMapOrientationOrigin = upperLeftLatLongPoint.h;
			mapProjectionInfoPtr->planarCoordinate.yMapOrientationOrigin = upperLeftLatLongPoint.v;
...
{	
	...
	CoordinateDialogHandleInvalidValidEPSGCode (
											dialogPtr,
											epsgCode,
											parametersSetFlag,
...

{
	...
	switch (referenceSystemCode)
		...
		case kGeographicRSCode:				
			...
			hideProjectionComboFlag = TRUE;
			if (!initialFlag)
//				gMapUnitsSelection = kDecimalDegreesCode + 1;
		...
		case kByEPSGCodeCode:
			...
			if (!initialFlag)
//				gMapUnitsSelection = kMetersCode + 1;
				mapUnitsCode = kMetersCode;
...
//						gMapUnitsSelection = kUSSurveyFeetCode + 1;
...
{		
	...
	if (gPresentCursor == kCross)
		...
		int numberChars = sprintf ((char*)&gTextString3,
												" SMapTran::DrawCursorCoordinates (imageViewCPtr, imageFrameCPtr): %d, %d%s",
												imageViewCPtr, 
												imageFrameCPtr,
												gEndOfLine);
...

{
	...
	DrawPopUpMenuItem (dialogPtr, itemNumber, gPopUpMapUnitsMenu, 
											gMapUnitsSelection, TRUE);
...

{
	...
	DrawPopUpMenuItem (dialogPtr, itemNumber, gPopUpHorizontalDatumMenu, 
											gDatumSelection, TRUE);
...

{
	...
	DrawPopUpMenuItem (dialogPtr, itemNumber, gPopUpEllipsoidMenu, 
											gEllipsoidSelection, TRUE);
...

{
	...
	DrawPopUpMenuItem (dialogPtr, itemNumber, gPopUpProjectionMenu, 
											gProjectionSelection, TRUE);
...

{
	...
	DrawPopUpMenuItem (dialogPtr, itemNumber, gPopUpReferenceSystemMenu, 
											gReferenceSystemSelection, TRUE);
...
{	
	...
	if (displaySpecsPtr != NULL)
		...
		if (scale > 0)
			...
			numberTitleChars = sprintf( (char*)&scaleTitleString, 
											"Scale");
			...
			if (scale >= 1)
				...
				numberValueChars = InsertCommasInNumberString (
											(char*)&gTextString2, 
											numberValueChars, 
											-1,
											numberValueChars);
			...
			CMImageView* imageViewCPtr = GetWindowPtr(windowInfoHandle);
			CMImageFrame* 	imageFrameCPtr = imageViewCPtr->GetImageFrameCPtr();
			imageFrameCPtr->UpdateScaleInformation( scale,
																	(char*)scaleValueString);
...

{
	...
	if (coordinateViewUnits == kLineColumnUnitsMenuItem)
		...
		/*
		int numberChars2 = sprintf ((char*)&gTextString3,
												" SMapTran::DrawSelectionCoordinates (imageViewCPtr, imageFrameCPtr): %d, %d%s",
												imageViewCPtr, 
												imageFrameCPtr,
												gEndOfLine);
		ListString ((char*)&gTextString3, numberChars2, gOutputTextH); 
		*/
		#if defined multispec_lin
			((wxStaticText*)imageFrameCPtr->m_coordinatesBar->FindWindow(IDC_SelectionLine))->Show(true);
			((wxStaticText*)imageFrameCPtr->m_coordinatesBar->FindWindow(IDC_SelectionColumn))->Show(true);
		#endif	// multispec_lin
	...
	if (windowRectPtr != NULL && windowCode == kToImageWindow)
      ...
      /*
		SInt16 numberChars = sprintf ((char*)&gTextString3,
													" ready to call ConvertWinRectToMapRect: %ld, %ld, %ld, %ld %s", 
													windowLongRect.top,
													windowLongRect.bottom,
													windowLongRect.left,
													windowLongRect.right,
													gEndOfLine);
		ListString ((char*)&gTextString3, numberChars, gOutputTextH); 
		*/
...
{ 
	...
	mapProjectionInfoPtr = (MapProjectionInfoPtr)GetHandlePointer(
											fileInfoPtr->mapProjectionHandle, kNoLock, kNoMoveHi);
...

{
	...
	if (mapProjectionInfoPtr->gridCoordinate.projectionCode == kOrthographicCode)
		...
		}		// end "else if (...->gridCoordinate.projectionCode == kLambertAzimuthalEqualAreaCode)"
...

{
	...
	if (windowInfoPtr != NULL && mapProjectionInfoPtr != NULL)
		...
		windowInfoPtr->boundingLatLongRectangle.left = windowInfoPtr->boundingMapRectangle.left * factor;
		windowInfoPtr->boundingLatLongRectangle.top = windowInfoPtr->boundingMapRectangle.bottom * factor;
		windowInfoPtr->boundingLatLongRectangle.right = windowInfoPtr->boundingMapRectangle.right * factor;
		windowInfoPtr->boundingLatLongRectangle.bottom = windowInfoPtr->boundingMapRectangle.top * factor;
		...
		boundingLatLongRectangle.left = windowInfoPtr->boundingMapRectangle.left * factor;
		boundingLatLongRectangle.top = windowInfoPtr->boundingMapRectangle.bottom * factor;
		boundingLatLongRectangle.right = windowInfoPtr->boundingMapRectangle.right * factor;
		boundingLatLongRectangle.bottom = windowInfoPtr->boundingMapRectangle.top * factor;
		ConvertMapRectToLatLongRect (mapProjectionInfoPtr,
												gridCode,
												&boundingLatLongRectangle);
												
		windowInfoPtr->boundingLatLongRectangle.left = MIN(windowInfoPtr->boundingLatLongRectangle.left,
																				boundingLatLongRectangle.left);
												
		windowInfoPtr->boundingLatLongRectangle.right = MAX(windowInfoPtr->boundingLatLongRectangle.right,
																				boundingLatLongRectangle.right);
												
		windowInfoPtr->boundingLatLongRectangle.top = MIN(windowInfoPtr->boundingLatLongRectangle.top,
																				boundingLatLongRectangle.bottom);
												
		windowInfoPtr->boundingLatLongRectangle.bottom = MAX(windowInfoPtr->boundingLatLongRectangle.bottom,
...
//											yMapCoordinate11;
	...
	numberChars = sprintf( (char*)&gTextString, 
		...
		GetPort (&savedPort);
//		SetPort (GetWindowPtr (windowInfoHandle));
	...
	SetCoordinateViewSelectionStart (
		...
		//((wxStaticText*)(coordinatesBar->FindWindow(IDC_CursorLine)))->SetWindowStyle (wxST_NO_AUTORESIZE);
		((wxStaticText*)(coordinatesBar->FindWindow(IDC_CursorLine)))->GetSize (&linWidth, &linHeight);
		((wxStaticText*)(coordinatesBar->FindWindow(IDC_CursorLine)))->SetSize (width, linHeight);
	
		//((wxStaticText*)(coordinatesBar->FindWindow(IDC_CursorColumn)))->SetWindowStyle (wxST_NO_AUTORESIZE);
		((wxStaticText*)(coordinatesBar->FindWindow(IDC_CursorColumn)))->GetSize (&linWidth, &linHeight);
		((wxStaticText*)(coordinatesBar->FindWindow(IDC_CursorColumn)))->SetSize (width, linHeight);
	#endif	// defined multispec_lin
	...
	#if defined multispec_lin
		((wxStaticText*)(coordinatesBar->FindWindow(IDC_SelectionLine)))->GetSize (&linWidth, &linHeight);
		((wxStaticText*)(coordinatesBar->FindWindow(IDC_SelectionLine)))->SetSize (width, linHeight);
	
		((wxStaticText*)(coordinatesBar->FindWindow(IDC_SelectionColumn)))->GetSize (&linWidth, &linHeight);
		((wxStaticText*)(coordinatesBar->FindWindow(IDC_SelectionColumn)))->SetSize (width, linHeight);
	#endif	// defined multispec_lin
...
{   
	if (mapProjectionInfoPtr != NULL)
		...
		mapProjectionInfoPtr->gridCoordinate.latitudeOrigin = latitudeOrigin;
		mapProjectionInfoPtr->gridCoordinate.longitudeCentralMeridian = longitudeCentralMeridian;
...
{   
	if (mapProjectionInfoPtr != NULL)
		...
		mapProjectionInfoPtr->gridCoordinate.latitudeOrigin = latitudeOrigin;
		mapProjectionInfoPtr->gridCoordinate.longitudeCentralMeridian = longitudeCentralMeridian;
		mapProjectionInfoPtr->gridCoordinate.scaleFactorOfCentralMeridian = scaleFactorOfCentralMeridian;
	...
	switch (order)
		...
		case 3:
			...
			*eastingPtr = eCoefPtr[0] +
					eCoefPtr[1] * inputEasting + eCoefPtr[2] * inputNorthing  +
					eCoefPtr[3] * e2 + eCoefPtr[4] * en  + eCoefPtr[5] * n2  +
					eCoefPtr[6] * e3 + eCoefPtr[7] * e2n + eCoefPtr[8] * en2 + eCoefPtr[9] * n3;
				
			*northingPtr = nCoefPtr[0] +
					nCoefPtr[1] * inputEasting + nCoefPtr[2] * inputNorthing  +
					nCoefPtr[3] * e2 + nCoefPtr[4] * en  + nCoefPtr[5] * n2  +
					nCoefPtr[6] * e3 + nCoefPtr[7] * e2n + nCoefPtr[8] * en2 + nCoefPtr[9] * n3;
			break;
