		...
		if ( gProjectInfoPtr->listMatrixType & 
			...
			if (returnFlag)
				...
				if (covarianceStatsToUse == kOriginalStats)
					...
					else if (classNamesPtr[classStorage].mixingParameterCode == kIdentityMatrix)
...
// Value Returned:	None	
// 
// Called By:			ListProjectStats in SStatLst.cpp
//							ListStatsControl in SStatLst.cpp
//
...
// Value Returned:	None
// 
// Called By:			HistogramFieldStats in statHistogram.c
...
*/		
	...
	if (numberPixels > 0)
		{
		strLength = sprintf ((char*)&gTextString, " Number of samples = %lld", numberPixels);
		
		sprintf ((char*)&gTextString[strLength], " Number of samples used for statistics = %lld", numberPixelsUsedForStats);
...

{
	...
	for (channel=0; channel<numberChannels; channel++)
		{
		sprintf (stringPtr1,  "\t%*hd", 
									fieldSize, gProjectInfoPtr->channelsPtr[channel]+1);
		stringPtr1 += stringSkip;
			
		}		// end "for (channel=0; channel<numberChannels; ..." 
				
	sprintf(stringPtr1, gEndOfLine);
	stringLength = (Ptr)stringPtr1 - (Ptr)gCharBufferPtr1 + 
	...
	for (channel=0; channel<numberChannels; channel++)
		...
		for (covChan=0; covChan<=channel; covChan++)
			{
//			sprintf (stringPtr1, "\t%*.*f", fieldSize, precision, *matrixPtr);
...
// Value Returned:	None
// 
// Called By:			ListClassStats in statList.c
//							ListFieldStats in statList.c
//
//	Coded By:			Larry L. Biehl			Date: 11/21/1988
//	Revised By:			Larry L. Biehl			Date: 03/25/2014	
...

{
	...
	if (continueFlag && (gProjectInfoPtr->listMatrixType & kListMeansStdDev))
		...
		if (continueFlag)
			...
			stringLength = (Ptr)stringPtr2 - (Ptr)gCharBufferPtr2 + 
															gNumberOfEndOfLineCharacters;
	...
	if (continueFlag && numberPixels > 0)
		...
		if (gProjectInfoPtr->listMatrixType & kListCovariance)
			...
			if (!ListString( 
					gCharBufferPtr1, strlen(gCharBufferPtr1),  gOutputTextH) )
																							return;
...

{
	...
	if (statsWindowMode >= 2 && statsWindowMode <= 4)
		...
		if (continueFlag)
			...
			gCharBufferPtr2 =  (HPtr)GetHandlePointer(
															gCharBufferHandle2,
															kLock,
															kNoMoveHi);
			
			gCharBufferPtr3 =  (HPtr)GetHandlePointer(
															gCharBufferHandle3,
															kLock,
															kNoMoveHi);
	...
	
{
	...
	dialogPtr = LoadRequestedDialog (kStatListSpecificationID, kDoNotCopyScrap, 1, 2);
	if (dialogPtr == NULL)												
																					return (FALSE);
