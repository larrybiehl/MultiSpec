...
//	Revised By:			Larry L. Biehl			Date: 03/04/2017
...
{   
	...
	fileNamePtr = (char*)GetFileNamePPointer (fileInfoPtr);
			...
			if (errCode == noErr)
				...
				if (errCode == noErr)
					...
					appendToFileInfoPtr =  (FileInfoPtr)GetHandlePointer(
													appendToFileInfoHandle, kLock, kNoMoveHi);
			    	...
			    	fileInfoLoadedFlag = 
			    				LoadImageInformation (NULL, appendToFileInfoHandle, FALSE, NULL, 0);
	...
	if (errCode == noErr)
		...
		fileInfoPtr->channelToHdfDataSetHandle = appendToFileInfoPtr->channelToHdfDataSetHandle;
		...
		GetHierarchalFilePointers (NULL, fileInfoPtr);
				...
				// This is not to be done for thematic image files or a multispectral image
				// file that is just being modified.
		
		if (!fileInfoPtr->thematicType &&
				!modifyFlag &&
				(*writeChannelDescriptionsFlagPtr || gImageWindowInfoPtr->descriptionCode == 0) && 		
			...
			appendToFileNumberChannels = fileInfoPtr->numberChannels - inputNumberChannels;
			...
			if (fileInfoPtr->channelDescriptionH != NULL)		
				...
				*writeChannelDescriptionsFlagPtr = TRUE;
				...
				if (appendToFileInfoPtr->channelDescriptionH != NULL)
					...
					BlockMoveData (appendToDescriptionPtr, descriptionPtr, (UInt32)numberBytes);
			...
			if (fileInfoPtr->channelValuesHandle != NULL)
				...
				if (appendToFileInfoPtr->channelValuesHandle != NULL)
					...
					BlockMoveData (appendToChannelValuesPtr, channelValuesPtr, (UInt32)numberBytes);
...
//													appendFileInfoPtr->numberChannels == 1);
	...
	if (dialogPtr != NULL)
		...
		StringPtr appendFileNamePtr = (StringPtr)GetFileNamePPointer (appendFileInfoPtr);
...
{  
	...
	if (ChangeErdasHeaderDialog (gImageFileInfoPtr, &requestedFormat) )
		...
		if ( gImageFileInfoPtr->format == 0 )
			...
			headerRecordPtr =  (unsigned char*)GetHandlePointer(
													headerRecordH, kLock, kNoMoveHi);
...

{
	...
	DrawPopUpMenuItem (dialogPtr, itemNumber, 
						gPopUpHeaderOptionsMenu, gHeaderOptionsSelection, TRUE);
...

{
	...
	DrawPopUpMenuItem (dialogPtr, itemNumber, 
						gPopUpOutputFileMenu, gOutputFileSelection, TRUE);
...

{
	...
	if (gProcessorCode == kRefFieldsToThematicFileProcessor)
		inFileNamePtr = (FileStringPtr)GetFileNamePPointer (gProjectInfoPtr);
			
	else		// gProcessorCode != kRefFieldsToThematicFileProcessor
		inFileNamePtr = (FileStringPtr)GetFileNamePPointer (inputFileInfoPtr);
	...
	if (inFileNamePtr != NULL && outFileNamePtr != NULL)
		...
		if (gProcessorCode != kRefFieldsToThematicFileProcessor)
			...
			if (gImageWindowInfoPtr != NULL && gImageWindowInfoPtr->numberImageFiles > 1)
				{			
						// Try to use the file name base which is common to all of the linked
						// files.
...

{
	...
	outFileNamePtr = (FileStringPtr)GetFileNamePPointer (outFileStreamPtr);
//	MGetString (outFileNamePtr, kFileIOStrID, IDS_ImageFile);
	...
	inFileNamePtr = (FileStringPtr)GetFileNamePPointer (gImageFileInfoPtr);
	
	if (inFileNamePtr != NULL)
		{
		//if (!strcmp ((char*)inFileNamePtr, (char*)outFileNamePtr) && outFileNamePtr[0] < 251)
		if (!StringCompare (inFileNamePtr, outFileNamePtr) && outFileNamePtr[0] < 251)
			...
			if (outFileNamePtr[stringLength-3] == _T('.'))
				...
				//StringCopyCharacters (&savedSuffixPtr[1], &outFileNamePtr[stringLength-3], 4);
				strncpy ((char*)&gTextString[1], (char*)&outFileNamePtr[stringLength-3], 4);
				savedSuffixPtr[0] = 4;
			...
			if (gMultiSpecWorkflowInfo.workFlowCode == 0)
				...
				//StringCopyCharacters ((TBYTE*)&outFileNamePtr[stringLength-3], (TBYTE*)_T("_new"), 4);
				
				}		// end "if (gMultiSpecWorkflowInfo.workFlowCode == 0)"
				
			else		// gMultiSpecWorkflowInfo.workFlowCode != 0
				{
				tempStringPtr = (UCharPtr)strcpy ((char*)&outFileNamePtr[1], (char*)&gMultiSpecWorkflowInfo.defaultName[1]);
				//tempStringPtr = StringCopy ((TBYTE*)&outFileNamePtr[1], 
				//										(TBYTE*)&gMultiSpecWorkflowInfo.defaultName[1]);
				stringLength = strlen ((char*)tempStringPtr);
				//strcpy (&tempStringPtr[stringLength], (char*)&gTextString2[0]);
				//stringLength = strlen ((char*)&outFileNamePtr[1]) - 4;
			...
			if (savedSuffixPtr[0] == 4)
				...
				//StringCopyCharacters (&outFileNamePtr[stringLength+1], &savedSuffixPtr[1], 4);
				//
...
#	endif
	...
	if (reformatOptionsPtr->outputFileCode == kNewFileMenuItem)
		...
		outFileNamePtr = (FileStringPtr)GetFilePathPPointer (outFileStreamPtr);
		...
		{
		errCode = AppendFile (outFileInfoPtr, 
										gNumberFileTypes, 
...

{
	...
	if (errCode == noErr)
		...
		continueFlag = (ioBufferPtr != NULL);												
			...
			// Display any disk io error at this point except for disk full.  It	
...
*/												
	...
	if (continueFlag && gProcessorCode == kRefMosaicImagesProcessor)
		...
		fileNamePtr = (FileStringPtr)GetFileNameCPointer (mosaicFileInfoHandle);
	...
	fileNamePtr = (FileStringPtr)GetFileNameCPointer (outFileInfoPtr);
	sprintf ((char*)&gTextString,
				...
				
{
	...
	if (bytesToAdd > 0)
		...
		fileNamePtr = (char*)GetFileNamePPointer (fileStreamPtr);
		vRefNum = GetVolumeReferenceNumber (fileStreamPtr);
...

{
	...
	if (errCode == noErr && appendToFileFlag && fileInfoPtr->channelDescriptionH != NULL)
		{
