...
//		    		DebugStr ("\pprincipalComponents.c");
//
//	File:						principalComponents.c
//
			...
			
{
	...
	if (gNonProjProcessorSpecs.principalCompSpecsH == NULL)
		...
		if (gNonProjProcessorSpecs.principalCompSpecsH != NULL)
			...
			gPrincipalCompSpecsPtr->fileInfoVersion = gImageWindowInfoPtr->fileInfoVersion;
	...
	if (returnFlag)
		{
		bytesNeeded =
			(SInt32)gImageWindowInfoPtr->totalNumberChannels * sizeof(short int);
			...
			
{
	...
	if (continueFlag)
		...
		numberBytes = 
			(UInt32)gPrincipalCompSpecsPtr->numberChannels *
	...
	if (continueFlag)
		{		
		numberBytes = 
			(UInt32)gPrincipalCompSpecsPtr->numberChannels * sizeof(SInt16);
...
/*	
		if ( SetupMatrixInversionMemory ( (UInt16)gPrincipalCompSpecsPtr->numberChannels,
														...
														NULL) )
			{												
			if (ComputeEigenvectorsSVD (covariancePtr, 
												gPrincipalCompSpecsPtr->numberChannels, 
												gPrincipalCompSpecsPtr->numberChannels, 
...
//				"\pListing Eigenvalues and Eigenvectors" );
	...
	doubleWorkVectorPtr = CheckAndDisposePtr (doubleWorkVectorPtr);
	...
	if (eigenvectorOKFlag && gPrincipalCompSpecsPtr->saveEigenvectorFlag)
		...
		eigenvectorOKFlag = SaveTransformationMatrix (1, 
										gPrincipalCompSpecsPtr->eigenVectorHandle,
										gPrincipalCompSpecsPtr->eigenValueHandle,
										gPrincipalCompSpecsPtr->channelsHandle,
										NULL,
...
{			
	...
	classPtr = (short int*)GetHandlePointer(
									gPrincipalCompSpecsPtr->classHandle,
									kNoLock,
									kNoMoveHi);
									
	featurePtr = (short int*)GetHandlePointer(
									gPrincipalCompSpecsPtr->featureHandle,
									kNoLock,
									kNoMoveHi);
									
	channels = (short int*)GetHandlePointer(
									gPrincipalCompSpecsPtr->channelsHandle,
									kNoLock,
									kNoMoveHi);
...
{   
	...
	if (principalCompSpecsPtr != NULL)
		...
		if (principalCompSpecsPtr->channelSet == kAllMenuItem)	// All channels or features 
			LoadFeatureVector (kNoTransformation,
