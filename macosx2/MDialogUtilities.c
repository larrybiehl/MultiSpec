...
{                 
	...
	if (gDialogListHandle != NULL)
		{
		numberSelectedChannels = AddChannelsToDialogList (gDialogListHandle,
				...
				if (selEnd-selStart != stringLength)
					...
					while (i<=stringLength && decimalLocation == 256)
						...
						i++;
							...
							// Limit the number of decimal places to decimalMaximum.													
...
{ 
	...
	if (newValue < minimumValue)
		NumberErrorAlert (currentInterval, 
								dialogPtr, 
								itemHit);
...
void DialogLineColumnHits (
				...
				SInt32								theNum)
{
	DialogLineColumnHits (dialogSelectAreaPtr,
									dialogPtr,
									itemHit,
									theHandle,
									theNum,
...

{
	...
	switch (parameter)
		...
		default:		// Entire area to selected area switch.			
			UpdateLineColumnValues (imageWindowInfoPtr,
	...
	if (dialogSelectAreaPtr->entireIconItem != 0 && parameter >= 1 && parameter <= 6)
		{
...
// Value Returned: 	None
//
// Called By:			DisplayThematicDialog in SDisplay.cpp
//
...
//									All rights reserved.
//
//	Function name:		SInt16 GetDLogControl
//
...

{
	...
	if (dlogHandle == NULL)
		...
		where->v = 	dialogRectPtr->top + 
				...
				// Adjust so that 1/5 of space is above and 4/5 of space is below.																			
...

{
	...
	if (listHandle != NULL)
		{
				// Make certain that the list view is just tall enough for an integer 
				// number of lines.													
...
//
//	Software purpose:	The purpose of this routine is to hide the requested continuous 
				...
				
{
	...
	SInt32								resourceSize;
			...
			// First verify that memory is available for the modal filter if requested.																			
			...
			if (itemHit > 2)
				...
				switch (itemHit)
					...
					case 3:		// Select all items in the list. 
						...
						LUpdate (GetPortVisibleRegion (
														GetDialogPort (dialogPtr), gTempRegion1), 
									gDialogListHandle);
						SetEmptyRgn (gTempRegion1);
					...
					case 11:		// List end 
						if (theNum == 0 || theNum > numberInputVecItems)
							NumberErrorAlert ((SInt32)listEnd, dialogPtr, itemHit);
					...
					case 13:		// List interval 
						if (theNum == 0 || theNum > numberInputVecItems)
							NumberErrorAlert ((SInt32)listInterval, dialogPtr, itemHit);
...

{
	...
	//if (what == updateEvt && 
	//					(WindowPtr)theEventPtr->message != GetDialogWindow (theDialogPtr))
		{
		...
		*itemHitPtr = 0;
		...
		if (errorReturn == noErr)
			...
			if (what == activateEvt || what == updateEvt)
				{
						// Get the hilite of the OK button. Make sure that that hilite is 
						// not changed upon exit from standardProc.
			...
			*/				
			returnFlag = InvokeModalFilterUPP (
										theDialogPtr, theEventPtr, itemHitPtr, standardProc);
		...
		switch (what)
			{
	 		case mouseDown:
	      		...
	      		case inContent:
						...
						else if (gProcessorCode == kAboutMultiSpecProcessor)
							{			
...

{
	...
	if (theType == kUserDialogItem)
		SetDialogItem (
						dialogPtr, itemNumber, theType, (Handle)userItemProcPtr, rectPtr);
...

{
	...
	if (gAppearanceManagerFlag)
		...
		errorCode = ::GetDialogItemAsControl (
													dialogPtr, appearanceItemNumber, &theControl);
		
		if (errorCode == noErr)
			{
					// The following is done to be sure that the popup menus that were 
					// loaded at the beginning are used. When all dialogs are converted to 
					// take advantage of the Appearance manager, setting preloaded popup 
					// menu handles will not have to be done.
					
			errorCode = GetControlData (theControl, 
													0, 
			...
			if (popupMenuHandle != inputPopupMenuHandle)
				{
				errorCode = SetControlData (theControl, 
														0, 
														kControlPopupButtonMenuHandleTag, 
														sizeof (MenuHandle),
...
{	
	...
	if (theType != kUserDialogItem)
		...
		osErr = GetBevelButtonContentInfo (theControl, &contentInfo);
		
		contentInfo.u.resID = resID;
		osErr = SetBevelButtonContentInfo (theControl, &contentInfo);
		...
		if (ButtonDownEvent (theBoxPtr, TRUE))
			...
			if (!gEntireImageFlag)
				...
				*columnEnd = windowInfoPtr->maxNumberColumns;
				...
				if (GetSelectionRectangle (windowPtr, 
													&gTempLongRect, 
