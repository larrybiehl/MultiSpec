...
//
//	Software purpose:	This routine handle mouse down events in the content part of
//							a window.
//
//	Parameters in:				
//
...

{
	...
	updateRegionH = NewRgn ();
		...
		GetWindowRegion (window, kWindowUpdateRgn, updateRegionH);
			...
			//OSStatus returnCode = GetWindowRegion (
			//											window, kWindowUpdateRgn, updateRegionH);
	...
	if (callCoordinateUpdateFlag)
		...
		DrawOneControl (((WindowInfoPtr)*windowInfoHandle)->coordinateUnitsControl);
		DrawOneControl (
...
{	
	...
	MoveTo (legendLeftStart + 4, legendTopStart + 11);
	...
	if (drawPopUpBoxFlag && 
		...
		GetThematicPopupBox (&legendTitlePopUpBox, windowInfoPtr->windowInfoHandle);
...
//
//	Software purpose:	The purpose of this routine is to get the rectangle that
//							encloses the thematic legend popup box.
//
//	Parameters in:		None
//
//	Parameters out:	None
...
//
//	Software purpose:	The purpose of this routine is to handle the mouse down movement
//							within the legend list that is displayed within the Thematic
//							image windows.
//		
//	Parameters in:				
//
...

{
	...
	if (gPresentCursor == kBlinkOpenCursor1)
		...
		if (spectralClassFlag &&
				lCell.h >= 0 &&
					lCell.v >= 0 &&
						lCell.v < ((ListPtr)*legendListHandle)->dataBounds.bottom &&
...

{
	...
	offScreenMapHandle = ((WindowInfoPtr)*gActiveImageWindowInfoH)->offScreenMapHandle;
	...
	SInt16 grafPortType = ((WindowInfoPtr)*gActiveImageWindowInfoH)->grafPortType;
	if (((FileInfoPtr)*gActiveImageFileInfoH)->numberGroups > 0 && 
		...
		DisableMenuItem (gPopUpClassGroupMenu2, kGroupDisplay);
			...
			// Display the popup menu and let the user select the desired option.
	...
	if (gOSXCoreGraphicsFlag)
		{
			...
			rowBytes =
					((WindowInfoPtr)*gActiveImageWindowInfoH)->cgInfo.contextRowBytes;
			
			boundsBottom =
						((DisplaySpecsPtr)*displaySpecsH)->imageDimensions[kVertical];
			boundsRight =
						((DisplaySpecsPtr)*displaySpecsH)->imageDimensions[kHorizontal];
...
//
//	Software purpose:	The purpose of this routine is to handle mouse down events in
//							the legend part of the thematic image window.
//
//	Parameters in:		None
//
//	Parameters out:	None
...

{
   ...
   bytesPerRow = 100;
	...
	offScreenBufferPtr = GetHandlePointer (imageBaseAddressH);
				
			// Note that the image base address handle is already locked.
			
	provider = gCGDataProviderCreateWithDataPtr (NULL,
																offScreenBufferPtr, 
																offscreenMapSize, 
																NULL);
	...
	colorSpace = gCGColorSpaceCreateIndexedPtr (baseColorSpace,
																numberEntries-1, 
																osxColorTablePtr);
	
			// Create the image.
			
	((WindowInfoPtr)*gActiveImageWindowInfoH)->cgInfo.imageRef =
											gCGImageCreatePtr (imageWidth,
																		imageHeight,
																		8,
																		8,
																		bytesPerRow,
